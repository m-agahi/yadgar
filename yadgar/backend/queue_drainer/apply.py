...
def _apply_span():
    ...
    except Exception:  # BLE001-KEEP: span construction per replayed write: a degraded or swapped OTel provider raises arbitrary types from get_tracer (the I3 case documented in tracing.py), and the caller must fall through to nullcontext
...
def _set_apply_op(op: str) -> None:
    ...
    except Exception:  # BLE001-KEEP: same degraded-provider surface; setting the op attribute is decoration and must never fail the replay
        pass
