...
def phase_contradiction(ctx: MemorizeContext) -> dict | None:
    ...
    except Exception as exc:  # noqa: BLE001 — outer guard over the whole conflict-resolver phase: it spans an Ollama HTTP call, JSON decode and storage writes, which share no common base, and any failure must degrade to a plain ADD rather than lose the memory
...
def _handle_update(ctx: MemorizeContext, target_id: int, cr_reason: str) -> dict | None:
    ...
    except Exception as exc:  # noqa: BLE001 — the UPDATE arm: storage raises with no common base, and a failed update must fall through to ADD so the write is never lost
...
def _handle_delete(target_id: int, cr_reason: str) -> dict:
    ...
    except Exception as exc:  # noqa: BLE001 — the DELETE arm: storage raises with no common base, and a failed delete must still return the resolver's decision envelope
