...
def _bump_cache_epoch(ctx: MemorizeContext) -> None:
    ...
    except Exception:  # pragma: no cover  # BLE001-KEEP: _bump_epoch_for_context spans a lazy import, git-root resolution and the on-disk epoch store, which share no common base, and cache invalidation must never fail the write it follows
        pass
...
def _run_engram(ctx: MemorizeContext) -> None:
    ...
    except Exception:  # BLE001-KEEP: post-write engram allocation: it reaches storage and the slot allocator, which share no common base, and the memory is already committed so a failed allocation must not fail the write
...
def _zero_gap_4_micro_checkpoint(ctx: MemorizeContext, settings) -> None:
    ...
    if should_micro:
        ...
        except Exception:  # BLE001-KEEP: post-write micro-checkpoint: create_micro_checkpoint reaches storage with no common base, and the memory is already committed
    ...
    try:
        ...
        for r in related:
            ...
            if len(ctx.related_context) >= settings.REINJECTION_MAX_RESULTS:
                break
    except Exception:  # BLE001-KEEP: post-write reinjection recall: it runs the retrieval pipeline (storage + embeddings + rerankers) with no common base, and the memory is already committed so a failed recall only costs the related-context block
