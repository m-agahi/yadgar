...
class GraphAPINodesMixin:
    ...
    def _assemble_memory_nodes(
        ...
        except Exception:  # BLE001-KEEP: per-node-type degradation in the viz payload builder: storage._q raises RuntimeError over HTTP and arbitrary SDK types embedded with no common base; an empty memory set still renders the wiki and entity layers
            memories = []
        mem_ids: set[int] = set()
    ...
    def _assemble_wiki_nodes(
        ...
        except Exception:  # BLE001-KEEP: per-node-type degradation for wiki_page rows; same untypeable storage._q surface
            wiki_pages = []
    ...
    def _assemble_entity_nodes(self, nodes: list[dict], max_entities: int = 2000) -> None:
        ...
        except Exception:  # BLE001-KEEP: per-node-type degradation for entity rows; same untypeable storage surface
            all_entities = []
        if max_entities > 0:
    ...
    def _expand_memory(self, raw_id: int, nodes: list, seen: set) -> None:
        ...
        except Exception:  # BLE001-KEEP: per-node expansion: a single get_memory over the untypeable storage surface; an unreadable id is skipped so the rest of the expansion still returns
            return
        if m is None:
            return
        nid = f"mem:{raw_id}"
