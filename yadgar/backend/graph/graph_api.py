...
class GraphAPI(GraphAPINodesMixin, GraphAPIEdgesMixin):
    ...
    def _count_nodes_hidden(
        ...
        for cap, node_type, table in specs:
            ...
            except Exception:  # noqa: BLE001 — best-effort hidden-node count: the COUNT goes through storage._q (RuntimeError over HTTP, arbitrary SDK types embedded, no common base) and a missing count must leave the payload intact, per the comment on the continue below
    ...
    def _count_edges_hidden(self, edges: list[dict], transitions_cap: int) -> int:
        ...
        try:
            ...
            total = int(rows[0]["c"]) if rows else 0
        except Exception:  # noqa: BLE001 — best-effort hidden-edge count; same untypeable storage._q surface as the node-count helper above
    ...
    def get_graph_stats(self) -> dict:
        ...
        except Exception as exc:  # noqa: BLE001 — graph_stats is a whole-payload best-effort read over several storage._q counts with no common base; an empty stats dict degrades the viz header rather than failing the request
