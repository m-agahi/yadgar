...
def _fetch_similar(candidate: dict, k: int) -> list[dict]:
    ...
    try:
        ...
        return results
    except Exception as exc:  # noqa: BLE001 — fail-soft candidate fetch: the query goes through storage, which raises RuntimeError over HTTP and arbitrary SDK types embedded with no common base, and no candidates means the resolver degrades to ADD
...
def resolve_conflict(candidate: dict) -> dict[str, Any]:
    ...
    try:
        ...
        return _result
    except Exception as exc:  # noqa: BLE001 — trailing arm after the typed httpx.TimeoutException handler above: the Ollama call spans httpx, JSON decode and response-shape checks, and every non-timeout failure must degrade to ADD rather than lose the write
        from yadgar._shared.observability.exception_telemetry import (
            record_exception,  # noqa: PLC0415
        )
