...
``yadgar._shared.cognitive_map`` — the numpy matrix/prediction compute is
BACKEND territory (semantic law: compute over DB rows runs next to the DB on
7 CPUs, behind ``POST /restore``). The session-side transition recording stays
layer-shared in the ``SRTransitionRecorder`` base
(``yadgar._shared.runtime.sr_session``) — the core recall path records
transitions; only this subclass computes.
"""
...
class CognitiveMap(SRTransitionRecorder):
    ...
    # -- Recording: record_transition / invalidate / has_sufficient_data are
    # inherited from SRTransitionRecorder (single-source transition writes).
