...
class _CommunityMixin:
    ...
    def detect_communities(self) -> list[dict]:
        ...
        except Exception:  # BLE001-KEEP: algorithm fallback: louvain_communities is absent on older networkx (ImportError) and raises its own errors on degenerate graphs, so the catch breadth IS the fallback condition for label_propagation_communities below
