"""Count-capped LRU cache + shared msgpack snapshot format for the backend cache.

Two things live here, extracted from ``cache.py`` (task #18 C2 internal split):

  * ``LRUCache`` (backend v5.4.0) — the original count-capped LRU. Kept intact
    for its own tests + snapshot format; NOT the unified surface (that is
    ``Cache`` in ``cache.py``).
  * ``_write_snapshot`` / ``_read_snapshot`` — the shared free-function snapshot
    encoder/decoder that the unified ``Cache`` delegates to for ce/embed parity.
            ...
            with self._lock:
                ...
                for k, v in items:
                    ...
                    if self._max > 0 and len(self._store) > self._max:
                        ...
                        self.evictions += 1
        except Exception as exc:  # noqa: BLE001 — deserialising an on-disk snapshot possibly written by an older build: msgpack raises its own package-private unpack errors alongside OSError and ValueError, with no common base, and the documented recovery is to discard the cache
