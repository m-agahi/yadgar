...
async def _fetch_body_via_wiki(body_reader: Callable[[str], dict[str, Any]], slug: str) -> str:
    ...
    try:
        ...
        if hasattr(page, "__await__"):
            page = await page
    except Exception as exc:  # noqa: BLE001 — body_reader is an INJECTED callable (the Car G test seam): any caller-supplied reader may raise anything, sync or awaited, and the documented contract here is 'empty string on failure'
