...
def merge_memory(
    ...
    if merged_embedding is not None:
        ...
        except Exception:  # BLE001-KEEP: the vector half of a two-part merge write: update_memory_fields above already committed the content, and update_vector reaches storage with no common base, so a failed sidecar write must not undo the merge
            pass
