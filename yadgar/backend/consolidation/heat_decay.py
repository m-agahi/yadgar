...
class _HeatDecayMixin:
    ...
    def _build_domain_multiplier_map(self) -> dict[int, float]:
        ...
        try:
            ...
            return domain_mult
        except Exception:  # noqa: BLE001 — the astrocyte-pool read that supplies per-domain decay multipliers: storage raises with no common base, and an unavailable pool must leave every multiplier at 1.0 rather than stop the decay pass
