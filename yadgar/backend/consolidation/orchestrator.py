...
class _OrchestratorMixin:
    ...
    # NOTE (R3 Car 1 D1): _maybe_precompute_graph_layout moved to
    # yadgar.core.consolidation.orchestrator — it uses core.graph_api /
    # core.graph_layout (viz, core-owned). Core runs it around the forwarded
    # compute; the backend cycle no longer touches graph layout.
    ...
    def _run_post_cycle_tasks(self, stats: dict, start: float) -> None:
        """Compute-side post-consolidation tasks: consolidation log + MTREE probe.

        R3 Car 1 D1: the invariant-check + auto-vacuum tail moved to
        yadgar.core.consolidation.orchestrator._run_core_post_cycle_tasks — they
        touch core.server._run_check_invariants / core.ops._fire_vacuum_service
        (core-owned). Core runs them AFTER the forwarded compute returns. Only the
        storage-local tail (insert_consolidation_log + mtree_probe) stays here.
        """
        _t = time.monotonic()
