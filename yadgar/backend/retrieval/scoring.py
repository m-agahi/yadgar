...
def _set_stage_attrs(**attrs: int) -> None:
    ...
    except Exception:  # BLE001-KEEP: sets dynamic attributes on the active OTel span; a degraded or swapped provider raises arbitrary types here (the I3 case documented in tracing.py), and span decoration must never fail the recall
        pass
...
class _ScoringMixin:
    ...
    def _run_fts_bm25(self, params: FTSParams, scores: dict) -> None:
        ...
        except Exception:  # BLE001-KEEP: per-signal isolation in hybrid scoring: the FTS read reaches storage, which reports a missing full-text index as RuntimeError over HTTP but as an arbitrary SDK type embedded; a dead signal contributes no scores and the other signals still rank
            pass
    ...
    def _run_entity_fts(self, params: FTSParams, scores: dict) -> None:
        ...
        try:
            ...
            if hits:
                ...
                _normalize_fts_hits(hits, scores, strength)
        except Exception:  # BLE001-KEEP: per-signal isolation for the entity FTS pass; same untypeable storage surface as the content pass above
            pass
    ...
    def _run_comet_fts(self, params: FTSParams, scores: dict) -> None:
        ...
        except Exception:  # BLE001-KEEP: per-signal isolation for the COMET-term FTS pass; same untypeable storage surface
            pass
        ...
        try:
            ...
            if temporal_info["month_hints"]:
                ...
                if month_matches:
                    ...
                    if w_temporal == 0.0:
                        w_temporal = 0.6
        except Exception:  # BLE001-KEEP: per-signal isolation for the temporal signal: it reaches storage and the date parser, which share no common base, and the weight is left at its pre-signal value so ranking continues without it
