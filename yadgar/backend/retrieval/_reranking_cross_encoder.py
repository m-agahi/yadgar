...
@observe(tier="hot", metric="retrieval.ce.build_expanded_pairs")
...
@observe(tier="hot", metric="retrieval.ce.aggregate_max_scores")
...
@observe(tier="hot", metric="retrieval.ce.apply_weights")
...
class _CrossEncoderMixin:
    ...
    @observe(tier="stage", metric="retrieval.cross_encoder_rerank")
    def cross_encoder_rerank(
        ...
        except Exception as e:  # noqa: BLE001 — ML client boundary: score_ce_cached reaches a local torch CrossEncoder or the remote embed service over httpx, which share no common base, and an unavailable reranker must return the pre-rerank order
    ...
    @observe(tier="stage", metric="retrieval.score_pair")
    def score_single_pair(self, query: str, document: str) -> float:
        ...
        except Exception:  # noqa: BLE001 — same ML client boundary as cross_encoder_rerank; the documented degrade is 0.0, matching the circuit-breaker-open None case handled on the line above
            return 0.0
    ...
    def score_documents(self, query: str, documents: list[str]) -> list[float]:
        ...
        except Exception:  # noqa: BLE001 — same ML client boundary; the documented degrade is a 0.0 per document, matching score_single_pair's semantics
