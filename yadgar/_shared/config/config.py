...
class YamlConfigSource(PydanticBaseSettingsSource):
    ...
    def _load(self) -> None:
        ...
        except Exception:  # BLE001-KEEP: the optional YAML config layer: ruamel is a LAZY import inside this try, so its YAMLError has no importable name here, and it sits next to open()'s OSError with no common base. A broken or missing yaml means 'no yaml layer', never a failed startup
            import logging
