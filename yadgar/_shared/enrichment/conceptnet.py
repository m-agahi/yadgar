...
class ConceptNetExpander:
    ...
    def _try_lite(self, term: str, relations: list[str], min_weight: float) -> list[str]:
        ...
        try:
            ...
            return results
        except Exception:  # BLE001-KEEP: conceptnet_lite is an optional dep whose SQLite-backed query raises its own package types plus sqlite3 errors, with no common base; a failure permanently disables the lite path via the flag below
    ...
    def _try_http(self, term: str, relations: list[str], min_weight: float) -> list[str]:
        ...
        try:
            ...
            return results
        except Exception:  # BLE001-KEEP: outer arm after the typed httpx RequestError/HTTPStatusError/TimeoutError/OSError handler above; it disables the HTTP path so enrichment falls through to the hardcoded table
