...
def _record_recall_sr_transition(merged: list[dict]) -> None:
    ...
    if prev_id is not None and prev_id != top_id:
        ...
        except Exception:  # noqa: BLE001 — SR transition bookkeeping on the recall path: the cognitive map reaches storage, whose failures share no common base, and instrumentation must never fail the recall it observes
