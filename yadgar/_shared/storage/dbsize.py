...
def _fetch_remote_db_size(embed_url: str, threshold: int, timeout_sec: float) -> dict | None:
    ...
    try:
        ...
        return _zero_size_dict()
    except Exception as exc:  # noqa: BLE001 — trailing arm after the typed httpx HTTPStatusError/TimeoutException handlers above; a size telemetry read must degrade to zeros rather than fail its caller
