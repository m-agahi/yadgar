...
def _derive_enrichment_resync(
    ...
    except Exception as e:  # noqa: BLE001 — the try body drives the optional enrichment pipeline and the embedding encoder (both third-party ML code with no common base); the handler's whole purpose is the null-out fallback that keeps stale enrichment off a rewritten memory
        import logging
...
class _MemoryMixin:
    ...
    def _enrich_memory_if_enabled(  # noqa: C901 — pipeline flag+length+embedding guards + 6-field mapping; extract further degrades locality without reducing count
        ...
        except Exception as e:  # noqa: BLE001 — spans the enrichment pipeline, the embedding encoder and a storage UPDATE, which share no common base; enrichment is best-effort and must not fail the write it decorates
            import logging
    ...
    def delete_memory(self, memory_id: int):
        ...
        except Exception:  # noqa: BLE001 — mid-delete cleanup: the vector delete reaches storage (no common base) and the row DELETE below must still run, or the memory survives its own deletion
        ...
        except Exception:  # noqa: BLE001 — mid-delete cleanup: same storage surface, and the row DELETE below must still run
    ...
    def find_memory_ids_by_entity_name(self, entity_name: str) -> list[int]:
        ...
        except Exception:  # noqa: BLE001 — this IS the FTS-unavailable fallback: SurrealDB reports a missing full-text index as a RuntimeError over HTTP but as an arbitrary SDK type in embedded mode, so there is no type to name; the handler retries with string::contains
            rows = self._q(
    ...
    def find_memory_ids_by_entities(self, entity_names: list[str]) -> dict[str, list[int]]:
        ...
        except Exception:  # noqa: BLE001 — degrades a batched _q_multi to the per-name path, which repeats the FTS fallback above; same untypeable transport split
