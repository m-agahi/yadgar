...
class _VectorMixin:
    ...
    def update_memory_embedding(self, memory_id: int, embedding: bytes, embedding_model: str):
        ...
        except Exception:  # noqa: BLE001 — upsert emulation: update-then-insert against storage, whose failures share no common base; a miss here falls through to the insert below
            try:
                self.insert_vector(memory_id, embedding)
            except Exception:  # noqa: BLE001 — second half of the upsert emulation; the row was already written by the UPDATE above, so the sidecar write is best-effort
                pass
    ...
    def recreate_vector_table(self, new_dim: int):
        ...
        except Exception as exc:  # noqa: BLE001 — best-effort pre-flight backup before non-transactional DDL: it runs DEFINE/DELETE/INSERT whose failures share no common base, and a missing backup must not block the index rebuild it protects
    ...
    def probe_vector_indexes(self) -> bool:
        ...
        try:
            ...
            return True
        except Exception:  # noqa: BLE001 — corruption probe: its contract is that ANY failure of the KNN read means the MTREE index is unusable, so the catch breadth IS the answer being computed
            return False
    ...
    def rebuild_vector_indexes(self) -> bool:
        ...
        try:
            ...
            return True
        except Exception:  # noqa: BLE001 — same corruption contract as probe_vector_indexes: any REBUILD failure means the container must restart, so the breadth IS the answer
