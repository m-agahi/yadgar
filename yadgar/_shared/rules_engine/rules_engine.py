...
class RulesEngine:
    ...
    def _report_unmigrated_rules(self) -> None:
        ...
        except Exception:  # noqa: BLE001 — one-shot legacy-scope census: it queries storage, which raises RuntimeError over HTTP and arbitrary SDK types embedded with no common base, and a census is reporting — it must never break rule application
    ...
    def check_write_policy(
        ...
        for rule in write_rules:
            ...
            if action_type == "redact" and pattern:
                ...
                except Exception as exc:  # noqa: BLE001 — the timeout this catches is regex.TimeoutError, a type from the OPTIONAL `regex` package; naming it would require importing the very dependency whose absence the stdlib-re fallback above exists to tolerate
