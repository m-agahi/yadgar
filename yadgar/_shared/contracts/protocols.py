...
class StorageProtocol(Protocol):
    ...
    # Car C13f (0047 §5): ``project_id`` is the SCOPE arm, pushed into the query.
    # It is on the PROTOCOL, not just the implementation, so a substitute storage
    # that omits it fails type-check instead of silently searching corpus-wide —
    # which is exactly how this defect survived: the parameter existed one layer
    # up and nothing required the storage call to accept it.
    def search_beliefs_fts(
        self, query: str, limit: int = ..., *, project_id: str | None = ...
    ) -> list[dict]: ...
