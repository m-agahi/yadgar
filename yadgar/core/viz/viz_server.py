...
# The bundled UI lives at yadgar/core/static/. This module lives one level
# deeper (yadgar/core/viz/) after the t2-car-d3 package move, so resolve UP two
# levels from __file__ (viz/ → core/) then into static/. Using .parent alone
# would resolve to the nonexistent yadgar/core/viz/static/ and 404 the entire
# UI (see test_viz_proxy.TestVizStaticDirResolves). This matches the daemon's
# /graph route, which serves Path(http.py).parent.parent / "static".
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _handle_proxy(self) -> None:
        ...
        from yadgar._shared.config import (
            get_settings,  # read at request time — supports late env setup
        )
