...
def test_default_on_fires_detector(curator, storage, embeddings, monkeypatch):
    ...
    from yadgar.backend.curation.ingestion import find_similar_memories
    ...
    from yadgar.backend.curation.contradiction import detect_contradictions
    ...
    import yadgar.backend.curation as _curation_mod
    import yadgar.backend.curation.contradiction as _cmod
...
def test_detector_exception_does_not_block_write(curator, storage, monkeypatch):
    ...
    with patch(
        "yadgar.backend.curation.contradiction.detect_contradictions",
...
def test_llm_resolver_short_circuit_bypasses_lightweight(monkeypatch):
    """YADGAR_CONFLICT_RESOLVER=on + LLM returns NOOP → drainer short-circuits before
    phase_store, curator never reached, lightweight detector never runs.

    Post-R3 architecture: phase_contradiction runs in the drainer pipeline
    (run_memorize_replay), not at the MCP memorize() boundary.  Test drives
    run_memorize_replay directly so the assertion is at the correct seam.
    """
    ...
    with (
        ...
        patch.object(_state_mod, "_retriever", None),
        ...
        result = run_memorize_replay(
            content="anything",
            context="/test/wtc",
            tags=[],
        )

    assert result.get("stored") is False, "LLM NOOP must short-circuit before phase_store"
    assert result.get("reason") == "conflict_resolver_noop", f"unexpected reason: {result}"
