...
Design goals (v5.10.0):
- Single registry for all spawned SurrealDB pids across the test session.
- atexit handler fires on clean exit, ^C (SIGINT), and timeout-induced teardown.
- Per-fixture teardown helper (terminate → wait → kill) keeps existing logic
  co-located with the registry so belt-and-suspenders works.
...
"""Module-level registry of all SurrealDB subprocess PIDs spawned during tests.
...
production fixtures append via spawn_surreal().
"""
