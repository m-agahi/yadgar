...
- wait=True: gate runs in drainer, rejection surfaces synchronously via
  get_job_result() in _wiki_add_wait_path. Same observable contract for callers
  as v5.39 wait=True (DP-B: sync rejection preserved for wait=True).
- force=True, replace_slug, append=True: bypass gate in drainer.

Tests use a real in-process drainer (drain_now() triggered by wait_for_job).
"""
