...
def test_subdir_memorize_busts_parent_project_brief_epoch(monkeypatch):
    ...
    monkeypatch.setattr(project, "_resolve_project_root", fake_resolve)

    # project_brief reads the epoch on the RESOLVED root.
    parent_resolved = project._resolve_project_root("/repo")
    ...
    project._bump_epoch_for_context(subdir_ctx)
...
def test_bump_helper_normalizes_before_bump(monkeypatch):
    ...
    project._bump_epoch_for_context("/x/root/a/b")
    project._bump_epoch_for_context("/x/root")  # raw root, same normalization
...
def test_epoch_bump_busts_catalog_cache(monkeypatch):
    ...
    project._bump_epoch_for_context("/repo")  # structural write
