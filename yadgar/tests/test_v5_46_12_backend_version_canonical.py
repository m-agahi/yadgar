...
class TestBackendVersionConstant:
    ...
    def test_backend_version_value(self) -> None:
        """yadgar.BACKEND_VERSION must be '5.4.0' (current backend track)."""
        import yadgar

        assert yadgar.BACKEND_VERSION == "5.4.0", (
            f"yadgar.BACKEND_VERSION = {yadgar.BACKEND_VERSION!r}, expected '5.4.0'.\n"
            "Fix: set BACKEND_VERSION = '5.4.0' in yadgar/__init__.py."
        )
