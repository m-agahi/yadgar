...
class TestReembedAllSkipsNoneContent:
    ...
    def test_reembed_all_skips_none_content_rows(self):
        ...
        # R3 Car 3b: the reembed LOGIC moved to the backend /admin impl; the core
        # MCP tool is now a thin forwarder. Test the impl where the logic lives.
        from yadgar.backend.admin_exec.memory import reembed_all
        ...
        with patch("yadgar.backend.admin_exec.memory._get_storage", return_value=mock_storage):
            with patch(
                "yadgar.backend.admin_exec.memory._get_embeddings", return_value=mock_embeddings
            ):
                result = reembed_all({})
    ...
    def test_reembed_all_all_none_content_returns_zero(self):
        ...
        # R3 Car 3b: reembed logic lives in the backend /admin impl now.
        from yadgar.backend.admin_exec.memory import reembed_all
