...
def _build_sync_env(monkeypatch, *, memory_id=_FIXED_MEMORY_ID, get_memory_return=None):
    ...
    # R3: the sync pipeline entry moved to the backend run_memorize_replay,
    # which reads get_settings() per call — patch it there too so replay-path
    # tests see the same stubbed settings as core-path tests.
    import yadgar.backend.write_exec.memorize_impl as _impl_mod

    monkeypatch.setattr(_impl_mod, "get_settings", lambda: mock_settings)
...
def test_memorize_accept_path_returns_expected_dict(monkeypatch, tmp_path):
    """Sync accept path returns dict with stable shape.
...
def test_memorize_reject_write_gate_returns_expected_dict(monkeypatch, tmp_path):
    ...
    # R3 migration: the write gate fires in the backend replay pipeline
    # (phase_embed), not in the enqueue-only core shell.
    from yadgar.backend.write_exec.memorize_impl import run_memorize_replay
...
def test_memorize_reject_secret_leak_returns_expected_dict(monkeypatch, tmp_path):
    ...
    with patch(
        "yadgar._shared.write_exec.validate.gate_or_reject",
