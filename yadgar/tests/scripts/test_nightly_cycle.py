...
Lifecycle under test (steps 1-7):
  ...
  3. Consolidation (StorageEngine + run_nightly_consolidation — compute forwarded to backend, R3)
...
def _run_with_mocks(
    ...
    *,
    ...
    base = dict(
        ...
        # R3 Car 1 D3: nightly_cycle no longer constructs ConsolidationScheduler /
        # EmbeddingEngine — it calls run_nightly_consolidation (which forwards the
        # compute to the backend). Patch that function instead.
...
class TestHappyPath:
    ...
    def test_step_order_matches_lifecycle(self, tmp_path: Path) -> None:
        ...
        # R3 Car 1 D3: the nightly no longer constructs a ConsolidationScheduler —
        # it calls run_nightly_consolidation (compute forwarded to the backend).
        def _force_cons(*a, **kw):
        ...
        with patch.multiple(
            ...
            run_nightly_consolidation=_force_cons,
            Settings=MagicMock(return_value=SimpleNamespace(DB_PATH=str(db_dir))),
            configure_logging=MagicMock(),
            default_retention=MagicMock(return_value=3),
        ):
        ...
        # R3 Car 1 D3: scheduler construction moved backend-side; the core-side
        # ordering guarantee is storage opens before the compute call.
        assert call_order.index("storage_engine_init") < call_order.index(
            "run_nightly_consolidation"
        ), "StorageEngine must open before run_nightly_consolidation"
        # Step 4: vacuum
