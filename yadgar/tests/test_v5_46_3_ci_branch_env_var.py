...
Verifies .forgejo/workflows/{ci.yaml,release.yaml} set YADGAR_CI_BRANCH: master
...
class TestCIBranchEnvVar:
    ...
    def test_ci_yaml_exists(self):
        assert CI_YAML.exists(), f"ci.yaml not found at {CI_YAML}"
    ...
    def test_ci_yaml_has_yadgar_ci_branch(self):
        ...
        assert "YADGAR_CI_BRANCH" in content, "ci.yaml must set YADGAR_CI_BRANCH env var"
    ...
    def test_release_yaml_has_yadgar_ci_branch(self):
        ...
        assert "YADGAR_CI_BRANCH" in content, "release.yaml must set YADGAR_CI_BRANCH env var"
