...
# ── 7. Car B0 (#83): hash / source_file survive the FULL drainer round-trip ──
#
# The v5.42.2 trap: a unit test on run_wiki_add_replay(payload) directly goes
# green while prod stays broken if any drainer stage whitelists payload keys.
# These tests go through server.wiki_add(wait=True) → FileQueue → QueueDrainer
# → _fill_wiki_add_defaults → run_wiki_add_replay → WikiStore.add → storage,
# i.e. the EXACT production write path.


class TestHashSourceFileDrainerRoundTrip:
    def test_wiki_add_hash_source_file_persist_wait_true(self):
        """wiki_add(hash=, source_file=, wait=True) → stored page carries them."""
        _apply_migration()
        title = _make_unique_title("B0 Hash Page")
        result = server.wiki_add(
            title=title,
            content="module body",
            wait=True,
        ...
        assert result.get("committed") is True, f"not committed: {result}"
        slug = result.get("slug")
        page = _storage().get_wiki_page_by_slug(slug)
        assert page is not None
        assert page.get("hash") == "abc", f"hash dropped through drainer: {page.get('hash')}"
        assert page.get("source_file") == "/x.py"

    def test_wiki_add_replace_slug_updates_hash_wait_true(self):
        """replace_slug of an existing page with a new hash re-persists the hash."""
        _apply_migration()
        title = _make_unique_title("B0 Replace Hash Page")
        slug = title.lower().replace(" ", "-")[:64]
        ...
        pre = _storage().get_wiki_page_by_slug(slug)
        assert pre is not None and pre.get("hash") == "old"
        ...
        post = _storage().get_wiki_page_by_slug(slug)
        assert post is not None
        assert post.get("hash") == "new", f"replace_slug did not update hash: {post.get('hash')}"
