...
Covers:
  ...
  - Wiki draft CRUD (storage methods)
...
# R3 Car 3c: wiki_approve, wiki_discard, wiki_autolink forward to backend /admin.
...
# ── D. Wiki draft storage CRUD ────────────────────────────────────────────────


class TestWikiDraftStorage:
    def _make_draft(self, slug: str = "test-draft") -> dict:
        return {
            "slug": slug,
            "title": "Test Draft",
    ...
    def test_insert_and_retrieve(self):
        ...
        did = _storage().insert_wiki_draft(draft)
        assert isinstance(did, int)
        retrieved = _storage().get_wiki_draft_by_slug("test-draft")
        assert retrieved is not None
        assert retrieved["slug"] == "test-draft"
        assert retrieved["title"] == "Test Draft"

    def test_list_drafts(self):
        _storage().insert_wiki_draft(self._make_draft("draft-a"))
        _storage().insert_wiki_draft(self._make_draft("draft-b"))
        drafts = _storage().list_wiki_drafts()
        slugs = [d["slug"] for d in drafts]
        assert "draft-a" in slugs
        assert "draft-b" in slugs

    def test_delete_draft(self):
        _storage().insert_wiki_draft(self._make_draft("delete-me"))
        deleted = _storage().delete_wiki_draft("delete-me")
        assert deleted is True
        assert _storage().get_wiki_draft_by_slug("delete-me") is None

    def test_delete_nonexistent_returns_false(self):
        assert _storage().delete_wiki_draft("no-such-slug") is False

    def test_get_nonexistent_returns_none(self):
        assert _storage().get_wiki_draft_by_slug("no-such-draft") is None

    def test_list_empty(self):
        assert _storage().list_wiki_drafts() == []


# ── E. Wiki draft MCP tools ───────────────────────────────────────────────────


class TestWikiDraftTools:
    def _insert_draft(self, slug: str = "my-draft", title: str = "My Draft") -> None:
        _storage().insert_wiki_draft(
            {
                "slug": slug,
                "title": title,
    ...
    def test_wiki_drafts_empty(self):
        result = server.wiki_drafts()
        assert result == []

    def test_wiki_drafts_lists_pending(self):
        self._insert_draft("pending-draft")
        drafts = server.wiki_drafts()
        assert any(d["slug"] == "pending-draft" for d in drafts)

    def test_wiki_drafts_content_truncated(self):
        _storage().insert_wiki_draft(
            {
                "slug": "long-draft",
                "title": "Long",
        ...
        drafts = server.wiki_drafts()
        matching = [d for d in drafts if d["slug"] == "long-draft"]
        assert len(matching) == 1
        assert len(matching[0]["content"]) <= 200

    def test_wiki_approve_promotes_draft(self):
        # slug must match what wiki.add() will derive from the title
        self._insert_draft(slug="my-draft", title="My Draft")
        result = server.wiki_approve("my-draft")
        assert result["approved"] is True
        assert result["slug"] == "my-draft"
        # Draft deleted after approval
        assert _storage().get_wiki_draft_by_slug("my-draft") is None
        # Page now in wiki (title "My Draft" → slug "my-draft")
        page = _wiki().read("my-draft")
        assert page is not None
        assert page["title"] == "My Draft"

    def test_wiki_approve_nonexistent_returns_error(self):
        result = server.wiki_approve("no-such-draft")
        assert result["approved"] is False
        assert "error" in result

    def test_wiki_discard_removes_draft(self):
        self._insert_draft(slug="discard-me", title="Discard Me")
        result = server.wiki_discard("discard-me")
        assert result["discarded"] is True
        assert _storage().get_wiki_draft_by_slug("discard-me") is None

    def test_wiki_discard_nonexistent_returns_error(self):
        result = server.wiki_discard("no-such-draft")
        assert result["discarded"] is False
        assert "error" in result

    def test_approve_removes_from_drafts_list(self):
        self._insert_draft("promote-me")
        server.wiki_approve("promote-me")
        drafts = server.wiki_drafts()
        assert not any(d["slug"] == "promote-me" for d in drafts)

    def test_discard_removes_from_drafts_list(self):
        self._insert_draft("trash-me")
        server.wiki_discard("trash-me")
        drafts = server.wiki_drafts()
        assert not any(d["slug"] == "trash-me" for d in drafts)
