...
def _add(*args, **kwargs):
    """``WikiStore.add`` with this file's project NAMED.

    ADR-0227 / C5: ``add()`` derives nothing. A page insert that names no
    project raises ``UnresolvedProjectError`` at the storage chokepoint, so
    every write below has to say who owns the page. The naming is centralised
    here rather than repeated 60+ times, but it is NOT hidden: a test that
...
class TestCRUD:
    ...
    def test_add_and_read(self):
        result = _add("Test Page", "Some content.", "reference", ["test"])
    ...
    def test_delete(self):
        _add("Delete Me", "Content.", "reference")
    ...
    def test_list_pages(self):
        _add("Page A", "Content A", "architecture")
        _add("Page B", "Content B", "decision")
    ...
    def test_list_pages_filtered_by_category(self):
        _add("Arch Page", "Architecture stuff", "architecture")
        _add("Debug Page", "Debugging stuff", "debugging")
...
class TestUpsert:
    ...
    def test_upsert_updates_content(self):
        _add("Upsert Test", "Original content.", "reference")
        _add("Upsert Test", "Updated content.", "reference")
        page = _wiki().read("upsert-test")
        assert page["content"] == "Updated content."

    def test_upsert_merges_tags(self):
        _add("Tag Test", "Content.", "reference", ["tag-a"])
        _add("Tag Test", "New content.", "reference", ["tag-b"])
...
class TestIngest:
    ...
    def test_ingest_creates_new_page(self):
        result = _ingest("New content.", title="Ingest New")
    ...
    def test_ingest_appends_to_existing(self):
        _add("Ingest Existing", "Original.", "reference")
        _ingest("Appended content.", title="Ingest Existing")
    ...
    def test_ingest_merges_tags(self):
        _add("Ingest Tags", "Content.", "reference", ["tag-1"])
        _ingest("More.", title="Ingest Tags", tags=["tag-2"])
...
class TestCrossReferences:
    ...
    def test_crossrefs_created_on_add(self):
        _add("Page With Links", "See [[target-page]] for details.", "reference")
    ...
    def test_crossrefs_updated_on_upsert(self):
        _add("Link Page", "See [[old-target]].", "reference")
        _add("Link Page", "Now see [[new-target]].", "reference")
    ...
    def test_crossrefs_cleaned_on_delete(self):
        _add("Deleting Linker", "See [[some-target]].", "reference")
    ...
    def test_incoming_crossrefs_cleaned_when_target_deleted(self):
        ...
        """
        _add("Source Page", "See [[target-to-delete]].", "reference")
        ...
        _add("Target To Delete", "I will be deleted.", "reference")
...
class TestLint:
    ...
    def test_lint_detects_orphan(self):
        _add("Orphan Page", "No one links here.", "reference")
        report = _wiki().lint()
    ...
    def test_lint_detects_broken_ref(self):
        _add("Broken Ref Page", "See [[nonexistent-page]].", "reference")
        report = _wiki().lint()
    ...
    def test_lint_no_issues_on_linked_pages(self):
        _add("Hub Page", "See [[spoke-page]].", "reference")
        _add("Spoke Page", "See [[hub-page]].", "reference")
        report = _wiki().lint()
        orphans = [i for i in report["issues"] if i["type"] == "orphan"]
...
class TestValidation:
    ...
    def test_invalid_category_defaults_to_reference(self):
        result = _add("Bad Cat", "Content.", "invalid_category")
...
class TestVersioningRegression:
    ...
    def test_wiki_add_produces_version_1(self):
        ...
        result = _add("Versioning Test", "initial content", "reference", ["v5.41"])
        pid = result["id"]
        rows = storage._q(
    ...
    def test_wiki_add_upsert_produces_version_2(self):
        ...
        _add("Upsert Version", "v1 content", "reference")
        result2 = _add("Upsert Version", "v2 content", "reference")
        pid = result2["id"]
...
class TestAutolink:
    ...
    def test_autolink_dry_run_no_mutation(self):
        ...
        _add("Recall Pipeline", "scoring details", "reference", opts=self._opts())
        _add(
            "Memorize Path",
    ...
    def test_autolink_skips_short_titles(self):
        ...
        _add("API", "interface stuff", "reference", opts=self._opts())
        _add(
            "Memorize Path",
    ...
    def test_autolink_word_boundary(self):
        ...
        _add("Memory", "memory subsystem", "reference", opts=self._opts())
        _add(
            "Other Page",
    ...
    def test_autolink_preserves_metadata_on_apply(self):
        ...
        """
        _add("Recall Pipeline", "scoring details", "decision", opts=self._opts())
        _add(
            "Memorize Path",
            "The Recall Pipeline ranks memories.",
            "decision",
