...
def _force_consolidate():
    """Run one consolidation cycle against the live engines.

    R3 migration: ``server._consolidation`` is None core-side (consolidation
    moved to the backend). Build the backend ConsolidationScheduler from the
    engines init_engines() populated in ``_st`` — same wiring as the backend
    service — and force one cycle.
    """
