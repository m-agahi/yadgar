...
def test_pool_identity_and_single_init(tmp_path, monkeypatch, engine_set):
    ...
    try:
        ...
        # R3 Car 1: core init no longer builds _consolidation on ANY path —
        # the ConsolidationScheduler is a backend singleton built lazily
        # (backend/consolidation/service._get_scheduler) with pool=_st._pool
        # injected. Assert the injection contract directly: constructing the
        # scheduler the way the backend does must adopt _st._pool verbatim
        # and must NOT re-run init_processes.
        assert _st._consolidation is None, "core init must NOT build _consolidation (R3 Car 1)"
        ...
        assert scheduler.pool is _st._pool, "scheduler.pool must BE the same object as _st._pool"
        assert calls["n"] == 1, (
            f"scheduler construction re-ran init_processes ({calls['n']}x total, expected 1)"
        )
    finally:
        server.shutdown()
