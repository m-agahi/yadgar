...
def test_compute_valid_until_emits_span(span_exporter):
    """server_helpers._compute_valid_until opens a hot span (pure ttl compute)."""
    ...
    # yadgar._shared.server_helpers, so the qualname tail follows the new module.
    names = _span_names(span_exporter)
    assert any(n.endswith(".server_helpers._compute_valid_until") for n in names), names
