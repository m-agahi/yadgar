...
def _engines(tmp_path_factory):
    ...
    tmp_path = tmp_path_factory.mktemp("wiki_similarity_gate")
    server.init_engines(
...
class TestWikiCheckDuplicate:
    ...
    def test_check_duplicate_returns_empty_when_no_pages(self, monkeypatch):
        ...
        monkeypatch.setattr("yadgar.backend.queue_drainer._drain_local.active", True, raising=False)
    ...
    def test_check_duplicate_finds_near_clone(self, monkeypatch, _unit_backend_harness):
        """Reproduces the 2026-05-30 incident: near-clone detected."""
        drainer = _unit_backend_harness

        # Insert page A via wiki_add (R3: enqueue + drain to persist)
        add_result = server.wiki_add(
            title="Yadgar Roadmap Future Improvements",
            content=_ROADMAP_CONTENT_A,
