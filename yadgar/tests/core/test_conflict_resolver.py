...
def test_enabled_ollama_add_returns_add():
    ...
    with patch("yadgar.backend.conflict_resolver._get_client", return_value=mock_client):
...
def test_enabled_ollama_update_returns_update():
    ...
    with patch("yadgar.backend.conflict_resolver._get_client", return_value=mock_client):
        with patch("yadgar.backend.conflict_resolver._fetch_similar", return_value=[{"id": 42}]):
            result = cr.resolve_conflict(_candidate())
...
def test_enabled_ollama_delete_returns_delete():
    ...
    with patch("yadgar.backend.conflict_resolver._get_client", return_value=mock_client):
        with patch("yadgar.backend.conflict_resolver._fetch_similar", return_value=[{"id": 7}]):
            result = cr.resolve_conflict(_candidate())
...
def test_enabled_ollama_noop_returns_noop():
    ...
    with patch("yadgar.backend.conflict_resolver._get_client", return_value=mock_client):
        with patch("yadgar.backend.conflict_resolver._fetch_similar", return_value=[{"id": 99}]):
            result = cr.resolve_conflict(_candidate())
