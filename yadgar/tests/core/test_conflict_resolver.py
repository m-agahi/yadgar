...
def test_enabled_ollama_add_returns_add():
    ...
    with patch(
        "yadgar.backend.conflict_resolver.conflict_resolver._get_client", return_value=mock_client
    ):
        with patch(
...
def test_enabled_ollama_update_returns_update():
    ...
    with patch(
        ...
        with patch(
            "yadgar.backend.conflict_resolver.conflict_resolver._fetch_similar",
            return_value=[{"id": 42}],
        ):
            result = cr.resolve_conflict(_candidate())
