"""Tests for threshold auto-trigger vacuum (v4.9 item 2).

TDD: tests written first before implementation.
Run with: YADGAR_TEST=1 pytest yadgar/tests/test_vacuum_auto_trigger.py -x

R3 Car 1 D1: the auto-vacuum trigger moved out of the backend
ConsolidationScheduler cycle into the CORE orchestrator
(yadgar.core.consolidation.orchestrator._maybe_auto_vacuum, driven by
_run_core_post_cycle_tasks after the forwarded compute returns). The cooldown
timestamp is now the module-level global orchestrator._last_vacuum_at (was
scheduler._last_vacuum_at), and the _now_local() hook was inlined to
datetime.now() — tests inject time by patching the module's datetime.
"""
...
from yadgar.core.consolidation import orchestrator as consolidation_mod
from yadgar.core.consolidation.orchestrator import _in_window


def _fake_datetime(local_now: datetime):
    """datetime stand-in for the orchestrator module: naive .now() returns the
    injected local test time; tz-aware .now(tz) stays real (cooldown math)."""
...
class TestAutoTriggerWindow:
    def test_outside_window_does_not_fire(self):
        ...
        with (
            ...
            with patch.object(consolidation_mod, "datetime", _fake_datetime(outside_time)):
                consolidation_mod._run_core_post_cycle_tasks(storage, scheduler._settings)
    ...
    def test_inside_window_fires(self):
        ...
        with (
            ...
            with patch.object(consolidation_mod, "datetime", _fake_datetime(inside_time)):
                consolidation_mod._run_core_post_cycle_tasks(storage, scheduler._settings)
...
class TestAutoTriggerCooldown:
    def test_cooldown_skips_when_recent(self):
        ...
        with (
            patch.object(consolidation_mod, "_fire_vacuum_service") as mock_fire,
            # R3: cooldown timestamp is the orchestrator module global now.
            patch.object(
    ...
    def test_fire_updates_last_vacuum_at(self):
        """After firing, the module-global cooldown timestamp is set."""
        scheduler, storage = _make_scheduler()
        ...
        with (
            ...
            # Assert inside the patch scope (patch.object restores on exit).
            assert consolidation_mod._last_vacuum_at is not None
