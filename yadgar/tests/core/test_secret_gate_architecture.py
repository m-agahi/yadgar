...
class TestWikiUpdateAPIGate:
    ...
    def test_clean_fields_pass(self, isolated_server, monkeypatch, admin_backend_bypass):
        import yadgar._shared.runtime.state as _st
...
class TestI26Lint:
    ...
    SCRIPT = Path(__file__).parent.parent.parent.parent / "scripts" / "check_secret_gate.py"
...
class TestBackfillScanScript:
    SCRIPT = Path(__file__).parent.parent.parent.parent / "scripts" / "scan_db_for_secrets.py"
