"""Tests for yadgar/core/config_sync/sync.py — incremental YAML config sync.

Wave 2 coverage: yadgar/config_sync.py (100 stmts, 0% pre-wave); T2 Car A moved
the module (and this test, mirror layout) from _shared to core.
...
class TestComputeMissingViaSync:
    def test_no_missing_when_all_present(self, tmp_path):
        ...
        with (
            ...
            patch("yadgar._shared.config.config_yaml.get_config_path", return_value=config),
            patch("yadgar._shared.config.config_yaml.FIELD_META", {}),
        ):
            # No exception / no file modification expected
            from yadgar.core.config_sync.sync import _compute_missing
...
class TestComputeUnknownViaModule:
    def test_returns_empty_when_disabled(self):
        from yadgar.core.config_sync.sync import _compute_unknown
...
class TestHandleDryRun:
    def test_no_missing_prints_synced(self, capsys):
        settings = MagicMock()
        with patch("yadgar._shared.config.config_yaml.FIELD_META", {}):
    ...
    def test_missing_printed_with_default(self, capsys):
        ...
        with patch(
            "yadgar._shared.config.config_yaml.FIELD_META", {"new_field": {"desc": "some desc"}}
        ):
...
class TestApplyMissing:
    def test_adds_missing_key(self):
        ...
        with patch(
            "yadgar._shared.config.config_yaml.FIELD_META", {"new_field": {"desc": "test desc"}}
        ):
    ...
    def test_no_desc_in_field_meta_still_adds(self):
        ...
        settings = MagicMock()
        settings.MY_KEY = "val"
        with patch("yadgar._shared.config.config_yaml.FIELD_META", {}):  # no entry for my_key
