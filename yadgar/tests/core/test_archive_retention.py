...
def test_nightly_cycle_invokes_purge(storage, monkeypatch):
    ...
    from yadgar.backend.consolidation.cleanup import _CleanupMixin  # noqa: PLC0415
...
def test_archive_purge_dry_run_default(storage, admin_backend_bypass):
...
def test_archive_purge_explicit_run(storage, admin_backend_bypass):
...
def test_archive_purge_retention_override(storage, admin_backend_bypass):
...
def test_archive_purge_secret_gated(monkeypatch, admin_backend_bypass):
