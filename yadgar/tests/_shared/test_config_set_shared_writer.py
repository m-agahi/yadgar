...
def _cfg_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("yadgar._shared.config.config_yaml.get_config_path", lambda: path)
    return path
...
def test_cmd_config_set_delegates_to_shared_writer(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("yadgar._shared.config.config_yaml.set_config_value", _spy)
