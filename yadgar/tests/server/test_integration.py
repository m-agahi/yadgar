...
class TestServerStartupShutdown:
    def test_clean_startup_and_shutdown(self, tmp_path):
        ...
        _prospective, _narrative, _sleep) moved to the backend — core builds
        only the shared engines + StalenessDetector, and their core _st slots
        stay None by design. The backend drainer/consolidation service builds
        them lazily on first use.
        """
        db_path = str(tmp_path / "lifecycle.db")
        server.init_engines(db_path=db_path, embedding_model="all-MiniLM-L6-v2")

        # Verify the core engine set is initialized. No assertion on the
        # backend-only slots (_consolidation, _curator, _prospective,
        # _narrative, _sleep) — core init leaves them alone; the backend
        # drainer/consolidation service populates them lazily, so earlier
        # tests in the module may have built them already.
