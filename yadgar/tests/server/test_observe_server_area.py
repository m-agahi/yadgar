...
def test_helpers_file_hash_instrumented():
    # R2b/R3: _file_hash moved from core.server._helpers to _shared.server_helpers.
    from yadgar._shared import server_helpers as _helpers
...
def test_phase_validate_instrumented():
    # R3 Car 1: phase_validate moved backend/write_exec/_memorize_phases/_phase_validate.py
    # → yadgar/_shared/write_exec/validate.py.
    from yadgar._shared.write_exec import validate as _phase_validate

    assert _has_span(_phase_validate._validate_gate_and_policy)


def test_phase_post_write_instrumented():
    from yadgar.backend.write_exec._memorize_phases import _phase_post_write
