...
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/core/server/tools/wiki.py::_check_wiki_add_context`, `yadgar/core/server/tools/wiki.py::_wiki_write_canonical`, `yadgar/core/server/tools/wiki.py::CANONICAL_PAGE_TYPES`, `yadgar/core/server/tools/_dir_branch.py`, `yadgar/core/hooks/session-start-context.py`, `yadgar/core/server/http.py::_persist_dir_branch_context`, `yadgar/backend/admin_exec/project.py::upsert_dir_branch_context`, `yadgar/backend/admin_exec/project.py::get_dir_branch_context`, `yadgar/_shared/storage/wiki.py::upsert_dir_branch_context`, `yadgar/backend/queue_drainer/dlq.py`
- **wiring:** The SessionStart context hook computes the TRUSTED per-directory `gitness` fact HOST-SIDE (`git rev-parse --is-inside-work-tree`) and passes it as a query param on the GET `/hooks/session-context` endpoint — the SOLE set-channel (no model-callable tool writes it → non-forgeable). `_persist_dir_branch_context` upserts it DURABLY (restart-safe) via the `upsert_dir_branch_context` admin op (ADR-0078: core never touches the DB) and fires the Manual cache invalidate. `_dir_branch.get_context` serves it through the `dir_branch_context` read-through cache (core `Cache` engine, `Manual`+`TTL(300s)`; miss → one backend `get_dir_branch_context` read → fill). At wiki-write time `_check_wiki_add_context` applies DIRECTORY enforcement only: empty directory + `YADGAR_DIRECTORY_ENFORCEMENT` on → REJECT `missing_directory`; otherwise proceed. Sanctioned server-side callers reach the canonical write seam via `_wiki_write_canonical` (sets `_internal`; asserts `page_type ∈ CANONICAL_PAGE_TYPES` as defense-in-depth). The drainer honors `_internal` and strips it before the DB write — unchanged. ADR-0215/0216: the `default_branch` half of the chain and the four-flow branch router are DELETED; `gitness` survives as the trusted non-forgeable fact.
- **explanation:** The trusted, non-forgeable per-directory `gitness` mechanism: a fact set ONLY by the host-side SessionStart hook, persisted durably and served from a core cache, never assertable by a model. Originally built (Car 0) so the canonical-vs-branch-scoped wiki-write decision could fall out of a trusted fact rather than a model-passable value; ADR-0215 then removed branch scoping entirely, so `_check_wiki_add_context` collapsed to directory enforcement and no longer consults `gitness`. ADR-0216 explicitly KEEPS the mechanism rather than deleting it with branch. `CANONICAL_PAGE_TYPES = {task_list, adr}` is a spoofable defense-in-depth assertion inside `_wiki_write_canonical`, NOT the gate — the real boundary is server-side-only reachability of that seam. The forgeable `__canonical__` sentinel from the prior draft is KILLED.
...
- **explanation:** Fixes the field-broken task-list mirror. The shipped template told the model to `wiki_add(page_type="task_list", NO branch_hint)`, which the branch router hard-rejected `missing_branch` in a git dir — `page_type` was never a canonical gate (forgeable) — so the mirror never persisted. `wiki_write_task_list` is a dedicated sanctioned writer whose sanction is STRUCTURAL (purpose-built, bounded to the `{project}-task-list` slug), NOT a spoofable arg — a model cannot use it to write an arbitrary page through the canonical seam. ADR-0215 removed branch scoping, so the original hard-reject is gone; the structural sanction and its coverage (`tests/core/test_car1_task_list_writer.py`) remain.
