...
- **migrations:** `024`
- **bc:** —
- **refs:** `yadgar/_shared/storage/migrations.py::_migration_024_wiki_page_hash_source`, `yadgar/_shared/storage/wiki.py::insert_wiki_page`, `yadgar/core/repo_wiki/generator.py::generate_module_page`, `yadgar/core/server/tools/project.py::_scan_stale_wiki_slugs_db`
...
- **tools:** —
- **migrations:** —
- **bc:** `BC-G1`, `BC-G2`, `BC-G3`, `BC-G4`, `BC-G6`, `BC-G7`, `BC-G8`, `BC-G9`, `BC-G10`
...
- **explanation:** BC-G1–BC-G10 specify wiki subsystem invariants: directory stamping on `wiki_add`, cross-project isolation on `wiki_query`, §25 slug resolution order (`dir+branch → dir+null → global → not-found`), immutable version creation on every write, similarity-based near-duplicate blocking, bookmark CRUD, branch-page cleanup, positional edit primitives, and multi-row `wiki_set_metadata` reach. BC-G5 (draft/approve workflow) was removed in v5.157.0 (Fix #76) — no production path ever produced drafts; `wiki_add` always committed directly. All remaining rows are ⏳ (unverified by automated test) or ⏳[u] in the contract as of v5.49.x.
...
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/core/server/tools/_forward.py::_forward_admin`, `yadgar/backend/embed_service/embed_service.py::admin_route`, `yadgar/backend/admin_exec/__init__.py::run_admin_op`, `yadgar/backend/admin_exec/bookmarks.py`, `yadgar/backend/admin_exec/blocks.py`
...
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/core/server/tools/_forward.py::_forward_restore`, `yadgar/backend/embed_service/embed_service.py::restore_route`, `yadgar/backend/restoration/__init__.py`, `yadgar/backend/admin_exec/restoration.py::pre_compact_drain`, `yadgar/core/cli/_shared.py`
...
- **category:** wiki
- **settings:** —
- **tools:** `wiki_cleanup_merged_branches`
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/core/server/tools/project.py::wiki_cleanup_merged_branches`, `yadgar/core/server/tools/project.py::_scan_stale_wiki_slugs`
- **wiring:** `wiki_cleanup_merged_branches` is `@_tool(power=True)`-registered in `project.py`. It queries `wiki_page` rows with `branch IS NOT NONE` then cross-references against `git branch -a` to find orphaned branch pages; with `dry_run=False` it deletes them. The stale-count signals path (`_scan_stale_wiki_slugs` → `_compute_stale_wiki_count`) runs host-side; the count is TTL-cached and surfaced in `project_brief(mode='signals')` — no MCP tool needed for detection, only for cleanup.
- **explanation:** `wiki_cleanup_merged_branches` finds wiki pages whose branch no longer exists in git and can delete them to prevent accumulation of dead feature-branch pages (§26 hygiene). Stale-page detection (`_scan_stale_wiki_slugs`, TTL-cached, 300 s default) surfaces a `stale_wiki_count` signal in `project_brief(mode='signals')` but does not expose a separate MCP tool — per ADR-0157, host-source operations that require filesystem access (`.local-review/wiki/*.md` hash scan) are CLI-only; `wiki_refresh_stale` was removed in #83 Car C.
...
- **refs:** `yadgar/core/server/tools/agent_prompts.py::agent_prompt_save`, `yadgar/core/server/tools/agent_prompts.py::_read_agent_prompt`, `yadgar/core/server/tools/dispatch_helper.py::agent_dispatch_prelude`
...
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/core/repo_wiki/__init__.py`, `yadgar/core/repo_wiki/scanner.py`, `yadgar/core/repo_wiki/generator.py`, `yadgar/core/cli/repo_wiki.py`
- **wiring:** CLI-only (host-side): `yadgar repo-wiki [REPO_PATH]` → `scan_repo(directory)` → `generate_wiki_pages(records, directory_context)` → emits page dicts to stdout (`--json`) or stderr summary. The caller then writes each page via `wiki_add(replace_slug=/force=, hash=…, source_file=…, wait=True)`. CLI registered in `yadgar/__main__.py` via `repo_wiki.register(subparsers)`. The MCP tool `repo_wiki_generate` was removed in #83 Car C (ADR-0157): it ran daemon-side in the container which cannot reach host paths, returning empty/error as if it were a real result. No Settings flag needed (pure read-only host-side scan).
- **explanation:** Native Python AST-based code-structure wiki generator. Walks a repository (reusing `_should_skip_dir` from `seed._scan`), parses each `.py` file with `ast.parse`, extracts module docstrings, function/method signatures and docstrings, and class hierarchies. Emits one wiki page per module (per-module granularity, not per-function) with `directory_context` stamped to the repo root absolute path — fixing the prior `global` mis-stamp that affected 364 `fn-`/`mod-` pages written by the external `/repo-wiki:repo-wiki` skill. The CLI is generate-and-emit-only; daemon contact was removed per ADR-0157 (container-blind anti-pattern). Option B (AST-graph via tree-sitter + leidenalg community detection) is a noted follow-on, not built here.

### CAP-WIKI-023 — Repo-wiki refresh stop-hook cadence (#83 Car D)

- **status:** LIVE
- **category:** wiki
- **settings:** `REPO_WIKI_REFRESH_STOP_INTERVAL`
- **tools:** —
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/core/hooks/stop-memory-checkpoint.py`, `yadgar/core/hooks/templates/repo_wiki_refresh_prompt.md`, `yadgar/core/cli/repo_wiki.py`
- **wiring:** Adds a third `MaintenanceItem` (`name="repo_wiki_refresh"`, priority 2, `state_key="last_repo_wiki_refresh"`) to the single Stop hook's ordered registry alongside checkpoint (priority 0, `INTERVAL=25`) and anchor-audit (priority 1, `ANCHOR_AUDIT_STOP_INTERVAL=100`). FIRST-DUE-WINS + advance-only-the-injected-item's-counter hold, so a due checkpoint or anchor-audit preempts the (slower) repo-wiki refresh, which then fires on the next eligible stop without consuming its turn. `REPO_WIKI_REFRESH_STOP_INTERVAL` (default 200, I25 three-way registered) is the human-message cadence between injections — slowest of the three because code-structure drift is rare absent an active refactor. The injected `repo_wiki_refresh_prompt.md` template is a dumb pipe that branches on per-project state: (1) `wiki_read("{project}-repo-wiki-index")` PRESENT ⇒ ENABLED — build the `{slug: hash}` baseline from `wiki_list`, pipe to `yadgar repo-wiki {dir} --stale-only --stored-hashes -`, write drifted pages via `wiki_add(replace_slug=/force=, hash=…, source_file=…, wait=True)`, `wiki_delete` removed slugs, rewrite the TOC when `toc_stale`; silent no-op when nothing drifted; NEVER asks. (2) TOC ABSENT ⇒ `recall(tags=["repo-wiki-optout"])`: if the opt-out marker exists, silent no-op (no nag); else ASK the user once — YES dispatches a background bulk-regen agent (`wiki_add(wait=True)` all pages + TOC) + creates one `[[{project}-repo-wiki-index]]` pointer-anchor; NO records the opt-out via `memorize(tags=["repo-wiki-optout"], is_protected=True)`. The TOC page's existence is itself the ENABLED marker — no separate "enabled" marker is stored.
- **explanation:** Wires the auto-refresh loop that CAP-WIKI-020's generator + CAP-WIKI-022's `--stale-only` host-side hash-diff were built for (#83's root-cause fix #5: `wiki_refresh_stale` existed but nothing called it, and was container-blind). Opt-in per project, self-maintaining once enabled, and never nags: the opt-out marker is a protected tagged memory because recall is natively per-`directory` (a config knob is global-only and cannot express per-project state; a sentinel file would be a new mechanism to build+test), and `is_protected=True` stops it decaying out and re-opening the ASK. Small acknowledged race: a stop between YES-dispatch and the background agent writing the TOC could re-ASK — negligible at the ~200-message cadence, not worth extra state.
...
- **tools:** —
- **migrations:** —
- **bc:** `BC-CODEGRAPH-1`, `BC-CODEGRAPH-2`, `BC-CODEGRAPH-3`, `BC-CODEGRAPH-4`, `BC-CODEGRAPH-5`
...
- **wiring:** GATED SWAP of the Stop hook's priority-2 slot (transition, not hard cutover). Both `repo_wiki_refresh` and a new `code_graph_refresh` `MaintenanceItem` (priority 2, `state_key="last_code_graph_refresh"`) are registered; their `is_due` functions are mutually exclusive on `code_graph.config.is_enabled(cwd)`, which reads the `code_graph.enabled` **runtime-config row** (ADR-0163 — SUPERSEDES the old `CODE_GRAPH_ENABLED` env flag). enabled ⇒ code_graph fires INSTEAD OF repo-wiki, disabled ⇒ repo-wiki fires as today and code_graph is inert — exactly one is ever due, so the shared priority never double-fires. **Dir-aware (ADR-0163):** the Stop payload's `cwd` is threaded through every `is_due` (uniform `(count, session_state, cwd)` signature; only code_graph uses `cwd`) so a per-repo opt-out (`code_graph.enabled=false` at that dir) → code_graph NOT due there (no wasted nudge); repo_wiki's is_due stays global-scoped (it is being retired, #33). Fail-open: daemon down → host client returns default True (flipped 2026-07-27, ADR-0163 addendum) → code_graph active. repo-wiki is NOT deleted (decommission is #33). Cadence `CODE_GRAPH_REFRESH_STOP_INTERVAL` (default 200, I25 three-way registered) mirrors `REPO_WIKI_REFRESH_STOP_INTERVAL`. The injected `code_graph_refresh_prompt.md` template is a dumb pipe: run `yadgar code-graph refresh <repo>`; on `skipped:true` do nothing (silent no-op); else `block_update(name="code_graph", scope="project", directory=<payload.directory>, content=<payload.content>)`, falling back to `block_create(...)` when the block does not exist. SessionStart soft-suggest (`session_suggest_line` → `_code_graph_suggest_line` in `http.py`, appended after `render_blocks_section`): when code_graph is enabled + not opted out (via `is_opted_out`, which reads the `code_graph.enabled` store row) AND no `code_graph` block exists for cwd, append a one-line nudge to run the refresh CLI; a present digest block is already injected so no suggestion; never auto-runs. **Container-blindness FIXED (ADR-0163):** `http.py` injects the in-process daemon resolver (`_runtime_config.config_get`) so the daemon reads the flag from its OWN DB — no longer blind to a host env var it never saw.
- **explanation:** Transitions the priority-2 maintenance slot from repo-wiki (CAP-WIKI-023) to the code_graph architecture digest behind an enable flag so repo-wiki keeps running until code_graph is proven (ADR-0162 Car F pilot-gate), then flips by setting `code_graph.enabled=true` in the runtime-config store (Car G5 setup prompt / `config_set`). The digest is an always-injected memory BLOCK (recall-free) rather than recall pages (repo-wiki's pages proved recall noise). Enable/opt-out is DB-backed + directory-scoped (ADR-0163): the old `CODE_GRAPH_ENABLED` env var (runtime enable) and `.code-graph-disable` repo-marker file are GONE — `CODE_GRAPH_ENABLED` survives only in `cli/setup.py` as a host-binary INSTALL trigger. **2026-07-27 addendum:** the pilot-gate is now satisfied (digest-layers PII/URL-literal leak fixed; proven live on Java/Go/PHP + yadgar itself) — `code_graph.enabled` DEFAULTS TO TRUE (opt-out) rather than requiring the Car G5 setup prompt / `config_set` to flip it on; `setup` persisting `true` is now redundant-but-harmless, and repo-wiki now yields to code_graph by default rather than only after an explicit opt-in.

### CAP-WIKI-021 — Canonical-write + trusted-gitness branch model (Car 0)

- **status:** LIVE
- **category:** write-path
- **settings:** `BRANCH_ENFORCEMENT`, `DIRECTORY_ENFORCEMENT`
- **tools:** `wiki_add`
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/core/server/tools/wiki.py::_check_wiki_add_context`, `yadgar/core/server/tools/wiki.py::_wiki_write_canonical`, `yadgar/core/server/tools/wiki.py::CANONICAL_PAGE_TYPES`, `yadgar/core/server/tools/_dir_branch.py`, `yadgar/core/server/tools/project.py::_get_default_branch`, `yadgar/core/hooks/session-start-context.py`, `yadgar/core/server/http.py::_persist_dir_branch_context`, `yadgar/backend/admin_exec/project.py::upsert_dir_branch_context`, `yadgar/backend/admin_exec/project.py::get_dir_branch_context`, `yadgar/_shared/storage/wiki.py::upsert_dir_branch_context`, `yadgar/backend/queue_drainer/dlq.py`
- **wiring:** The SessionStart context hook computes two TRUSTED per-directory facts HOST-SIDE (`gitness` via `git rev-parse --is-inside-work-tree`; `default_branch` via `symbolic-ref refs/remotes/origin/HEAD`, NULL when non-git) and passes them as query params on the GET `/hooks/session-context` endpoint — the SOLE set-channel (no model-callable tool writes them → non-forgeable). `_persist_dir_branch_context` upserts them DURABLY (restart-safe) via the `upsert_dir_branch_context` admin op (ADR-0078: core never touches the DB) and fires the Manual cache invalidate. At wiki-write time, `_check_wiki_add_context` reads the trusted `gitness` through the `dir_branch_context` read-through cache (core `Cache` engine, `Manual`+`TTL(300s)`; miss → one backend `get_dir_branch_context` read → fill) and applies the §0.4 flow table: (2a) git+branch_hint → branch-scoped; (2b) git+no-branch → REJECT `missing_branch`; (3) non-git → canonical (`branch=None`+`_internal`, hint ignored); (4) unknown dir / backend error → fail-safe require branch_hint. Sanctioned server-side callers reach canonical via `_wiki_write_canonical` (sets `branch=None`+`_internal`; asserts `page_type ∈ CANONICAL_PAGE_TYPES` as defense-in-depth). The drainer honors `_internal` (skips the branch reject, `dlq.py:137`) and strips it before the DB write (`dlq.py:254`) — unchanged.
- **explanation:** Foundation for canonical (`branch IS NULL`) wiki writes under `YADGAR_BRANCH_ENFORCEMENT=true`. Previously a missing branch was hard-rejected at both the MCP boundary and the drainer, and the only carve-out (`_internal`) was unreachable by a model — so canonical writes were impossible through `wiki_add`, breaking the task-list mirror and mis-pinning non-git ADR logs to a bogus "master". Car 0 makes the canonical decision fall out of the TRUSTED per-directory `gitness` (non-forgeable: set only by the SessionStart hook, read via a durable store + core cache) rather than any model-passable value. `CANONICAL_PAGE_TYPES = {task_list, adr}` is a spoofable defense-in-depth assertion inside `_wiki_write_canonical`, NOT the gate — the real boundary is server-side-only reachability of flow-1 canonical and trusted-gitness for flow-3. The forgeable `__canonical__` sentinel from the prior draft is KILLED. `_get_default_branch` now sources the trusted `default_branch` (NULL for non-git) instead of a daemon-side `git symbolic-ref` that cannot see the host `.git`. The `adr_add` / task-list consumers are wired in Cars 1/2.
...
- **wiring:** The stop-hook checkpoint protocol (step 4c of `stop_checkpoint_prompt.md`) calls `wiki_write_task_list(project, content, directory)` to persist the harness task list. The tool builds a fixed payload (`slug={project}-task-list`, `title="{project} task list"`, `page_type="task_list"`, `tags=["task-list"]`, `replace_slug={project}-task-list`), applies the same secret-gate / size / surrogate guards as `wiki_add`, then routes through the server-side `_wiki_write_canonical` (Car 0 CAP-WIKI-021 flow 1: `branch=None`+`_internal`, `wait` param threaded to the existing `_wiki_add_wait_path`). The page lands CANONICAL (`branch IS NULL`) so the SessionStart restore-nudge resolves it from ANY branch and from a non-git project.
- **explanation:** Fixes the field-broken task-list mirror. The shipped template told the model to `wiki_add(page_type="task_list", NO branch_hint)` to land canonical, but Car 0's router decides the branch from trusted `gitness` — `page_type` is NOT a canonical gate (§0.6 KILLED it as forgeable) — so in a git dir with no `branch_hint` the write hit flow 2b → hard-reject `missing_branch` and the mirror never persisted. `wiki_write_task_list` is a dedicated sanctioned writer whose sanction is STRUCTURAL (purpose-built, bounded to the `{project}-task-list` slug), NOT a spoofable arg — a model cannot use it to canonical-write an arbitrary page, so the killed `page_type`-as-gate hole is not reopened. Regression coverage (`tests/core/test_car1_task_list_writer.py`) exercises the REAL write THROUGH the daemon gate (the coverage hole that let the hard-rejected write ship) and pins the boundary that a raw `wiki_add(page_type="task_list")` in a git dir still rejects.
...
- **refs:** `yadgar/core/vacuum/phases.py::_atomic_swap`, `yadgar/core/vacuum/phases.py::_recover_interrupted_swap`, `yadgar/core/vacuum/phases.py::_vacuum_snapshot_and_drop`
...
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/core/server/tools/misc.py::checkpoint`, `yadgar/core/server/tools/misc.py::restore`, `yadgar/core/server/tools/_forward.py::_forward_restore`, `yadgar/backend/restoration/checkpoint_restore.py`, `yadgar/backend/restoration/__init__.py::run_restore`
...
- **explanation:** These thirteen tools form the core wiki CRUD and versioning surface. `wiki_add`/`wiki_update` write pages with directory+branch scoping and create immutable `wiki_page_version` records (BC-G4). `wiki_read`/`wiki_get` look up by slug with §25 resolution: directory+branch → directory+null → global → not-found (BC-G3). `wiki_query` performs semantic search scoped to a directory (BC-G2). `wiki_check_duplicate` runs the similarity gate (BC-G6). `wiki_history`/`wiki_read_version`/`wiki_diff`/`wiki_restore` expose the immutable version history. `wiki_set_metadata` patches metadata across all branch rows for a slug (BC-G10). `wiki_lint` validates a page against the wiki lint rules.
...
- **refs:** `yadgar/core/server/tools/wiki.py`, `docs/contracts/BEHAVIOR_CONTRACT.md`
- **wiring:** `wiki_append_section`, `wiki_insert_at`, `wiki_insert_after`, `wiki_insert_before`, `wiki_replace_at`, `wiki_replace_text`, `wiki_replace_markdown_block`, `wiki_delete`, `wiki_delete_at`, `wiki_delete_text` are in `wiki.py`; `wiki_cleanup_merged_branches` is in `project.py`. All `@_tool`, all reachable via FastMCP.
- **explanation:** These tools provide surgical positional edit operations on wiki pages (BC-G9): append a section, insert at/after/before a line, replace at a position, replace by text pattern, replace a markdown block, delete a page, delete at a line, delete by text pattern. `wiki_cleanup_merged_branches` removes pages scoped to merged git branches (BC-G8). All edits are versioned (each write creates a new `wiki_page_version` record per BC-G4). Note: `wiki_coverage` and `wiki_refresh_stale` were removed in #83 Car C (ADR-0157 — container-blind anti-pattern).
