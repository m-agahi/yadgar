...
Read "`refs:` file path resolves" literally: only the **path** half of a
`path.py::symbol` ref is checked. `check_capability_coverage.py` never opens the
file to look for the symbol, so a `::symbol` can name something that moved, was
renamed, or was deleted outright while the lint stays green. Measured 2026-08-28:
19 of the ~470 `::symbol` refs were broken that way — most of them casualties of
the core→backend compute move and the `admin_exec/ledger.py` split. They are
corrected in this revision, but nothing stops them re-rotting. **Re-verify the
symbol half by hand when you touch a subsystem**, exactly as you would a `status:`.
...
- **refs:** `yadgar/backend/retrieval/fusion.py::_convex_fuse`, `yadgar/core/server/tools/recall.py`
...
- **migrations:** —
- **bc:** `BC-AC3a`, `BC-AC3b`
- **refs:** `yadgar/_shared/astrocyte_pool/astrocyte_pool.py::AstrocytePool.consensus_retrieve`, `yadgar/backend/embed_service/embed_service_routes.py::_run_landscape_backend`
- **wiring:** Exposed via `recall(mode="landscape", ...)`. Since the Phase-2a forwarder split the core `recall()` tool no longer holds the landscape body: it forwards `mode` to the backend, which dispatches to `_run_landscape_backend()` on `mode == "landscape"` — this calls the pool's `consensus_retrieve(query, top_k=max_results)` and post-filters results with `is_directory_eligible()` (v5.65 directory contract). Results carry `consensus_score` (float) and `voting_domains` (list[str]) metadata. Landscape dispatch is exclusive: it short-circuits before fan-out and profile routing. Pool is `_st._pool` (set from `_st._consolidation.pool` in `lifecycle.py`). `BC-AC3a` flipped ✅ in v5.81 (build #67). `BC-AC3b` remains ⏳.
...
- **migrations:** —
- **bc:** `BC-RR5`
- **refs:** `yadgar/backend/retrieval/recall_pipeline.py::_apply_quality_floor`, `yadgar/backend/retrieval/recall_pipeline.py::_dedup_by_content`
...
- **migrations:** `008`
- **bc:** `BC-AN1`, `BC-AN2`
- **refs:** `yadgar/core/server/tools/misc.py::anchor`, `yadgar/_shared/server_helpers/server_helpers.py::_compute_valid_until`
...
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/_shared/server_helpers/server_helpers.py::_file_hash`
...
- **migrations:** `030`
- **bc:** —
- **refs:** `yadgar/_shared/storage/migrations.py::_migration_030_wiki_mutability_override`, `yadgar/_shared/wiki/policy.py::MUTABILITY_BY_TYPE`, `yadgar/_shared/storage/mutability_gate.py::enforce_mutability`, `yadgar/_shared/wiki/store.py::set_mutability_by_slug`, `yadgar/core/server/tools/wiki.py::wiki_set_mutability`, `yadgar/backend/admin_exec/wiki.py::wiki_set_mutability`
...
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/backend/queue_drainer/dlq.py::_sim_gate_for_drainer`, `yadgar/core/server/tools/wiki.py::wiki_add`
...
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/core/server/tools/misc.py::project_seed`, `yadgar/core/cli/project.py::parse_map`, `yadgar/core/cli/project.py::classify_row`, `yadgar/core/cli/project.py::infer_kind`, `yadgar/core/cli/project.py::seed_row`, `yadgar/core/cli/project.py::cmd_project_seed`, `yadgar/core/cli/project.py::register`, `yadgar/backend/admin_exec/ledger_project.py::create_project_row`, `yadgar/backend/admin_exec/__init__.py::_ADMIN_OPS["create_project_row"]`
...
- **bc:** —
- **refs:** `yadgar/core/cli/project.py::cmd_project_list`, `yadgar/core/cli/project.py::register`, `yadgar/backend/admin_exec/ledger_project.py::list_stale_projects`, `yadgar/backend/admin_exec/ledger_project.py::list_project_rows`, `yadgar/backend/admin_exec/__init__.py::_ADMIN_OPS["list_stale_projects"]`, `yadgar/_shared/storage/sql/registry.py::list_stale_projects`, `yadgar/_shared/storage/sql/migrations/versions/005_project_last_validated.py`, `yadgar/_shared/storage/sql/registry.py::assert_project_registered`
...
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/backend/admin_exec/invariants.py::_run_check_invariants`, `yadgar/core/server/tools/admin_invariants.py::check_invariants`
...
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/backend/admin_exec/invariants.py::_check_db_size`, `yadgar/_shared/storage/`
...
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/core/server/tools/_recent_memories.py::recent_memories`, `yadgar/_shared/storage/memory.py::get_recent_memories_since`
...
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/_shared/runtime/offload.py::acquire_rerank_slot`, `yadgar/backend/ml_client/remote_ml_client.py::RemoteMLClient._rerank_rpc`, `yadgar/core/server/http.py::liveness_check`, `yadgar/core/server/http.py::_build_health_payload`, `yadgar/core/auth_middleware/auth_middleware.py::_EXEMPT_PATHS`
...
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/core/server/tools/_test_tools.py::register_test_tools`, `yadgar/core/server/tools/__init__.py` (import-time registration)
...
- **migrations:** —
- **bc:** —
- **refs:** `yadgar/core/server/tools/misc.py::backup_restore`, `yadgar/core/cli/snapshot.py::cmd_snapshot_restore`, `yadgar/__main__.py` (snapshot subcommand wiring), `yadgar/core/backup/backup.py::restore_snapshot`
