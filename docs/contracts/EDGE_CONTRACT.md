...
| **derived_from** (entity provenance) | consolidation / memify_derive (entity graph) | yes (entity graph) | `recall` PPR + spreading-activation frontier expansion — traverses ALL relationship types (`_get_adjacent_batch(..., None)` in `backend/retrieval/graph_helpers.py` + `core.py`), so `derived_from` feeds the entity-graph signal | **retrieval** | **viz-rest (#209)**: render in viz. The LARGEST relationship type (3304 rows live) yet was INVISIBLE — entities whose only edges were `derived_from` showed a misleading "0 connections" lone-sphere badge. Now emitted with `role="retrieval"` (`role="informational"` would be the "legend lie" this contract kills — it demonstrably drives recall). Toggleable, default ON. Shares the `caps.relationships` cap (no per-type cap). `semantic_similarity` stays HIDDEN (retired by ADR-0009). |
...
- **Not every edge becomes retrieval.** `temporal` is a weak signal and stays **display**. Resisting "use every edge" is the whole point of this contract (anti leverage-theater). (v5.87 C3: the `semantic` edge — the vector signal recall already runs, so wiring it in is cost+noise — was removed from the viz entirely; its lazy KNN compute path was deleted.)
