...
**261 SHALLs / 41 subsystems.** Today: **57 ✅ · 199 ⏳ · 1 ❌.** (+ 2 🗑 RETIRED — BC-CM2/CM3, v5.71.0 #47; + 1 🗑 DELETED — BC-T2 remember tool, v6 T3; + 4 🗑 DELETED — BC-G5/T36/T37/T38 wiki-draft tools, v5.157.0 #76; + 2 🗑 DELETED — BC-T56/T58 container-blind repo-wiki/coverage tools, v5.160.0 #83) Of the 199 ⏳:
**59 `[r]` (real-path coverage exists) · 108 `[u]` (unit-only) · 32 none.**
Goal: every SHALL → ✅ or ❌.
...
- BC-E6 every vacuum abort path leaves `yadgar` CORE running. An explicit `systemctl --user stop` is never undone by `Restart=`, so an abort that restarts only the backend takes the memory engine down silently. Phase 2 stopped both units directly until task:0111; it now stops the backend only, but a host whose `yadgar.service` was rendered before the `Requires=`→`Wants=` flip still cascades (a generator change does not rewrite installed units), so the belt is unchanged. ⏳[u] `yadgar/tests/core/test_vacuum_finalize_verification.py::TestAbortPathsRestartCore` (task:0027a, task:0111)
...
- BC-E8 a vacuum stops the BACKEND only — the `yadgar` core stays up for the whole run, and the generated core unit `Wants=` (never `Requires=`) the backend so a backend stop does not cascade. Stopping both took the memory engine down ~68 s per run and dropped every connected MCP session; the core holds no fd into the store (ADR-0078) so the stop bought nothing. The finalize gate is kept and re-purposed: core `/health` is READINESS and round-trips the backend, so it still proves the backend came up on the compacted DB. ⏳[u] `yadgar/tests/core/test_vacuum_core_stays_up.py` + `yadgar/tests/scripts/test_core_backend_dependency_cross_generator.py` (task:0111, ADR-0188). **Not provable from a unit diff — needs a fresh non-nix VM: bounce the backend, core must stay `active` with an unchanged `MainPID`.**
...
### CG. code_graph (host-side code-structure via codebase-memory-mcp, ADR-0162)
- BC-CODEGRAPH-1 code_graph SHALL be default-ON (opt-out, flipped 2026-07-27): with no `code_graph.enabled` runtime-config row (ADR-0163) — or the daemon down (fail-open) — `is_enabled(dir)` returns True and every index/refresh/suggest path runs; an explicit `code_graph.enabled=false` (global or per-dir) SHALL skip (no indexing, no digest, no nudge). ⏳ [u] `tests/core/test_code_graph_config.py::TestEnabledFlag::test_default_on_when_no_row` P2
- BC-CODEGRAPH-2 indexing/query SHALL be a HOST-SIDE CLI (`yadgar code-graph index|query|refresh`) shelling out to the codebase-memory-mcp binary; the read-only MCP core container never runs the binary. ⏳ [u] `tests/core/test_code_graph_cli.py::TestRegistration::test_register_adds_code_graph` P2
...
- BC-CODEGRAPH-4 refresh SHALL index ONLY the latest `origin/<default-branch>` in a temp worktree (never the WIP tree); no remote / offline / fetch-fail → SKIP with a reason, never fall back to the working tree. **Clarification (task:0067):** "skip" binds the INDEXER — it SHALL never index the WIP tree. The CLI MAY still re-emit the previously-**cached** digest marked stale on such a skip (BC-CODEGRAPH-7); that re-emit reads only the existing index and never touches the working tree, so this SHALL is unaffected. ⏳ [u] `tests/core/test_code_graph_default_branch.py::TestSkipGuards::test_fetch_failure_skips_not_fallback` P2
...
- BC-CODEGRAPH-7 when `code-graph refresh` cannot re-index for reason `fetch_failed` but BOTH a cached index and a resolvable `head_sha` (`git rev-parse origin/<default>`) exist, it SHALL re-emit the cached digest with `skipped: false` and a `stale @ <12-char sha>` marker (rendered in the reserved preamble — BC-CODEGRAPH-8); when either guard is unmet, or the reason is `opted_out` / `no_remote_or_default_branch`, it SHALL hard-skip with a reason and emit no digest. A successful index SHALL carry no marker. ⏳ [u] `tests/core/test_code_graph_cli.py::TestDispatch::test_refresh_reemits_stale_marked_digest_on_fetch_failure` P2
- BC-CODEGRAPH-8 the `stale @ <sha>` freshness marker SHALL be rendered in a budget-reserved preamble immediately after the header, so it survives truncation on any digest that fills `DIGEST_CHAR_BUDGET`; the total digest SHALL remain ≤ `DIGEST_CHAR_BUDGET` (BC-CODEGRAPH-3 unaffected). ⏳ [u] `tests/core/test_code_graph_digest.py::TestStaleSurvivesBudget::test_marker_survives_budget_filling_digest` P2
...
- BC-CONFIG-2 config reads SHALL be a read-through PTC cache: a miss resolves via the durable store and caches the resolved value under the REQUESTED `(key, directory)` pair; a second `config_get` for the same pair is a hit that does NOT touch storage; a write whole-flushes the cache. ⏳ [u] `tests/core/test_runtime_config_resolver.py::TestPTC::test_miss_then_hit_second_get_skips_storage` P2
- BC-CONFIG-3 host-side config READS SHALL be fail-open: `runtime_config_client.get` returns the caller's `default` on daemon-down / timeout / non-200 / malformed JSON / null-value and NEVER raises (the stop-hook opt-out depends on this) — while host-side WRITES (`set`/`delete`) are NOT fail-open: any failure returns `False` so the caller can report "couldn't enable". ⏳ [u] `tests/core/test_runtime_config_client.py::TestHostClientFailOpen::test_connection_refused_returns_default` P2
- BC-CONFIG-4 code_graph enable SHALL default ON via the store (opt-out, flipped 2026-07-27): with no `code_graph.enabled` row (or the daemon down, fail-open) the resolver yields the `True` default so code_graph is active — there is ONE key and NO separate env-flag / repo-marker (ADR-0163 supersedes ADR-0162's mechanism); set `code_graph.enabled=false` (global or per-dir) to opt out. ⏳ [u] `tests/core/test_code_graph_config.py::TestEnabledFlag::test_default_on_when_no_row` P2
