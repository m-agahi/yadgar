...
- BC-CODEGRAPH-1 code_graph SHALL be default-OFF: with no `code_graph.enabled` runtime-config row (ADR-0163) — or the daemon down (fail-open) — `is_enabled(dir)` returns False and every index/refresh/suggest path skips (no indexing, no digest, no nudge). ⏳ [u] `tests/core/test_code_graph_config.py::TestEnabledFlag::test_default_off_when_no_row` P2
...
- BC-CONFIG-4 code_graph enable SHALL default OFF via the store: with no `code_graph.enabled` row (or the daemon down, fail-open) the resolver yields the `False` default so code_graph is inert — there is ONE key and NO separate env-flag / repo-marker (ADR-0163 supersedes ADR-0162's mechanism). ⏳ [u] `tests/core/test_code_graph_config.py::TestEnabledFlag::test_default_off_when_no_row` P2
