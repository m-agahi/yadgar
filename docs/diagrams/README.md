...
A small, **YAML-driven** flow-diagram generator. Each diagram is a data file
(current specs live under [`archive/<date>/`](archive/); `specs/` is the
authoring dir for new specs — see "Add a new diagram" below); [`generate.py`](generate.py)
renders a spec to SVG + PNG in `out/` (generated, git-ignored — regenerated by
`generate.py`, see #68). **Adding a new diagram is a new YAML file — no code change.**
...
# render every spec in specs/ (new authoring dir); current specs live in archive/<date>/
