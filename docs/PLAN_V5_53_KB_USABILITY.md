...
3. **Honest read-first rule (D).** Companion edit to the user's `~/.claude/CLAUDE.md` yadgar section (HAND TO USER via MIGRATION_NOTES — do not edit global CLAUDE.md ourselves): "Wiki = the map (conventions, module purpose, decisions, subsystem location); grep = the territory (exact current lines). Check the bootstrap catalog first, then grep there. Don't `wiki_query` for coordinates."
