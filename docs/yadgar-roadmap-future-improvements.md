...
|---|---|---|
| Core | **v5.45.1** | macOS launchd plist generation (paper-only, host verification deferred per `docs/maintainer-notes/todo.md` Deferred Verifications). Image build + nix bump PENDING (user-action follow-up). |
| Backend | **v5.4.0** | unchanged whole batch |
| Master | `4554a4e` | Deferred Verifications TODO section added. Pushed to codeberg. |
| Image registry (local) | `docker.io/openfantasy/yadgar:5.42.2 → 5.44.0` | v5.45.0 + v5.45.1 BUILD PENDING (user must `podman build --arch amd64 -t docker.io/openfantasy/yadgar:5.45.1`) |
| Nix repo | `d0c422c` | v5.45.x bump PENDING (user-action follow-up) |
...
**Recently shipped this cycle:** v5.42.2 → v5.42.6 hotfix train + v5.43.0 MCP schema discipline + v5.44.0 subagent MCP wiring + v5.45.0 make-canonical setup foundation + v5.45.1 macOS launchd (paper-only). See `## Recently shipped` for entries.
...
| **v5.46.0** | distribution — PyPI + brew + nix flake + SBOM (cut: no cross-repo PR auto-open) | drafted | 2-3d |
| **v5.46.1** | cross-repo PR auto-open (split from v5.46) | drafted (docs/v5.46.1-plan @ 94b458d) | 1-2d |
