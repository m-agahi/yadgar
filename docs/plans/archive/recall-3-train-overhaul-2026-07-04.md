...
**RE-NUMBERED 2026-07-09 evening (user decision):** a NEW **layer-boundary train is inserted as T2** ("without the clean and lean boundaries and code I can not move to T2 and T3") — `_shared` dual-import law, no-lone-files packaging, core↔backend misplacement moves, DB-path forwarding. Restructure becomes **T3**, Ettin swap becomes **T4**; observability P-SB queues after T4. The boundary train gets its own plan file (`layer-boundary-train-*.md`, sweep in progress); references below to "Train 2/3" keep their ORIGINAL content meaning (restructure/Ettin) — only the program-level numbering shifted.
...
- **Train 1 detail + audit:** `docs/plans/archive/recall-pipeline-to-backend-2026-07-04.md` (BLUF, target architecture, migration strategy, measure-first gate, and the Statefulness + Fanout audit whose verdict was GO-WITH-CAVEATS; shipped).
