> ARCHIVED 2026-07-10 — executing on feat/t2-layer-boundary, ships with this PR

# Plan: T2 — Layer-boundary train (placement law + no-lone-files + DB-path forwarding)

**Status:** EXECUTING — ONE-PR train on `feat/t2-layer-boundary`. Cars C + A + #38 hook fix + Car B (restore→backend, no waivers) + ADR-0093 memory-normalize fold-in + **Car D (lone-file→package conversions D1/D2/D3 + timing.py deletion + README/AGENTS doc scaffold + subsystem-README lint)** DONE on train; Car E (+ F verification) remains. Claims: core 5.124.0 + backend 5.36.0.
...
| **D — lone-file→package conversions + doc scaffold** | D1 `_shared` (29), D2 backend (8), D3 core (21); mechanical, PEP-562 shims, test-mirror moves. **Docs (user-agreed 2026-07-09): README.md per SUBSYSTEM** (layer roots + major packages: storage, retrieval→backend, server, viz, config, observability… ~12-15 total) + **AGENTS.md per LAYER root** (3: core/, backend/, _shared/ — layer rules, placement laws, forward seams); root README signposts them. Leaf modules: docstrings remain the doc (feed the mod-/fn- wiki generator — no second source of truth). Existence lint: every major package dir has README (I-invariant, existence only). **Module versioning REJECTED** (user): versions belong to the 2 release units; per-module change tracking = `git log -- path` | biggest churn, low risk | rebase AFTER hardening Car 1 merges (conftest churn) |
