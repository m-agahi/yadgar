...
**Status: BUILD COMPLETE, CUTOVER NOT RUN — 2026-08-12. This plan stays LIVE per ADR-0081's
partial-ship clause; it is deliberately not archived.**

> **The paragraph below this one is the 2026-08-02 status, and it is now FALSIFIED by the tree.**
> It is kept rather than deleted because it is the record of what was believed at build-ready
> time, and because "no code exists yet" is exactly the claim a reader should be able to see
> was overtaken.
>
> **Shipped:** all 16 cars (A0, A, B, C1, C2, C3, D, E, F, G, H, I, J, K, L, M) landed in the
> **0047 spine train (PR #40)**, together with remediation cars **C0–C16**
> ([`archive/0047-pr40-remediation-2026-08-10.md`](archive/0047-pr40-remediation-2026-08-10.md)).
> §10's two open questions were **decided at Car H build time** and are binding
> ([`archive/0047-car-H-tier-subsystem-rollups.md`](archive/0047-car-H-tier-subsystem-rollups.md)).
> §9's items were filed separately by construction. Car E shipped partial scope, and its
> graceful fallback to the legacy wiki path is discharged now that Car D has merged.
>
> **NOT shipped — the reason this plan is still live: §6.2, the D35 cutover.** It has never
> executed, anywhere. The C6 backfill has never run, the `project` registry has no production
> rows, the C7 read flip has never met the live corpus, and D35d's kept-and-ignored marking of
> the three index pages has not happened. That work has a tracked owner:
> **[`archive/0047-spine-train.md`](archive/0047-spine-train.md) §3 — the operator runbook.**
> The rehearsal that §8 of the remediation plan required before the cutover **was not executed**;
> the train doc §4 records the measurement.
>
> **Archive this plan when the cutover has run and D35c's verification gate has passed** — not
> before. Archiving it now would make a live, unexecuted migration invisible, which is the
> failure ADR-0081 exists to prevent.

**Status (2026-08-02, superseded — see above):** BUILD-READY. All §12 items resolved. §10 items
deferred to Car H. Not started; **no code exists yet** — there is no `task` table in
`migrations.py` and no `_LedgerMixin`, despite ADR-0183 D30 naming the latter "the engine seam."
