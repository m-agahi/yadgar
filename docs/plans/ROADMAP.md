...
| [fix-claude-code-mcp-auth-token-missing-2026-07-28](fix-claude-code-mcp-auth-token-missing-2026-07-28.md) | install / mcp-auth (#71) | **PLAN — ready to build** | Fresh-VM QA bug: `yadgar install --client claude-code` writes a headerless (401'ing) MCP entry into `~/.claude.json`. Root cause: write-path token resolved from `os.environ` only (`cli/install.py:97`, `mcp_register.py:274`); on a fresh VM the token lives in `secrets.env` (which the daemon sources, the install shell doesn't) → `BEARER_LITERAL` drops the header on empty token. `--print` + opencode mask it (both emit env-ref unconditionally). Fix = shared `resolve_mcp_auth_token()` with `secrets.env` fallback (mirrors `yadgar setup`'s proven path); real read-back e2e test at the token-resolution seam (AC-2). Option B (flip CC to `BEARER_ENVREF`) rejected — CC env-ref expansion in headers unverified (D5 TODO). |
...
| Plan | Theme | Status | Notes |
|------|-------|--------|-------|
| [fix-systemd-generate-missing-queue-base-2026-07-28](fix-systemd-generate-missing-queue-base-2026-07-28.md) | install / systemd (#72) | **DESIGN — awaiting user decisions** | Fresh-VM QA: real `yadgar-setup` install renders backend units with **zero** `YADGAR_QUEUE_BASE` → drainer disabled → queued writes never commit (`memory_stats` stuck 0). Bug is in the `.in` template + launchd plist (renderers are dumb `sed`), NOT `generate_systemd.sh`. Value must be `/data` (shared bind mount), not `/queue-data`. #233 fixed 3 of 5 generators, missed these 2. Fix = 2 one-line template patches + a cross-generator regression test (mount↔base coherence). Separate co-symptoms tracked as own tasks: `/admin` 503 `StorageEngine not initialized`, and `safe_start.__main__` missing (ADR-0084 packaging regression — split-brain guard silently dead, possible cascade into the 503). Open decision: consolidate 5 generators vs minimal patch. |
