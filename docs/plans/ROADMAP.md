...
- **v5.166.0 + v5.166.1** — OpenCode hook port train, 10 cars + 4 follow-up
  commits ([opencode-hook-port-train-2026-07-26](archive/opencode-hook-port-train-2026-07-26.md),
...
| [recall-scoring-c4-2026-07-18](archive/recall-scoring-c4-2026-07-18.md) | recall / scoring (#62) | **DECISIONS LOCKED — C4.0/C4.1/C4.3 BUILT (v5.151.0); 1b-fix + S3 DEFERRED** | C4 recall-scoring scoping plan; tie-order + abbreviation-miss cars landed, corpus-side SNR (S3) and the 1b-fix remain deferred. |
...
| [db-audit-fix](archive/db-audit-fix.md) | data-integrity | **DEFERRED INDEFINITELY** | Re-audited 2026-07-16: store healthy. Residuals: ~5 dangling edge rows + unverifiable `last_decay_at` — not worth work. |

### In-flight migrations / investigations
| Plan | Theme | Status | Notes |
|------|-------|--------|-------|
| [investigation-migration-script-system-2026-07-26](archive/investigation-migration-script-system-2026-07-26.md) | db-migrations (#45) | **undecided — review-only** | Survey of the DB migration-script system now that yadgar has multiple users; design proposed, no commitments. Sister to `surrealmigrate-fork-2026-07-26` and `settings-to-db-config-migration-2026-07-24`. |
| [surrealmigrate-fork-2026-07-26](archive/surrealmigrate-fork-2026-07-26.md) | db-migrations | **undecided — review-only** | Proposal to fork `surrealdb/surrealkit` for LLM-authoring/Python/embedded-mode work while upstreaming small Flyway-parity gaps. No commitments. |
...
| [task-table-refactor-2026-07-29](task-table-refactor-2026-07-29.md) | data-model (#0047, #0080) | **BUILD COMPLETE, CUTOVER NOT RUN — NOT archived, per ADR-0081** | `_LedgerMixin` spine: dedicated tables for `task`, `adr`, `agent_prompt`. ADR-0182 (supersedes ADR-0181). All 16 cars (A0–M) shipped in the 0047 spine train (PR #40), plus remediation cars C0–C16; §10's two open questions were decided at Car H build time. **What remains is §6.2's cutover — never executed:** the C6 backfill has not run, the `project` registry has no production rows, and the C7 read flip has not met the live corpus. Operator runbook: [`archive/0047-spine-train.md`](archive/0047-spine-train.md) §3. |
| [0047-spine-train](archive/0047-spine-train.md) | data-model / **operator runbook** (#0047) | **CODE SHIPPED (PR #40) — deploy pending** | Train doc for the 0047 spine train and the **tracked operator runbook** for its cutover (deploy order, backup, `dry_run` sequence, per-migration verification queries, rollback). Also the rehearsal record (§4): remediation-plan §8 was **NOT executed** — the VM and snapshot it names do not exist, so its §9 exit criterion stays **unticked**. §5 carries the plan-lifecycle resolutions and §5.1 the `check-plan-signature-drift` remit verdict (out of remit twice — `archive/**` excluded at `check_plan_signature_drift.py:400`, and it models call kwargs, not prose claims; **do not widen it**). |
...
| Plan | Theme | Status | Notes |
|------|-------|--------|-------|
| [cpu-burst-rootcause-and-embedding-scan-fix](archive/cpu-burst-rootcause-and-embedding-scan-fix.md) | perf | **Part 2 buildable; Part 1 deferred** | Part 2 = kill `SELECT *` consolidation scans (projection + server-side decay + sample-then-fetch + incremental linking). Part 1 = fan-burst deferred (task #26: likely non-yadgar / observer-effect, unresolved). See also ADR-0005 (OPEN). improvement-train A1 / #30–33. |
...
| [viz-trace-replay-2026-07-09](archive/viz-trace-replay-2026-07-09.md) | viz | **Phases 0–2 SHIPPED (core 5.145.0); Phase 3 OPEN** | Traces tab — live trace-replay mesh + oscilloscope design language. Phase 3 (SSE `trace_complete` + live metrics) deferred, see task backlog. |
