...
| [test-suite-speedup-2026-07-01](test-suite-speedup-2026-07-01.md) | test-perf / ci | **NEW — measured, buildable** | Profiled the pytest suite: wall-clock is FLAT across `-n4..auto` (437±3s) → tuning `-n` is NOT a lever. Floor = per-test `init_engines()` (schema+migrations, ~5–10s) in 91 function-scoped autouse fixtures. Severe surreal flakes are CROSS-RUN (CI-vs-local collision; CI bypasses `make test` TEST_LOCK), not in-run. Levers: P0 module-scope engines fixture (biggest wall-clock), P1 fix OTLP per-test DNS drain (cheapest), P2 CI-vs-local mutual exclusion (biggest reliability), P3 tighten e2e pre-push gate, P4 separate CI runners. HF-cache + mock-embed + `-n`-tune measured LOW-value. Plan only. |
