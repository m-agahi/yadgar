...
- **Car 2** — formalize **ALL** Protocols: `MLClientProtocol` + `CacheProtocol` + **`StorageProtocol`** (user override, no defer) in `_shared/protocols.py` + DI wiring → `core→backend` allowlist EMPTY. **+ glue-edge class-b:** MOVE `server.tools._recall_shadow` → `_shared/runtime/cache_epoch` (used by `_shared/storage/wiki` + `consolidation/cls`; no core deps; ~7 call sites).
- **Car 3** — **slim `_state`**: `engine_set="slim"` kwarg; backend builds the 14; core/memorize/consolidation build full. Behavior-neutral (recall parity + backend-recall smoke). **+ glue-edge class-c:** SPLIT `lifecycle.main()` — extract pure `_shared` engine-init from core app-startup (`server._app.mcp_server` + `tools.misc.sync_instructions`/`install_hooks` → core startup); + move recall_pipeline utils (`_is_episodic_query`/`_bounded_set`) off `server._helpers`.

**GLUE-EDGE class (Car-1 flag, investigation a9b76d54) — `_shared → server(→core)` edges that WILL fail Car-4 hard-fail:** import-linter 2.0 catches **function-local imports (AST)** → Car-1's func-local deferrals do NOT hide these; ALL must be genuinely resolved before Car 4. (a) TEXT-PATH (metrics.py→`_offload`/`_state`, already shimmed to `_shared.runtime`) → Car 4 import-update + shim-delete. (b) MOVE → Car 2 (`_recall_shadow`). (c) INVERT `lifecycle.main()` core-startup → Car 3.
