...
- **Naming rule, and it binds titles everywhere — not just the seeder.** A title carries the CURRENT ledger id and nothing else. No `[L41]` wrapper, no pre-migration ids (`0047`, `0157`, `task:0080`), no "formerly X". Those old numbers survive legitimately in *filenames* (`docs/plans/archive/0047-spine-train.md`) and in prose that cites a document, but a title that leads with a retired id makes the reader resolve two numbering schemes to find one row. Observed drift 2026-08-16: task 91's title was written as "0047 train: …" purely because the archive filenames use that stem — no reasoning behind it, and it contradicted this very spec written hours earlier.
...
**SECOND DEFECT IN THE SAME FUNCTION — `title` is silently discarded on UPDATE (ledger task 111).** Found 2026-08-16 by the user after I reported two task titles as updated on the strength of `ok: true` and did not re-read the rows. `_build_update_payload` (`task.py:199-237`) forwards `status`, `state`, `active_form`, `plan_path`, `body_slug`, `blocked_by` and `blocks` — and never `title`. This is worse than the `blocks` case (task 92): `title` is a REQUIRED argument of `task_write`, is run through `_validate_write_inputs`, and is then dropped. Storage already supports it — `update_task_row`'s allowlist includes `"title"` (`mariadb.py:421-459`). Pure core-side gap, and the `ok: true` return makes a discarded write look applied.

Fix with the clear flags below, same function, same commit: forward `title` on the update path. It is required, so it is always present; a caller that states a title means it. Verify by re-reading the row, not by trusting `ok`. Also confirm whether `project_id` is deliberately immutable on update or dropped by the same oversight.
