...
- [`PLAN_V5_9_ANCHOR_AUDIT.md`](../PLAN_V5_9_ANCHOR_AUDIT.md) — `audit_anchors(dry_run=True)` MCP tool. Extend `consolidate_now()` with anchor pass. Promote-to-wiki draft generator (no auto-`wiki_add`).
- [`PLAN_V5_10_TEST_HARNESS_HARDENING.md`](../PLAN_V5_10_TEST_HARNESS_HARDENING.md) — pytest-timeout plugin + SurrealDB fixture atexit cleanup + xdist port-range determinism + watchdog systemd-user timer. Deferred from v5.7.14 after v5.8.0 shipped first. Closes recurring CPU-fan / orphan-process / false-regression root cause (2026-05-28 investigation).
- [`PLAN_V5_11_ANCHOR_CROSS_PROJECT.md`](../PLAN_V5_11_ANCHOR_CROSS_PROJECT.md) — cross-project anchor dedup (`scope=both` aware). Optional Jira MCP integration (graceful degrade). `is_protected` flag repurposed as verified-by-3-clean-audits. Was originally v5.10.

Sequencing: v5.8 (shipped) → v5.9 (anchor audit) → v5.10 (test harness) → v5.11 (cross-project + Jira). Backend v5.4.0 caching independent, can run in parallel.
