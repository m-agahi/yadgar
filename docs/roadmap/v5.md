...
- [`PLAN_SESSION_END_CAPTURE.md`](../PLAN_SESSION_END_CAPTURE.md) — sentinel-marker pattern for `SessionEnd` hook: hook writes `~/.yadgar/session-ends/<session_id>.json` (no daemon dependency), SessionStart imports into `_session_end_sentinel` memory row, `project_brief(mode="signals")` surfaces `extract_last_session_findings` action. Closes: short sessions never triggering stop hook, tail loss after last stop-hook fire, and `checkpoint()` findings not cross-session-searchable. 4 new env knobs (I25). Ships as v5.10.3 after v5.10.2 unified train. **Drafted + advisor-reviewed 2026-05-29.**
