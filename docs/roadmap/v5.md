...
- [`PLAN_BACKEND_V5_4_CACHING.md`](../PLAN_BACKEND_V5_4_CACHING.md) — backend recall caching (drafted, not started).

### Anchor hygiene trilogy (drafted 2026-05-27, post-v5.7.12)

- [`PLAN_V5_8_ANCHOR_HYGIENE.md`](../PLAN_V5_8_ANCHOR_HYGIENE.md) — `valid_until` field + `tier` enum (`semantic_immortal | conditional | ephemeral`) on memorize/anchor. 3 new signals in `project_brief(mode="signals")` (`anchor_count_project`, `anchor_redundancy_candidates`, `anchor_promote_candidates`). 4 new `recommended_actions` action types. 6 env knobs. Schema migration with grace period.
- [`PLAN_V5_9_ANCHOR_AUDIT.md`](../PLAN_V5_9_ANCHOR_AUDIT.md) — `audit_anchors(directory, dry_run=True)` MCP tool. Extend `consolidate_now()` with anchor pass. Promote-to-wiki draft generator (no auto-`wiki_add`).
- [`PLAN_V5_10_ANCHOR_CROSS_PROJECT.md`](../PLAN_V5_10_ANCHOR_CROSS_PROJECT.md) — cross-project anchor dedup (`scope=both` aware). Optional Jira MCP integration (graceful degrade). `is_protected` flag repurposed as verified-by-3-clean-audits.

Sequencing: v5.7.13 hotfix sweep → v5.8 → v5.9 (after 2 weeks real-world data) → v5.10 (after 4 weeks audit history). Backend v5.4.0 caching independent, can run in parallel.
