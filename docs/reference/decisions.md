...
- Roadmap pipeline `yadgar-roadmap-future-improvements`: "v7.0.0 Real-time synthesis" row deleted; "v8.0.0 Team usability" row renamed → "v7.0.0 Team usability"; cross-references updated.
...
- Roadmap pipeline (`yadgar-roadmap-future-improvements`) — v7.0.0 row needs update based on Option
...
**Decision:** v5.46.2 CI run surfaced 27 issue classes (24 BLOCKING + 3 WARNING) per `docs/reports/ci/ci-issues-2026-06-05.md` @706e61d. Fix in a chained release cycle v5.46.3 → v5.46.6 before v5.47.0 dispatch. Umbrella plan at `docs/PLAN_V5_46_CI_GREEN_CYCLE.md`.
...
2. Add a one-line pointer in the plan file back to this file. For example: "See `docs/reference/decisions.md` — 2026-05-30 Plan-derived deferrals."
...
**Audit doc:** `docs/reports/audits/competitor-audit-2026-05-30.md` (commit `635781e`)
...
- **Evidence:** `docs/reports/audits/competitor-audit-2026-05-30.md` Refactor section R2. Current pipeline coupled in single `recall()` function.
...
- **Open questions retained in plan docs (not blockers for PD-37):**
  ...
  - Anonymous version-check payload exact wire shape — corporate firewalls + privacy auditors (resolved during v5.47 Step 0 implementation; documented in `docs/reference/privacy.md`).
...
- **This file** lives at `docs/reference/decisions.md` on master (renamed from `docs/AUDIT_DECISIONS.md` on 2026-05-30).
- **Mirrored** to wiki page `yadgar-audit-decisions-log` (search-discoverable).
- **When dispatching an audit agent**, include in the prompt: "Read `docs/reference/decisions.md` first. Do not re-recommend items marked KEEP-AS-IS, REJECT, or DEFER unless their revisit triggers have fired."
- **When drafting any plan**, any "does not ship / out of scope / later" item of architectural significance must be extracted here before merge. Add a pointer in the plan file: "See `docs/reference/decisions.md` — [dated section]."
