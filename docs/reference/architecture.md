...
6. **Reranking** — cross-encoder scores top-K pairs, torch fp32. Primary = **Ettin-32m** (`cross-encoder/ettin-reranker-32m-v1`, shipped in Train 4); **GTE-ModernBERT** (`Alibaba-NLP/gte-reranker-modernbert-base`) is the config-revert rollback (baked into `Dockerfile.backend` one cycle). FlashRank is NOT a dependency (absent from pyproject/uv.lock; `_try_flashrank` is a lazy no-op). (ONNX backends removed: `CROSS_ENCODER_BACKEND` in R3 after the ADR-0043 NO-GO; the `GTE_RERANKER_BACKEND=onnx-int8` path removed in the 5.131.0 deps train — optimum-onnx caps transformers<4.58, incompatible with the transformers>=5.0 floor.) (`yadgar/_shared/config/config.py`, `yadgar/backend/ml_client/ml_client.py`)
...
**Latency accounting (v5.102 → corrected ADR-0105, #192):** recall latency is FULLY accounted via MCP-tool Tempo traces (real user POV). The old "CE dominates ~90%" figure was a **dead-metric artifact** — the CE histogram (`embed_rerank_duration_seconds{mode="ce"}`) went silent at ADR-0078/T2 when retrieval CE moved in-process, so the 70–90% claim reflected the split-container era, not current arch. Corrected accounting (ADR-0105): on the **cold** recall wall CE is **~25%**; the signal-gather stage (KNN/FTS/PPR/spreading) is the ~45% dominator. Ettin-32m (Train 4) cut CE per-pass ~4.7× vs GTE-ModernBERT — cross-version sweep GTE ~12.8s vs Ettin ~5.3s @2cpu = 2.44× end-to-end (#193, `docs/testing/recall-perf-checklist.md`). The fusion CE pass stays **quality-load-bearing** (removing double-CE dropped MRR 0.84 → 0.74). Spreading activation was a real N+1 (per-activated-entity round-trips) — **batched in v5.104** (#156, exact-parity: recall ~5 s → tens-ms), heat writes batched in v5.102.

For the full measurement methodology, CPU-scaling series, and span-level breakdown, see `docs/testing/recall-perf-checklist.md` and `docs/testing/recall-span-attribution-2026-07-13.md`.
...
- **Layout (v5.87 → v5.88; viz-render-perf):** **precomputed server-side layout** is now unconditional — the nightly/full consolidation cycle runs `networkx.spring_layout` (3D, capped iterations), signature-caches x/y/z in a `graph_layout_cache` row, and `/api/graph` serves the precomputed coordinates. On a seed miss (empty cache, or nodes newer than the last precompute) the client falls back to its d3-force sim with localStorage warm-start. Node caps `VIZ_MAX_MEMORIES` (500) / `VIZ_MAX_WIKI` (200) / `VIZ_MAX_ENTITIES` (2000) trade load speed vs. completeness (`0`/`-1` = unlimited).
