...
**Depends on:** v5.50.0 shipped — this plan extends the Control tab + Info tab debug panel surfaces introduced there. Also depends on the `YADGAR_DEBUG_APIS_ENABLED` env gate shipped in v5.50.0.
...
1. **Debug viz interaction APIs.** Four categories of HTTP endpoints that let an external script (or a Claude agent) drive the viz programmatically: select / inspect a node, simulate overlay interactions, control the camera, query event log + take screenshots. Same auth gate as the Control tab (`YADGAR_DEBUG_APIS_ENABLED=on` + bearer token).

2. **Console log capture.** Two stacked panels in the Control tab (or Info debug panel — decided in Open Question 1 below): a top panel streaming daemon log lines via SSE or polling, a bottom panel capturing browser console output via a `window.console` proxy. Each panel has level filter (DEBUG/INFO/WARN/ERROR), pause, clear, copy-all.
...
- **No persistence of debug events.** The event log is an in-memory ring buffer, capped at 1000 entries. Restart wipes it.
...
| Asset | Path | Status |
|---|---|---|
| Debug API gate | `YADGAR_DEBUG_APIS_ENABLED` | Introduced in v5.50.0. Bool, default `False`. Auth middleware checks it on `/api/control/*`. |
...
| Control tab debug panel | placeholder, no live content | Empty container shipped in v5.50.0, awaiting v5.52.0. |
...
| File | Change |
|---|---|
| `yadgar/server/api/debug_viz.py` | NEW. Four endpoint groups (12 endpoints total — see API surface below). All gated on `YADGAR_DEBUG_APIS_ENABLED=on` + bearer token. |
| `yadgar/server/api/logs.py` | NEW. `GET /api/logs/_capabilities` (SSE vs poll detection). `GET /api/logs/stream` (SSE — `text/event-stream` of daemon log lines, level-tagged). `GET /api/logs/poll?since=<seq>` (fallback long-poll). |
| `yadgar/server/viz_state.py` | NEW. In-memory state mirror: `selected_node_id`, `camera` (position + look-at), `overlays` (per-name `{collapsed, position}`), `event_log` (ring buffer, 1000 entries). Updated via WebSocket from the browser tick loop OR via a polling endpoint the browser hits at 1Hz. |
| `yadgar/server/api/control.py` | EXTEND (from v5.50.0). Mount debug viz routes under the same auth gate. No new endpoints in `control.py` itself. |
| `yadgar/settings.py` | Register `YADGAR_DEBUG_LOG_BUFFER_SIZE` (int, default 1000), `YADGAR_DEBUG_VIZ_STATE_POLL_HZ` (int, default 1). Three-way per I25. |
| `config.yaml` | Add `debug.log_buffer_size=1000`, `debug.viz_state_poll_hz=1`. |

### Frontend

| File | Change |
|---|---|
| `yadgar/server/static/js/control.js` | EXTEND (from v5.50.0). Add two stacked log panels at bottom of Control tab. Top: daemon log via SSE/poll. Bottom: browser console proxy. Each panel: level filter row + pause/clear/copy-all controls + scrolling list of entries. |
| `yadgar/server/static/js/console_capture.js` | NEW. Installs `window.console` proxy at DOMContentLoaded. Captures `log`, `info`, `warn`, `error`, `debug`. Buffers up to 1000 entries in memory. Exposes a subscribe API for `control.js` to pull. |
| `yadgar/server/static/js/viz_state_publisher.js` | NEW. Hooked into `home.js`. On graph events (node select, camera move, overlay toggle), POST a delta to `/api/debug/viz/_publish_state`. Throttled to 1Hz. Skipped entirely if `YADGAR_DEBUG_APIS_ENABLED=off`. |
| `yadgar/server/static/js/home.js` | EXTEND. Hook viz events to `viz_state_publisher.js`. |
...
| `yadgar/tests/test_console_capture.py` | NEW. Jsdom. `console.log("foo")` captured. Level filter works. Pause halts new entries. Clear empties the buffer. |
...
1. **Where do the two log panels live?** Two options: (a) Control tab bottom section (visible whenever Control is open); (b) Info tab debug panel entrypoint (collapsed by default, expands on click). Lean (a) — Control tab is already auth-gated, fewer surprises. If Control tab becomes crowded, split: console capture in Info, daemon log in Control. Decide during Step 3.
...
- Confirm v5.50.0 shipped on master.
- Confirm `YADGAR_DEBUG_APIS_ENABLED` gate is wired into the auth middleware.
- Snapshot `control.js` size pre-extension for regression comparison.
...
- Stub all 12 endpoints in `debug_viz.py` returning `501 Not Implemented` (gate-passing) so the 403 tests go green.
- Mount under `/api/debug/viz/` with the v5.50.0 auth middleware.

### Step 2 — Viz state mirror (≤ 0.5 day)
- TDD: state publisher accepts a delta, GET returns the merged state.
- Implement `viz_state.py` in-memory mirror with ring buffer.
- Implement `viz_state_publisher.js` frontend hook. 1Hz throttle.
- Wire into `home.js` event handlers.
...
- Implement `/api/logs/stream` (SSE) and `/api/logs/poll` (long-poll fallback).
...
- TDD: `test_console_capture.py` — jsdom round-trip.
- Implement `console_capture.js` proxy. Install at DOMContentLoaded.
- Capture `log`, `info`, `warn`, `error`, `debug`. `error` also captures stack trace.

### Step 7 — Two log panels UI (≤ 0.5 day)
- Extend `control.js`. Two stacked panels at bottom of Control tab.
...
- [ ] Control tab bottom section shows two stacked log panels (daemon top, browser console bottom).
...
| Event ring buffer overflows on a busy session, losing early events | 1000 entries default. Bumpable via `YADGAR_DEBUG_LOG_BUFFER_SIZE`. Document the wrap-around behavior; callers must read with `since=<seq>` and check `next_seq` for gap detection. |
...
- **v5.50.0 must ship first.** This plan extends Control tab + Info tab + the `YADGAR_DEBUG_APIS_ENABLED` gate, all introduced there.
...
- This plan unblocks remote agent-driven viz QA — agents can now select / inspect / screenshot the viz without a human in the loop. Worth a separate wiki note post-ship.
