...
**DEPRECATED (per-train discussion; not in this car):** the parallel `yadgar install-hooks` CLI subcommand is now redundant with `yadgar install --client <name> --hooks`. Per AGENTS.md "forward-only, no backward-compat knobs/dual-paths", a follow-up train will (a) delegate `install-hooks` to `install_client` for one source of truth, (b) add a deprecation warning, (c) update `scripts/install/yadgar-setup.sh` step 6, (d) update docs.
