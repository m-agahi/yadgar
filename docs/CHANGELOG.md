...
**`check_backend_bump`'s trigger set was blind to `yadgar/_shared/**`, so a backend-executed change could ship stale with no version-bump gate — and one already had.** Core `5.187.0` → `5.187.1`; backend `5.78.0` → `5.78.1`. `Dockerfile.backend:8` is `COPY . /app` — the backend image ships the entire repo — but `BACKEND_BUILD_DIRS` in `scripts/check_backend_bump.py` only named `("backend",)`. `PR #60` (`b96360fd`) changed `yadgar/_shared/wiki/store.py` (`_METADATA_FIELDS` gained `"project_id"`); the guard never fired, so the deployed backend kept running its stale copy and `wiki_set_metadata(field="project_id")` still returned the pre-fix error in production. Measured the backend's real import graph before widening: `yadgar/backend/**` imports `yadgar/_shared/**` and `yadgar/backend/**` at hundreds of sites each, and `yadgar/core/**` at zero — so `BACKEND_BUILD_DIRS` is now `("backend", "_shared")`, not "everything `COPY . /app` covers" (which would demand a bump on every commit and get the gate turned off within a week). Anti-recurrence: `TestBackendBuildDirsMatchRealImportGraph` (`yadgar/tests/scripts/test_check_backend_bump.py`) derives the trigger set from `yadgar/backend/**`'s actual `ast` import statements at test time and asserts it agrees with `BACKEND_BUILD_DIRS`, so a hardcoded second copy of the list — which would only reproduce this same defect — cannot silently drift from the constant again. The guard runs in pre-commit (`.pre-commit-config.yaml`) and both CI workflows (`ci-pr.yml`, `ci-pr.yaml`) via the same script, so the one constant fix covers all three call sites. This entry's own version bump is the first real trip of the widened guard: it ships the `_shared`/`backend` code the previous, blind gate let through unversioned.

**`project_seed` reported success while rows genuinely failed, and `yadgar backfill --reslug-adr-pages --apply` gave zero signal on a collision — both found in a full correctness sweep of `backend/admin_exec` + `core/server/tools` + `core/cli` (ledger task 13).** Core `5.187.0` → `5.188.0`; backend unchanged. **`project_seed` (defect 1, the worse of the two — this caused real harm):** `misc.py:868` returned `{"ok": True, ...}` with `ok` a LITERAL `True`, reflecting only whether the TSV map parsed, never per-row outcome; the CLI twin (`project.py:271`) printed a `FAIL:` line per bad row to stderr and always `return 0`. On 2026-08-20 an operator ran `project_seed`, got a success-shaped result, and reported 3 failures as "transient" — they were deterministic: the TSV `note` column feeds `payload["display_name"] = row["note"][:255]` (`project.py:199`, duplicated at `core/migrations/rekey_corpus.py:633`) against the registry's `display_name VARCHAR(64)` column (`_shared/storage/sql/migrations/versions/003_project_registry.py:58`), so any note over 64 chars raises `DataError 1406`, swallowed into a bare `failed:N`. It only surfaced because a retry with a shorter note happened to work. This compounds because `project_seed` exists to satisfy the project-registry FK check (ADR-0078) — an unregistered project silently rejects engine-2 writes LATER, after the operator was told the seed succeeded. **Fixed:** the CLI now exits 1 when `counts["failed"] > 0`; the MCP tool's `ok` is now `counts["failed"] == 0`. Duplicates stay classified `skipped` (not `failed`) so an idempotent re-run of an already-seeded map still reports success. No in-repo caller reads either surface's return value (grepped `project_seed(` and `cmd_project_seed`) — `ok`'s meaning changes for external MCP clients, which is why this bumps minor rather than patch. **The `display_name[:255]` mismatch is named here, not fixed:** the correct truncation width is unverified from what this sweep read (the column is `VARCHAR(64)`, but whether the fix belongs at 64, or lower to leave room for multi-byte truncation, wants its own look) — out of scope for this car. **Defect 2, `yadgar backfill --reslug-adr-pages`:** the `reslug` admin op (`backend/admin_exec/reslug.py:338,406`) returns a `collisions` list and carries NO `ok` key at all; `backfill.py:150`'s `_print_reslug_report` only PRINTS collisions to stderr and the branch `return 0`ed on both dry-run and `--apply`. A collision means a page was deliberately left un-reslugged — the occupant is never overwritten, so data is safe — but the operator got no signal, and a later step disagreeing had no traceable cause. **Fixed, `--apply` only:** a non-empty `collisions` list now exits 1. Dry-run stays exit 0 regardless of collisions, deliberately: an operator running a dry run is reading the report by definition (that's why they ran a preview instead of `--apply`), so gating it teaches the operator to stop trusting the exit code on the run that matters — the `--adr-rows` branch already established this reasoning for its own always-mismatched dry-run gate (`backfill.py:163-169`), though that case is structurally different (its gate signal is CONSTANT on a dry run — nothing was written, so it always disagrees, which carries no information; `collisions` is VARIABLE on a dry run — empty or not, informative either way). The real reason: `--apply` is the unread path — nobody scripts a dry run and ignores its stdout, but an unattended `--apply` in a script has nobody reading stderr, which is exactly the harm this defect caused. **Root cause of both defects: no test exercised the failure path against the exit code / `ok` field** — `test_car_a_project_seed.py` covered structural map errors (`SystemExit(2)`) and per-row classification (`created`/`skipped`/`failed`) but never asserted a genuine failure's effect on `cmd_project_seed`'s return value or `project_seed`'s `ok`; `test_cli_backfill_module.py` covered the `--adr-rows` branch's collision-shaped gates but had no reslug-collision test at all. Both suites now pin the failure path (non-zero / `ok: False`) alongside the pre-existing success path (zero / `ok: True`).
**`memory_update(fields={"content": …})` left `enriched_content` — and its `enrichment_queries` / `enrichment_concepts` / `enrichment_comet` / `enrichment_logic` / `enrichment_model_versions` siblings — describing the SUPERSEDED text after a content correction (task 94).** Backend `5.78.0` → `5.79.0`; core unchanged. Car 2 Part B's re-embed guard (`admin_exec/memory.py`) re-encoded the raw `embedding` when `content` changed but never touched the enrichment columns `_enrich_memory_if_enabled` writes at insert time — so a corrected memory stayed retrievable by the claim that was retracted, and the row's embedding (re-derived from the bare new content) DISAGREED with what `enriched_content` still said: pre-fix, a content patch silently downgraded an enrichment-derived embedding to a raw-content one while the enrichment text kept claiming otherwise. New `resync_enrichment_on_content_change` (plus the extracted `_derive_enrichment_resync` helper, split out to stay under the I30 cyclomatic cap) reuses the SAME producer `insert_memory` uses (`_get_enrichment_pipeline` — `settings` via `get_settings()`, `embeddings` via the op's own `_get_embeddings()`, both already reachable from the update path) to re-derive enrichment from the NEW content, restoring insert-path parity (`encode_document_enriched` wins over the bare raw vector). When re-derivation isn't possible (disabled / content too short / no embeddings engine / pipeline raises), the six columns are explicitly NULLed instead of left stale — stale-but-honest beats stale-and-wrong. Runs unconditionally on any real content change, not only when the raw re-embed itself happened to succeed, so a failed raw re-embed still gets the stale text nulled. **Correcting the record on what actually goes stale:** `enriched_content` carries no FULLTEXT index (that lives on `content` alone, `migrations.py`) and is unreferenced anywhere in `yadgar/backend/retrieval/`; its only functional consumer is `encode_document_enriched` at write time. The stale text resurfaces because raw memory dicts (`recall()` results, `memory_get`, `memory_update`'s own return value) echo it verbatim next to `content` — not because it drives ranking. **Not fixed here, same class of bug:** `sleep_compute/embed_compress.py`'s nightly compression and `write_exec/_memorize_phases/_phase_contradiction.py` both rewrite `content` via `update_memory_fields` directly, bypassing `memory_update` entirely; the structural fix belongs in `update_memory_fields`, which has neither `settings` nor an embeddings engine to call — a separate car, not a retrofit here.

**`memory.project_id` had no restamp path through any MCP surface — the corpus re-key could not touch the ~2,900 memory rows it was meant to fix.** Core `5.186.0` → `5.187.0`; backend unchanged. `_MEMORY_UPDATE_ALLOWED` (`core/server/tools/admin_other.py`) was `{content, tags, is_protected, is_stale, importance, tier}`, so `memory_update(id, fields={"project_id": …})` raised `ValueError` and nothing else could set the column either. That allowlist was the ONLY gate: storage already accepts `project_id` (`_MEMORY_UPDATABLE_FIELDS`, Car L, complete with the `project_id = NONE` literal branch for the `option<string>` column), and the backend `memory_update` op forwards `**fields` without re-validating. Meanwhile `build_project_scope_clause` narrows every memory read on `project_id = $p OR <reach-tag> IN tags` — so a row carrying a `project_id` derived from wherever the writing session happened to be was unreachable from every project-scoped read, permanently. This is the memory half of the `wiki_set_metadata` fix (ledger task 246), which **merged first** as `b96360fd` (PR #60) at core `5.186.0` — so the wiki-side symbols this entry names (`WikiStore._metadata_value_error`) ARE greppable on master; its own entry named this memory gap as "adjacent and NOT fixed here". Both branches had bumped to `5.186.0` from the same base, which git resolves as no conflict rather than as a collision — this half is the one that merged second and therefore carries the re-bump to `5.187.0`. **Value validation is SHAPE only, matching the wiki half:** a non-empty string that is not one of `_NON_IDENTIFYING_PROJECT_IDS` (`global` / `unresolved` / `system` / empty) — the manufactured identities ADR-0227 deletes; global reach travels as the Car C7 tag, never as `project_id='global'`. `None` and `""` are rejected rather than treated as a clear, because `update_memory_fields` writes the bare `NONE` literal for any falsy value, which does not fail loudly — it NULLS the column and reproduces exactly the unreachability the restamp exists to repair. **No registry check runs, and the reasoning is stronger here than it was for wiki:** `_ensure_project_exists_sync` (ADR-0078) has ZERO production call sites — definition, `__all__`, and docstrings that claim it enforces something it never runs for — while the memory CREATE path (`resolve_effective_project`) validates non-empty-string and nothing else, so it would happily stamp `'global'` on a NEW row. The correction gate is therefore stricter than the creation gate, which is the opposite of the asymmetry task 246 argued against; a real registry check belongs on the create path first. The two-condition rule is DUPLICATED rather than imported from `WikiStore`'s `_metadata_value_error` (and will stay duplicated once that branch lands): that helper dispatches on wiki field names and lives in the module defining `WikiStore`, so sharing the function would drag the wiki store into the memory admin import graph and let a wiki-side dispatch edit silently change memory validation. What IS shared is the authority — both halves read the one `_NON_IDENTIFYING_PROJECT_IDS` frozenset, so the sentinel set cannot drift. **Two stale field lists corrected while here:** `memory_update`'s docstring had named four allowed keys since v5.158 (it never learned about `importance` / `tier`), and `CAPABILITY_REGISTRY.md` carried the same four in two places. The rejection message itself was already accurate — it renders `sorted(_MEMORY_UPDATE_ALLOWED)` — and a test now pins the render and the allowlist together so the message cannot go stale the way the prose did. Worth knowing for anyone reading the allowlist as a hard boundary: it is not one. `de_anchor` calls `_forward_admin("memory_update", …)` directly and bypasses it entirely (pre-existing, unchanged), which is the same fact that makes core the sole gate.
**A mis-stamped `project_id` on a wiki page had no correction path — the only tool that could edit page metadata accepted the retired key alone, and told callers so.** Core `5.185.0` → `5.186.0`; backend unchanged. ADR-0233 made `project_id` the sole scoping key and retired `directory` the way ADR-0215 retired `branch`, but `wiki_set_metadata` — the one all-rows metadata writer — kept `_METADATA_FIELDS = {"directory_context"}`. `wiki_add` with `replace_slug`, `force` or `upsert` all update the row **without** restamping `project_id` (measured live on `wiki_page:7710`, 2026-08-19), so a page stamped with the wrong owner was unreachable from every project-scoped read and delete-and-recreate was the only remaining move — a session that tried the re-store instead hit the v5.39 similarity gate and lost content. The rejection message made it worse rather than merely unhelpful: `invalid field 'project_id' — allowed: ['directory_context']` actively steers a caller asking for the current scoping key onto the concept ADR-0233 deleted. `project_id` is now settable and reaches **every** row sharing the slug, `directory_context` stays settable as retained legacy, and the message names both and marks which is which. **No registry check runs here, deliberately.** `_ensure_project_exists_sync` (ADR-0078) is reachable from the backend admin path, but no writer of `wiki_page.project_id` performs one: the create path (`_resolve_project_id_for_write`) takes the caller's value unvalidated, and `assert_project_registered` fires only on the engine-#2 ledger tables (`task` / `adr`) — `wiki_page` is a SurrealDB row with no FK to `project`. Gating the correction more tightly than the creation would also make a SurrealDB-only write hard-fail with `ProjectRegistryUnavailableError` wherever engine #2 is not composed, including the in-process test bypass. What IS validated: non-empty string, and not one of the manufactured identities ADR-0227 deletes (`global` / `unresolved` / `system`) — global reach travels as the Car C7 tag, never as a `project_id` — plus `None`, which would null the column and reproduce the unreachability the fix exists to repair. The two validators (`set_metadata` per row, `set_metadata_by_slug` up front) were verbatim copies and are now one helper, because a divergence between them has a silent shape: the all-rows loop counts only rows whose write reported `changed`, so a value the front gate admits and the per-row gate rejects returns `{ok: True, rows_updated: 0}` — success-shaped, zero writes. **Locked ADR pages ARE restampable, and that is now pinned rather than incidental.** `set_wiki_page_metadata` issues its own `UPDATE type::record('wiki_page', $pid)` and never reaches Car J's `enforce_mutability`, which guards only `insert_wiki_page` / `update_wiki_page` / `delete_wiki_page` — so a `page_type='adr'` row (effective mutability `locked`) accepts the restamp. That is the question deciding whether this fix unblocks the re-key at all, since task 41's cohort is mostly ADR pages, so it is a test rather than a footnote. A second test pins the MCP field gate against `WikiStore._METADATA_FIELDS`: the shell's accepted set is hand-written (the store's `sorted(frozenset)` cannot say which member is current and which is legacy), and two hand-maintained copies drift — a third field added to the frozenset alone would be silently rejected at the MCP boundary by a message that never mentions it. Adjacent and NOT fixed here: `memory_update`'s allowlist is `content, tags, is_protected, is_stale, importance, tier`, so `memory.project_id` still has no restamp path at all.
**The vacuum e2e suite could not run against a `docker:dind` sidecar, in four independent places (task 228, follow-up to ADR-0336).** Core `5.185.0` → `5.185.1`. `test-engine2-integration` reaches its daemon over `DOCKER_HOST=tcp://dind:2375`, so every port and every bind-mount path handed to that daemon is resolved in the SIDECAR's namespaces, not the job's. `7d88c8eb` fixed layer 1 (the backend container's published PORT) and left three: the backend container's BIND MOUNT, the phase-3 side-build container's mount, and the phase-3 side-build's PORT. The mount layers were the visible failure — `-v {data_dir}:/data` created an empty dir inside the sidecar, the backend wrote its whole DB there, and the vacuum's own preflight then reported `DB dir not found` against a path that existed and was empty. **Fixed by making the data dir namespace-INVARIANT rather than by proxying it**: `docker cp` is not an option, because `cmd_vacuum_impl` does `os.rename`/`copytree`/`rmtree` in-process and its crash-mid-swap recovery rests on POSIX rename atomicity, so a proxied filesystem would leave that invariant untested while looking tested. The integration fixture takes a new `YADGAR_TEST_SHARED_DATA_ROOT`; CI points it under the job workspace, which on this runner is bind-mounted at one absolute path by both the runner container and its sidecar — so one directory, one name, both sides. That closes the side-build container's mount for free, since it mounts the very same dir. Unset (ordinary local dev) the fixture falls back to `tmp_path_factory` unchanged; when set, each fixture instantiation gets its own `mkdtemp` subdir and removes it on teardown, because both tests assert on the ABSENCE of `surreal_db.old-*` / `.new-*` / `*.tmp` and a runner's workspace survives the job. **Layer 4 needed production code**: `launcher.py` published the side build on `127.0.0.1` and `vacuum/__init__.py` built `side_url` from a second, independent `127.0.0.1` literal — two hardcodes that agreed only by coincidence and stopped agreeing the moment the runtime moved namespaces. Both now resolve one `YADGAR_VACUUM_SIDE_HOST` knob (`launcher.side_build_host` / `side_publish_spec`), defaulting to `127.0.0.1` so production is byte-for-byte unchanged; a non-loopback value publishes on the daemon's interfaces instead of a loopback the caller cannot reach. The knob is deliberately explicit rather than derived from `DOCKER_HOST` — a throwaway SurrealDB holding a full copy of the corpus must not start binding wider because a host happens to point at a TCP daemon. **A fifth layer, previously unnamed:** the side build resolves its own image via `DOCKERHUB_BACKEND_IMAGE`, i.e. `server.json`'s PENDING `backend_version`, which on any PR that bumps it is unpublished and therefore absent from the local store — `backend_image_present()` answered no and the vacuum SKIPped its side build before doing anything. CI now exports the tag it already resolved as pullable to `YADGAR_BACKEND_IMAGE` too. Both workflow files carry an identical new step that PROVES workspace/daemon path-sharing with a marker round-trip before relying on it, so a change to infrastructure this repo does not own fails by name instead of resurfacing as `DB dir not found`.

**A whole family of wiki writes and reads addressed the wrong page, and the ADR ledger could not answer what supersedes what.** Core `5.183.9` → `5.184.0`; backend `5.76.4` → `5.77.0`. Eight cars, one minor bump. The unifying defect is **identity**: `project_id` became the sole scoping key under ADR-0233, and several paths never got the memo — they still keyed on `directory`, or on a slug derived from a title, or on nothing at all.

**`wiki_add(append=True)` wrote to a page the caller never named.** The append branch (`backend/write_exec/wiki_add_impl.py`) called `WikiStore.ingest`, which has no `slug` parameter and derives one from the TITLE — so an explicitly-slugged append landed on the title-derived slug instead, creating a shadow page there (with `directory_context: "global"` and the default `category`) when none existed. The response echoed back the slug the caller PASSED, so re-reading the response confirmed nothing; only re-reading the page caught it. Invisible whenever `slug == slugify(title)`, which is why it read as intermittent — every project-prefixed slug (the whole post-ADR-0211 scheme) diverges from its title and was always affected. `ingest` now takes a `WikiAddOptions` bundle rather than six more parameters, which would have breached I13's `params_hard=8`. The merge branch deliberately does NOT carry `directory_context`/`category`/`page_type`/`confidence`: `category` arrives as a non-None `"reference"` default on every drainer payload, so forwarding it would downgrade a `decision` page on every append — the create branch is the only one that manufactures a `global`-scoped row, and it now forwards all four. Detection signature for auditing an affected corpus: the shadow slug is truncated mid-word at 63 chars with a trailing bare hyphen.

**`wiki_read` and the entire section-patch family resolved on `directory` while accepting `project=`.** `project=` was validated and folded into the cache key, then never reached the query. Measured: the same slug with a correct `project=` resolved when `directory=` was omitted and returned not-found when it was supplied — adding a correct scoping argument made the read fail. Fixed in `wiki_read` via a deliberate TWO-query ladder (own-project, then the `global` reach tag), not `build_project_scope_clause`'s single `OR` — under `LIMIT 1` that picks arbitrarily between an own-project page and a global-reach page sharing a slug, breaking §25's own-project-first precedence. The same defect sat one layer over in `_resolve_page_id_by_slug`, feeding 14 call sites: every section-patch and versioning tool plus `adr_render`. That family is now project-keyed too, but deliberately NOT fail-loud — four browser endpoints call those tools positionally with a slug and nothing else, so strictness would take the viz version-history/diff/restore surfaces offline. The no-directory WARNING is removed: post-re-key it was factually false, and under ADR-0233 it was instructing callers to add the argument that causes the defect. **`project_id` and `directory_context` are not 1:1** — `quinyx/qwfm` owns rows at 6 distinct directories, and ~57 rows corpus-wide sit at subdirectory paths the exact-match ladder could never reach from their own project root.

**No canonical slug could ever get a correct `links` array, and the crossref table was corrupted the same way.** `_extract_wikilinks` ran bracket bodies through the title slugifier, so `[[m-agahi_yadgar_adr-0253]]` yielded `m-agahi-yadgar-adr-0253` — underscores hyphenated, matching no page. Both `wiki_page.links` AND `wiki_crossref` are fed from that one list, so the table is not an independent cross-check on the array; the earlier claim that it stayed correct is false, verified on the live corpus. Worse, `check_invariants` auto-DELETEs crossref rows whose `to_slug` names no page, so every corrupted canonical crossref was being silently harvested as "dangling" on each run — which is why those queries returned zero rows rather than wrong ones. Separately, the surgical edit tools (`replace_text`, `insert_after/before/at`, `delete_text/at`, `replace_markdown_block`, `append_section`) rewrote content without re-deriving links at all. Both fixed, with the 64-char cap retained on the verbatim path — exempting it would have broken 10 pages carrying slug-shaped link bodies of 65+ chars and 11 crossref rows that resolve at the truncated form.

**`adr_get` served another project's metadata, and nothing had ever read the supersede relation.** `get_adr_row` discarded `project_id` while `adr.id` is one global AUTO_INCREMENT that **interleaves** projects — not two contiguous slices, as previously documented. Confirmed live: `adr_get(adr_id="ADR-0002", project="m-agahi/yadgar")` returned a different project's row. Now scoped at both layers, and the admin op REFUSES without `project_id` rather than degrading to a corpus-wide lookup. On supersedes: **there is no `supersedes` or `superseded_by` column** — the relation lives only in the `adr_supersedes` join table, and the `"none"`/`"-"` everyone had been reading were the mapper's placeholders for fields that were never stored. `add_adr_supersedes` runs the join INSERT before the target status flip in one try block, so a flip observed in 22/22 cases across two corpora proves the INSERT succeeded: the writes worked and nothing ever read them back. A scoped `list_adr_supersedes` now joins both edge ends against `adr.project_id`, and `adr_get` emits `superseded_by` for the first time. `adr_add` also left `tier` NULL, making rows unreachable through every `adr_list` argument value; it is now derived from `status` per D27 — not a blanket `"binding"`, which would put superseded/rejected/deprecated rows into exactly the default list D27 excludes them from. `_flip_adr_status` never re-derived `tier` at all, so `adr_list(tier="historical")` returned zero rows against 20 historical-status rows; its replacement rule is **three-way** (historical / binding / leave-alone), because that function's other caller is the nightly sweep's `archived` transition and a two-way rule would re-tier the entire archived cohort to `binding` on a cron.

**Three guards that could not do what their names promised.** `check_ledger_chokepoint` was blind to aliased `sqlalchemy` imports and its allowlist was keyed on absolute paths and never wired into the hook — repairing it exposed two genuine D20 violations. `check_observe_coverage` reported OK on trees the pre-commit hook then rejected, because pre-commit STASHES the working tree and runs hooks against the git INDEX while a manual run reads the disk; it now reads the index via `git show :<rel>`, with a `git diff --name-only` fast path so unmodified files skip the per-file shell-out. Its live smoke test also had one `.parent` too many and had been AST-walking `.venv`, `.venv-test` and `.claude/worktrees` — correct-looking only because this checkout shares the package's name — which is why it hit a 300 s timeout. `test_check_staged_parses` now enumerates via `git ls-files` instead of `rglob` plus a hand-maintained skip set that listed `.venv` but not `.venv-test`.

**The weekly branch sweep had never deleted a single branch.** Its classifier was `git diff master..branch` — two-dot, which `git diff` treats as a full tree comparison, so any branch whose base is older than master reported unmerged purely because master advanced: `deleted=0 unmerged=170`. Three-dot, `merge-tree --write-tree`, `is-ancestor` and patch-id all fail too, because this repo's topology is multi-level — car branches merge via real `git merge` into a train-integration branch, and only that gets squash-merged. The working classifier asks the forge for PR state and then closes the transitive ancestor set, degrading loudly (never silently) when `gh` is unavailable. Two guards stop the failure mode found in review, where the first cut would have force-removed the worktrees of agents running at that moment: a zero-commit branch is classified `empty`, not `merged` (a fresh worktree branch is trivially an ancestor of master), and a branch with any attached worktree is never deleted regardless of classification. Worktree removal is now exclusively an age-gated sweep that always retains the branch — `.git` is 83 MB against 206 GB of worktrees, so the working directories are the cost and the branches are not.

**The vacuum e2e suite had never run, and hid three defects in series.** Wired into CI for the first time, its skip-is-not-a-pass gate went red three times, each revealing the next only after the previous was fixed: the container tag derives from `server.json`, but images publish at RELEASE, so on any version-bumping PR it cannot exist; then the auth probe posted `SELECT 1;`, invalid SurrealQL that answers 400 whatever the credentials, so a 60 s wait could never succeed; then the health probe still timed out because the CI runner reaches a `docker:dind` sidecar, and a port published as `127.0.0.1` lands in dind's namespace, unreachable from the runner's. The fixture now resolves a pullable image with a loud fallback, probes the daemon's actual host, and **fails rather than skips** when a started container never becomes healthy — a skip made three distinct causes indistinguishable from "no docker on this host".


**`recall(tags=[...])` filtered wiki results only — memory rows came back unfiltered, and the tag token was scored as ordinary text.** Core `5.183.8` → `5.183.9`; backend `5.76.3` → `5.76.4`. `_build_provider_tasks` (`backend/retrieval/recall_pipeline.py`) hands the caller's tag list to the `WikiProvider` constructor and builds `MemoryProvider` without it; the `Scope` carried `opt_in_tags` all along, but `MemoryProvider.candidates` never read the field. Under `type="memory"` the wiki provider is not even constructed, so the filter was **wholly inert** on that path. The failure was invisible because the tag string still reached the FTS and semantic scorers as ordinary query text — a plausible-looking result set came back, so nothing looked broken. Measured live 2026-08-19 against `m-agahi/yadgar` with `project_brief` reporting 97 project anchors: the anchor-audit gather (`anchor_audit_prompt.md` step 1) `recall("_audit_anchors sentinel anchor hygiene", tags=["_anchor"], max_results=25, type="memory")` returned **5 rows, 2 of which carry no `_anchor` tag at all** — both tagged `["semantic", "auto-abstracted"]`, matched because their auto-abstracted CONTENT contains the literal substring `[tags: _anchor]`. Every anchor-hygiene pass ever run was therefore a spot check over arbitrary rows. Fixed as a **row guard in `MemoryProvider`**, beside the existing `is_project_eligible` guard and for the same stated reason: the Car C7 stage-1 scope clause reaches the FTS and vector arms only, while the PPR graph walk and spreading activation hand back memory ids that clause never saw, so a SQL-side tag arm could add depth but could **not** replace this check — and it would move every tag-filtered recall onto the brute-force cosine path that `search_vectors` takes whenever a predicate is present. The filter is **conjunctive** by construction (every requested tag must be on the row) and says so, since a later reader is otherwise free to assume any-of and the two disagree the moment a caller passes two tags; a row carrying no `tags` at all satisfies no non-empty filter. **Precision is what improves; coverage does not, and the yield goes DOWN.** The guard removes non-matching rows from the pool the query already ranked — it does not refill it — so the live gather that returned 5 rows now returns the 3 real anchors among them. That is the honest trade and it is stated here rather than left to be discovered: a tag-filtered recall ranks the candidate pool the query produced, it does not scan a tag index, so `tags=["_anchor"]` returns the best-matching anchors and **no `max_results` makes it enumerate all 97**. Two consequences for `anchor_audit_prompt.md`, reported and deliberately NOT patched here (the fix belongs in the retrieval path, not the prompt): its step-2 empty-list gate is now MORE likely to fire, and an empty list means "no anchor ranked into this query's pool", not "no anchors need attention" — noise used to fill those slots and mask it; and "review the RETIRE candidates" reads as exhaustive over a set the gather never had, a query-dependent ~3-5% sample. The `recall` docstring now points at `audit_anchors` (which gathers with its own `'_anchor' INSIDE tags` SurrealQL predicate — a real enumeration, not a recall) and `db_inspect` for the enumerating case. The regression tests assert **both directions in one call** — the genuinely tagged row IS returned and the content-only match is NOT — because an absence-only assertion passes just as well on an empty result set, this repo's recurring false-green shape; the untagged-scope control pins that general recall still filters nothing. **The same change made a dormant divergence live, so it is closed in the same commit range rather than filed.** The S6 kill-gate (`AGENT_PROMPT_LIBRARY_ENABLED=False` → the agent-prompt library is inert) ran INSIDE `_build_provider_tasks`, where it rebound only that function's LOCAL `tags` — by then `_fanout_recall` had already stamped `Scope(opt_in_tags=tags)` with the UNSTRIPPED list. Harmless for as long as the memory arm ignored `opt_in_tags`; the moment `MemoryProvider` started reading it, flag-off recall handed `tags=None` to `WikiProvider` and `["agent-prompt"]` to the memory guard — two arms, two different answers, from one caller's request. "Benign because the data happens to be empty" (agent-prompt-tagged *memories* are ~nonexistent) is the precise shape that stops being benign the day someone tags a memory `agent-prompt`. The strip is now `_apply_library_kill_gate`, called ONCE in `_fanout_recall` **above** the `Scope` construction, so the value is stripped before it is copied anywhere and a second divergence is unrepresentable rather than merely absent; `_build_provider_tasks` does not re-apply it (a second copy is a second thing to drift) and its cyclomatic drops 11 → 10. The kill-gate itself is unweakened — the regression test pins BOTH arms in one flag-off call: `WikiProvider` constructed with `tags=None` and the default agent-prompt exclude back on, AND a row *not* tagged `agent-prompt` still returned by the memory arm. Mutation-checked by moving the strip back below the `Scope` construction: the memory-arm assertion fails while both wiki assertions still pass — the exact old shape.

**The ADR corpus could not be created, edited or deleted, and the documented way out returned a bare 400.** Core `5.183.7` → `5.183.8`; backend `5.76.2` → `5.76.3`. Car J's `enforce_mutability` guards three storage ops — `insert_wiki_page` (`_shared/storage/wiki.py:100`), `update_wiki_page` (`:221`) and `delete_wiki_page` (`:462`) — and `page_type='adr'` resolves to effective mutability `locked`, so every unsanctioned caller is refused on all three. The gate's own `PermissionError` names the remedy: *"or use `wiki_set_mutability` to override."* That tool was **never registered**. Its implementation exists (`backend/admin_exec/wiki.py:168`), its MCP tool exists and forwards to it (`core/server/tools/wiki.py:1416`), and `_ADMIN_OPS` (`backend/admin_exec/__init__.py`) lists fourteen `wiki_*` ops with `wiki_set_metadata` among them — but not this one, so `run_admin_op` raised `KeyError("unknown admin op")` and the route mapped it to **HTTP 400 with no logged reason**. A shipped, exported, documented, power-gated tool, dead on arrival, and the failure shape made it look merely broken rather than unwired. Verified live 2026-08-18 against core `5.183.7` / backend `5.76.2`: `wiki_add(page_type='adr')` returned `wait_timeout` and the drainer logged `forbids insert_wiki_page (page_id=None slug=None page_type='adr')` — `page_id=None` proving a **fresh insert**, not an update; the identical call without `page_type` committed instantly, isolating the cause. This is wider than the previously filed symptom, which recorded only delete and update: **creation was blocked too**, which is why `yadgar backfill --reslug-adr-pages --apply` has never been runnable. `adr_add` was unaffected throughout and the corpus kept growing, because it writes through `_write_adr_body_page`'s `_sanctioned=True` canonical path rather than `wiki_add` — which is precisely why the hole survived: the everyday writer never touches the gate. **The test is the class kill, not the instance.** `test_admin_op_dispatch_shapes.py` already asked "is every *registered* op callable?"; it now also asks "is every op the core *forwards* registered?", resolving `_forward_admin` targets from source (string literals plus module-level `_NAME_OP = "op"` constants, the two shapes that occur) and asserting each against the live table. Derived empirically so a tool added later with no registration fails there instead of in production, and guarded against its own rot by asserting the scan resolves a plausible op count rather than silently zero. Run across the tree it finds **71 forwarded ops and exactly one gap** — confirming no second dead tool was hiding behind the same shape.

**The ADR backfill could not run at all, and fixing only the obvious half would have misnumbered 221 ADRs permanently.** Core `5.183.4` → `5.183.5`; backend `5.75.3` → `5.76.0`. `seed_adr_rows` is the one-shot op that lifts 230 historical ADR wiki pages into the `adr` ledger, where `adr.id` is `AUTO_INCREMENT` and **is** the ADR number (ADR-0197) — it never moves backwards, and `TRUNCATE` is FK-blocked by `adr_supersedes`. Four defects, none independently shippable. **(1) It reached the wrong storage engine.** The op resolved `_get_storage()` (SurrealDB) and then called `create_adr_row` / `list_adr_rows` / `set_adr_body_slug` / `add_adr_supersedes` on it; the SurrealDB engine has zero ADR methods. Because the op is registered under `_kwargs_op` (`fn(**payload)`, which never injects `storage=`), `storage` was **always** `None` in production and always resolved wrong — every `AttributeError` was swallowed by a blanket `except Exception` and counted as a skip, so the visible outcome was `rows_inserted=0, rows_skipped=236`: indistinguishable from "already backfilled". The op had never once written a row. `storage` stays the wiki handle and `sql_storage` is added alongside; `retype_page_type` is untouched, since it makes only wiki calls and always resolved correctly. **(2) ADR-0006's mandated skip had no mechanism at any layer** — bare `store_true` on the CLI, a two-key payload, no parameter on the op. The governing ADR was accepted and never implemented. `--skip-adr` (repeatable, comma-separated, `7`/`0007`/`ADR-0007`) forwards `skip_adr_numbers`, filtered after the ascending sort and keyed on the ADR **number** so both slug shapes of one ADR collapse to a single decision. **(3) The dedup could never bind, twice over.** The row lookup was keyed on a project id regex-parsed out of the page slug (`yadgar-adr-0001` → `yadgar`) while rows live under `m-agahi/yadgar`, so `list_adr_rows` returned `[]` for every slug shape; and the comparison was exact string equality against the page's *current* slug, so a row stamped with a canonical `body_slug` never matched its legacy-slugged page — the shape all six existing rows have. One change fixes both: read the ledger **once** before the loop and reduce it to a set of ADR numbers. That reduction *is* the normalisation, symmetric by construction, and it closes the inverse hole (a number owning both slug shapes was inserted twice) while dropping 230 round trips to one. **(4) `rows_skipped` conflated three outcomes** — "already had a row", "the insert raised", "the insert returned no id" — split into `rows_already_present` / `rows_failed` / `rows_skipped_by_request`. The insert's blanket `except` is narrowed: `AttributeError`/`TypeError` means the *surface* is wrong and will hit every remaining page identically, so the loop stops and returns `{"ok": False}` with the partial counts and a resume point rather than 230 identical faults. `--adr-rows` now defaults to **dry-run** like the reslug branch beside it (the module's own docstring already claimed "no flag combination applies by accident" — false for exactly the irreversible branch) and returns the planned ADR-number → id mapping, with the base id read from `information_schema.TABLES.AUTO_INCREMENT`: ADR-0006 measured that deleting rows leaves the counter *ahead* of the highest surviving id, so `max(id)+1` under-predicts in precisely this table's state, and the preview a human gates an unrepairable write on must not be able to lie. `next_id_basis` is returned so a fallback read is visible rather than silent. **No test had ever exercised a real `AUTO_INCREMENT` id**: every existing test injects a `row_inserter` which per the op's own idempotency branch bypasses both the dedup and the real `create_adr_row`, handing back a synthetic `{"id": len(inserted_order)}` — and the ordering fixture asserted `yadgar-adr-0001` was inserted *first*, encoding a one-spent-id world and thereby demanding the misnumbering. New `yadgar/tests/integration/test_adr_seed_ledger_ids.py` composes both engines (containerised MariaDB + embedded SurrealDB) over the real 236-page census and asserts `id == number(body_slug)` across the whole range, the dedup in both directions plus its negative case, and that the dry-run mapping equals what the apply produces. `docs/prompts/backfill-adrs-to-ledger.md` was stale twice — Step 0's "the next insert gets `MAX(id) + 1`, always" and Step 4's idempotency claim — and now also states that `gate["exact_match"]` is *expected* false on this corpus (230 index rows vs 236 pages vs 227 ledger rows cannot reconcile when six ADRs own two pages each), so a correct run exits 1 and a human must not abort on it.

**`task_list` threw on its own default, and its regression test asserted the defect as a requirement.** Core `5.183.3` → `5.183.4`; backend `5.75.2` → `5.75.3`. `list_task_rows` wrote `" AND status IN (:status)"` and then bound `:status` with SQLAlchemy's `bindparam(..., expanding=True)`. The expanding bindparam renders its OWN parenthesised placeholder list at execution, so MariaDB received `status IN ((%s, %s))` — and `(a, b)` is a ROW CONSTRUCTOR there, making the clause `status = ROW('pending','in_progress')`: `(4078, "Illegal parameter data types varchar and row for operation '='")`. A single status was unaffected (`(x)` ≡ `x`), so the one shape that always failed was D37's open-only default — every bare `task_list(project_id=...)`. `_task_list_restore_nudge` swallowed the exception to `rows=[]`, so session start reported zero open tasks instead of an error. Same defect and fix at `list_task_rows_all_projects`. `test_task_list_bindparam.py` asserted `"status IN (:status)" in src` — the buggy literal, pinned as the contract, via source string-matching that never compiled or executed anything; those assertions are deleted rather than weakened. Their replacement executes the statement with two statuses against a real MariaDB (`yadgar/tests/integration/test_task_list_status_filter.py`) — a compiled-SQL assertion would not have sufficed, since `expanding=True` defers rendering the placeholder list until bind values arrive. Note the new test is `-m integration`, which the default addopts exclude from CI until the skip-gate's DESELECTED blindness is fixed.

**`task_list` returns `id`, `title` and `status` by default — 66% fewer characters.** Measured 2026-08-16 on the live corpus: 81 open rows render **26,242 chars** through the fixed 11-column `TASK_COLUMNS` (324 chars/row), against **8,900** for the three columns a listing caller actually reads (110/row) — a **66.1%** reduction. `project_id` is constant across a project-scoped result, and no consumer of a list result reads `state`, `active_form`, `plan_path`, `body_slug` or any of the three timestamps. `verbose=True` restores the full 11-column shape for callers that need it (chiefly the session-end catch-up sync and its `updated_at` staleness guard). **Row count is untouched** — every open task is still returned; only the width changes. `task_get` is unchanged: the single-row read stays full. Below the MCP tool the defaults run the OTHER way — storage (`list_task_rows` / `list_task_rows_all_projects`) and the admin op both default to the FULL projection, because `nightly_sweep` calls them with no flag and then reads `body_slug`, `completed_at` and `project_id` off those rows; a lean default there would leave the sweep archiving nothing and reporting success. The lean shape is a decision the `task_list` tool makes explicitly on every call, never one a silent caller inherits. `TASK_COLUMNS` itself is untouched — the lean shape is a second constant, `TASK_COLUMNS_SUMMARY`. The docstring also stops implying `limit=100` is a cap: no reader below the tool emits a `LIMIT` clause and `limit=5` returns every matching row.

**A re-slugged corpus would seed zero ADR ledger rows and report success.** Core `5.183.2` → `5.183.3`; backend `5.75.1` → `5.75.2`. `_is_per_adr_page_slug` / `_parse_adr_id_from_slug` (`yadgar/backend/admin_exec/adr_seed.py`) matched only the legacy hyphen separator (`r"-adr-\d{4}$"`), but the canonical post-reslug slug (D32 ③, `{project_id}_adr-NNNN`) separates with an underscore — `m-agahi_yadgar_adr-0001` never matched. The operator procedure is re-slug first, then backfill: after a `reslug` run every page is canonical, so `seed_adr_rows`'s candidate filter would match nothing, insert zero rows, and return a normal-looking success dict (the D35c gate only reports a mismatch, it never raises). Both helpers now accept either separator (`r"[-_]adr-\d{4}$"`); the `-adr-log`/`-adr-index` exclusions were widened to their `_adr-log`/`_adr-index` canonical forms for the same reason. Also: neither `seed_adr_rows` nor `reslug` had any operator surface — both are `_ADMIN_OPS` reachable only via `POST /admin`, with no MCP tool and no CLI subcommand. Added `yadgar backfill --reslug-adr-pages [--apply] | --adr-rows [directory] [--project ID]`, a thin forwarder over `yadgar.core.forward._forward_admin` (the same direct-to-backend pattern `drain`/`restore` already use — no new core-daemon HTTP route needed). Reslug stays dry-run by default; `--apply` is required to write. The seed mode exits non-zero when `gate["exact_match"]` is false or any row is `flagged`, since the op itself never raises on either condition.

**`seed_task_from_pages` deleted — it duplicated the corpus on every re-run, not just the first.** Backend `5.75.0` → `5.75.1`; core unchanged (`5.183.2`). The admin op backfilled the MariaDB `task` ledger from `{project}-task-list` wiki pages and documented itself as idempotent (D35a) on both the backend docstring and the CLI's `--seed-task-from-pages` help text, but dedup rode `UNIQUE(project_id, body_slug)` while inserting `body_slug=NULL` — and MySQL permits unlimited `NULL`s in a unique index, so a second run re-inserted every row instead of skipping it. It also silently dropped `description`, `context`, `blockedBy`, `blocks`, and the source task id — most of a task's content. Removed entirely: `yadgar/backend/admin_exec/seed.py` (`seed_task_from_pages`, `_parse_task_section`, `_list_task_list_pages`, the `_TASK_SEED_RE`/`_TASK_FIELD_RE` patterns), its `_ADMIN_OPS` registry entry and `_ADMIN_OPS_KWARG_SHAPED` adapter entry, the CLI's `--seed-task-from-pages`/`--project-id` flags and handler (`yadgar/core/cli/seed.py`), and the `/hooks/seed-task-from-pages` HTTP route (`yadgar/core/server/http.py`). Replaced by a human-run procedure: `docs/prompts/backfill-task-list-to-ledger.md`.
...
**Tier 2 was dead in the only transport the daemon runs.** `resolve_effective_project`'s session tier reads a project the `SessionBindMiddleware` stamps — but `_startup.py` sets `stateless_http=True` for streamable-http (deliberately: it makes daemon restarts transparent to the client), and stateless mode issues **no `Mcp-Session-Id` at all**. Verified live 2026-08-15: an `initialize` against the running daemon returns no such header. Both readers gated on that id before looking at anything else, so the `X-Yadgar-Project-Id` header they each already knew how to read was unreachable, tier 2 never resolved, and every wiki/ADR write without an explicit `project=` raised `unresolved_project`. A session id is the wrong precondition for a value that travels in its own header: the gate is removed in `_extract_session_project`, and `SessionBindMiddleware` now falls back to the header when the nonce lookup yields nothing. A static per-client header survives statelessness AND restarts, carries no sticky state an instance can forget to switch back, and lives in the client config rather than anywhere the model can rewrite mid-session. Both client shapes are supported. A **per-project `mcpServers` entry** carries a static `X-Yadgar-Project-Id` header, exactly the way it already carries `Authorization` — the middleware stamps it and tier 2 reads it. A **single global entry** (the common setup) makes every request identical on the wire, so there is nothing to key a header on; those sessions resolve through a new hook-authored `directory -> project_id` table (`_shared/runtime/session_map.py`, written atomically into the data dir both containers already mount). The SessionStart hook — the only process that can mint truthfully — registers the pair; the daemon only ever asks "has the hook told me about this exact directory?". **That is a lookup, not a derivation:** an unregistered directory returns nothing and the caller fails loud exactly as before, so no key is ever manufactured from a path and ADR-0227 stands. It also reintroduces no sticky state — resolution is per call, against a table an instance cannot write.
...
**The defect was the coupling, not the type error.** One UPDATE carried both `project_id = $pid` and the tag union, so a single string-tagged row rejected the whole statement and every row in that `directory_context` got **neither** the tag **nor** its project_id. The stamp is now its own statement — an always-safe write is no longer coupled to a data-dependent one.

**The tag step itself is now a read-modify-write in Python**, not a set-based `array::union`. The first attempt kept the union and gated it on `type::is::array(tags)`; SurrealDB v3.1.5 answered that with **400 Bad Request** — a function this repo uses nowhere else, and one whose unit tests passed because they asserted the SQL string rather than that the database accepts it. That is the same mock-verified-vs-live-verified gap that produced the original defect, caught only by running it on the VM. The cohort's rows are now selected, their `tags` normalised (array, JSON string, or unparseable), the reach tag added when absent, and written back with `UPDATE type::record(...) SET tags = $tags` — both shapes every other storage module already issues. String-tagged rows are **repaired** rather than skipped, since skipping would silently narrow them to a single project, reversing the very decision the `global` sentinel split exists to honour (owner and reach are separate axes, §1.4). A value that will not parse is preserved as a single tag rather than dropped. Idempotent by construction; only the `global` cohort pays the per-row cost.
...
**The cross-project leaks (Car 3).** `CheckpointRestore.restore` is a seven-sink fan-out that never built a `RecallScope` and never called `build_project_scope_clause`; it computed `scope = project_id or ""` and threaded it into five hand-rolled storage calls. Two sinks took no scope at all: `checkpoint_restore.py:386` called `get_recent_memories(limit=)`, whose callee had **no project parameter to pass**, and the SR-prediction path read through `search_vectors` with no `scope_sql` plus a bare `SELECT * FROM memory:{id}`. Verified live on two versions — `restore(directory=/home/max/git/nix)` returned quinyx/application-gitops, qwfm db-copy and quinyx/ai memories. The MCP resources `memory://hot` and `memory://stale` read the corpus with zero scoping and now fail closed. **The scoping predicate is `directory_context`, deliberately not the `project_id` column**, because no backfill has run and column-scoping would strand the entire pre-C10f corpus.

**The hooks that degraded to silence (Cars 2, 8).** After C5 deleted directory derivation, `/hooks/prompt-recall` raised without `?project=` and returned an empty injection — auto-recall on every user prompt had been dead. Both callers now mint host-side via the existing `mint_project_id`. `/hooks/block-reflect` read only `directory` and dropped the identity its client sent; it now scopes on `project_id` and **degrades rather than raises**, unlike prompt-recall, because `_block_project_clause` is an exact-column match with no leak vector and PostToolUse fires per write. `core/hooks/prompt-recall.py` — a second, unwired implementation of the same hook — is deleted; two implementations with one wired is why this bug existed twice.

**The writes that lied (Cars 4, 6).** `task_write` called the backend and returned `{"ok": True, "id": result.get("id")}` **without ever inspecting `result["ok"]`**; thirteen writes reported success, persisted nothing, and left no DLQ trace. Backend success envelopes carry no `ok` key at all — only `{"ok": False}` signals rejection — so the correct test is `result.get("ok") is False`. `task_list`/`task_get` swallowed rejections as `[]`/`None` and now raise: widening their return type would have fed `{"ok": False, …}` into `_task_list_restore_nudge`'s unguarded `list(_rows or [])`, iterating the envelope's keys as fake rows and relocating the bug rather than fixing it.

**The vacuum button (Car 10).** `control.py` called `vacuum_now(force=False)`; under the 200 MiB threshold that returns `{"started": False, "skipped_reason": "db_below_threshold"}` **without writing the trigger sentinel**, the route wrapped it in a 200 regardless, and the frontend checked only `r.ok`. An interactive click is already confirmed twice, so it now forces; the UI surfaces `skipped_reason`. The trigger path unit moves `PathExists=` → `PathChanged=` (edge-trigger: a stale sentinel blocked re-firing) in both renderers this repo owns. Stale copy claiming vacuum "stops the daemon" is corrected — false since ADR-0188/#62, and precisely why "nothing visibly restarted" read as failure.

**The guard that reported OK (Car 7).** `pyproject.toml`'s `per-file-ignores` was **23 of 28 entries rot** — 5 naming files moved to `backend/retrieval` (ruff drops missing entries silently) and 18 vacuous; measured by running ruff with the table emptied, which returns exactly 5 violations at the repo's own thresholds. Six allowlist *reasons* were factually false, including `directory_residue_allowlist.txt:159`'s "checkpoint and memory_block have no project_id" — migration 033 gave both one, and the same file contradicts itself two lines earlier. That entry says "five sinks"; the file has nine. **No guard in the repo checks whether a stated reason is still true**, only whether its subject exists, which is how a pre-033 justification survived past 033. New `check_ruff_ignores_liveness.py` closes the per-file-ignores half. `.complexity-baseline.json`'s 48.8% staleness was measured and deliberately left: keys are line-anchored and a miss falls through to *full* enforcement, so stale entries fail stricter, never looser.

**Install and host-CLI (Cars 5, 9).** `entrypoint-backend.sh`'s phase-C GRANT heredoc is unquoted, so unescaped backticks in a SQL *comment* expanded `${MARIADB_DB}.*` and bash tried to **execute** it on every backend boot. `@observe`'s error path leaked bare function qualnames to stderr because `configure_logging()` runs only on the no-subcommand server path, leaving every CLI subcommand on `logging.lastResort`; fixed with a `NullHandler` on the package logger. `YADGAR_EMBED_URL` now defaults to the published host port — and, found in the same pass, `forward.py`'s four forwarders hand-rolled the auth token instead of using `resolve_auth_token()`, so that fix alone would have traded "URL not set" for a silent 401. Fourteen more call sites routed; `check_auth_token_pattern.py` added.

**fix: Car K1 — a staged-file parse guard, and a measured refutation of the "PEP 758 made the except-tuple guard obsolete" premise.** Core `5.181.53` → `5.181.55`; backend unchanged (`5.72.37`).

**This car set out to DELETE `test_v5_46_16_except_tuple_sweep.py` and strip 73 `# fmt: skip` except-pins, on the premise that PEP 758 made the bare `except A, B:` form safe on the repo's `>=3.14` floor. That premise is FALSE, the work was reverted in full, and the guard and every pin stay.** The refutation is recorded here because the premise is superficially airtight and will be proposed again.

**What is true:** PEP 758 does make the unparenthesized form the TUPLE form in Python 3.14. Verified on 3.14.6 — `ast.parse` yields `handler.type=Tuple, handler.name=None`; raising the first type is caught, raising the second is caught, an unrelated type propagates, and `except A, B as e:` remains a `SyntaxError`. `requires-python` is `>=3.14` in `pyproject.toml` and `uv.lock`, classifiers list 3.14 only, all three images are `python:3.14-slim*`, and CI runs inside `yadgar-ci` built `FROM python:3.14-slim`.

**What is nonetheless false:** none of that is the interpreter the guard protects. **`language: system` pre-commit hooks do not run the project venv's Python.** During a real `git commit` they run the interpreter baked into `.git/hooks/pre-commit` as `INSTALL_PYTHON`, which is **3.13** (pre-commit 4.5.1 is itself a 3.13 build). Staging a bare except-tuple and committing produces `SyntaxError: multiple exception types must be parenthesized` from `complexity_config.py`, `check_test_weakening.py`, `check_e2e_assertions.py` and others simultaneously. So `check_directory_residue.py`'s `_BARE_EXCEPT_TUPLE` arm and its "a SyntaxError on ... the interpreter pre-commit's `language: system` hooks run" error text were **correct as written**, and the 79 `# fmt: skip` pins are load-bearing rather than formatter-fighting noise.

**The trap that makes this worth writing down: `pre-commit run` and `git commit` do not check the same language.** `pre-commit run --hook <id> --files ...` invoked from an interactive shell resolves `python3` from the ambient PATH (3.14 here) and PASSES a bare except-tuple; the identical hook during `git commit` runs 3.13 and rejects it. A probe of the first kind was what produced the false "premise refuted" reading mid-car. Anyone re-testing this must test through an actual commit. **The split is a real repo-level defect** — a file can pass `pre-commit run --all-files` and fail `git commit` — and is left for its own change rather than fixed here; `INSTALL_PYTHON` and the nix pre-commit derivation are untouched.

**What actually shipped: `check-staged-parses`, the guard that was genuinely missing.** The defect this week was not a bare except-tuple; it was a commit containing a file that did not parse. Pre-commit rewrote a file, `git add -A` re-staged the rewrite, and `93f3ed95` landed unparseable (reverted in `8f826592`); CI caught it two hours later. Nearly every AST gate in `scripts/` reads sources behind a `except SyntaxError: continue`-shaped arm — correct for a lint that must still report on the other four hundred files, and exactly why all of them skipped the broken file at once and scored it clean. `check_directory_residue.find_unparseable` is the nearest relative and is deliberately not duplicated: it walks six fixed scan roots on `always_run`, so it never sees `yadgar/tests/**` or `hooks/**`. The new hook is keyed to the STAGED SET — whatever is being committed, wherever it lives — and is **ordered after `ruff`/`ruff-format`**, since the incident was a formatter rewrite and a parse check ordered before the formatter cannot see what gets committed. No `benchmarks/` exclude. Failure names file, line, column, the `SyntaxError` message and the offending source line.

**It is built so it cannot become the repo's thirteenth guard that passes while checking nothing.** Empty argv is an ERROR (pre-commit never invokes a `pass_filenames: true` hook with no matches, so no files means mis-wired and passing forever on nothing); an unreadable or undecodable file is a violation, never a skip; and the interpreter is printed on every run, pass or fail, so the 3.13/3.14 split above stops being invisible. The interpreter check uses a **sanity floor deliberately below `requires-python`** — an earlier draft compared against the project floor and hard-failed every commit (floor 3.14, hook 3.13), and the comparison was backwards besides: validating under a grammar looser than the commit-time one is the direction that produces false confidence. A test pins that distinction so the "fix" is not re-applied. Discrimination proven end-to-end through real pre-commit: a broken staged file fails with `SYNTAX ERROR: …:4:7: invalid syntax`, the same path passes once fixed.

**fix: Car H3 — migrate the e2e suite (and the H1 unit-test blast radius) onto the new scope guard.** Core `5.181.52` → `5.181.53`; backend `5.72.36` → `5.72.37`.

**Car H1 made `build_project_scope_clause`/`is_project_eligible` raise `UnresolvedProjectError` on a falsy `project_id` instead of silently reading the whole corpus, with an explicit `unscoped=True` opt-out for the deliberate whole-corpus case.** That broke two classes of tests: ones whose PREMISE was the old silent-noop contract (migrated to spell `unscoped=True`, or to pass a real `project_id`), and ones that were genuine production callers H1 missed reaching the clause builder with nothing.

**Real callers found and fixed.** `benchmarks/run_eval.py::evaluate_pair` (the legacy whole-corpus benchmark path) now passes `unscoped=True`. The BC-VZ2 viz god's-eye search and the "instructions-loaded" hook — both deliberately project-less forwards via `_HookRecallForwarder("")` — were dropping that intent at three hops (`Scope`/`RecallScope` conversion in `backend/retrieval/providers/base.py`, `_fanout_recall` in `backend/retrieval/recall_pipeline.py`, and `MemoryProvider.candidates`' `Retriever.recall()` call + its residual `is_project_eligible` row guard). Fixed by threading a new `RecallRequest.unscoped` field explicitly on the wire (`embed_service_models.py`, `_forward_to_backend`, `_forward_hook_recall`) rather than re-inferring it from a falsy `project_id` downstream — the same failure shape H1 closed, one layer up. Two test harness shims that stand in for the real `/recall` route (`yadgar/tests/_backend_harness.py::patch_recall_bypass`, `yadgar/tests/conftest.py::recall_backend_bypass`) needed the identical fix to stay faithful to production.

**H1's blast radius also broke 13 non-`e2e`-marked unit tests** (`test_recall_deadline.py` × 5, `test_recall_wiki_metrics.py` × 8) with the identical `UnresolvedProjectError` — invisible to `make e2e` since they aren't `-m e2e`, but the same defect class this car exists to close. Fixed alongside rather than left for CI to catch.

**Known pre-existing flakes, not introduced by this car (verified against the H1 baseline commit with changes stashed):** `test_recall_backend_contract_e2e.py::test_backend_bootstrap_serves_recall` and `test_recall_backend_contract_e2e.py::test_forwarder_e2e_flag_on` fail identically at baseline (the latter deterministically, even in isolation — not load-dependent); `test_phase1_db_layer.py::test_consolidation_completes_no_violations` is a SurrealKV write-conflict retry-class flake and `test_vacuum_backup_safety.py::test_real_nightly_main_exits_zero_no_contention` a resource-contention flake (both pass on isolated retry). Each of the 8 e2e tests this car targets was re-verified green across three independent full-suite runs; a different subset of these four unrelated tests failed on each run, consistent with pre-existing suite instability under parallel xdist load rather than anything this car touched. `_run_landscape_backend` still takes `project_id` directly and bypasses `RecallScope`/`unscoped` entirely — `mode="landscape"` with an empty project is unhandled, deliberately left for a future car.

**fix(ci): Car G5 — the engine-#2 job ran end-to-end and still skipped all 41 tests, and the artifact could not say why.** Core `5.181.47` → `5.181.48`; backend unchanged (no `yadgar/backend/**` edits).

**Root cause, confirmed live on the actual self-hosted GitHub runner (`github-runners_runner_1`, read-only `podman exec`/`docker ps` diagnosis, no state mutated).** The runner is itself a container with a `docker:dind` sidecar. Both `podman` and `docker` binaries are on its PATH, but only `docker` — proxied through the sidecar — actually works: `podman info` / `podman run` fail INSTANTLY with `Error: cannot re-exec process` (exit 125), a rootless-podman-cannot-nest-in-a-container failure, reproduced directly. The workflow's `command -v podman || command -v docker` assert and all four `test_mariadb_*.py` fixtures' `shutil.which("podman") or shutil.which("docker")` runtime selection both check PRESENCE, not FUNCTION, and both prefer podman — so the broken binary was always selected, always failed a container start instantly, and every dependent test hit the fixture's `pytest.skip(f"could not start MariaDB container: …")` path. Four independent fixtures failing in well under a second each reproduces the observed "41 skipped in 2.31s" exactly. Verified NOT an import problem: with `sql`+`test` extras installed and a real container runtime, `sqlalchemy`/`alembic` import fine, all 41 tests collect, and (with a working runtime) all 41 PASS in ~50s locally.

**Two defects fixed in both CI mirrors.** (1) The runtime assert now runs a real throwaway container per candidate runtime (`<runtime> run --rm alpine:3 true`) instead of `command -v`, and fails loudly only when NO runtime actually works. (2) `pytest -rf` (failures only) discarded every skip's reason, which is why the artifact showed 41 `s` characters and nothing else; changed to `-ra` (every non-passing outcome) in both mirrors.

**The test fixtures themselves needed the same fix, or the (now-correct) assert would pass while the suite still skipped.** `yadgar/tests/integration/_podman.py` gains `select_container_runtime()` — probes `<runtime> info` (near-instant, same env as the real container commands) and returns the first WORKING runtime, still preferring podman over docker when both work. `test_mariadb_connection.py`, `test_mariadb_dump_arm.py`, `test_mariadb_restore_arm.py` now call it; `test_mariadb_migrations.py` keeps its own copy per its established (pre-existing) precedent of not importing `_podman.py`. Verified directly (not just by inspection): with a shim `podman` on PATH that reproduces the exact "cannot re-exec process" failure, `select_container_runtime()` correctly falls through to docker; with both runtimes broken, it returns `None` in <10ms (no hang, clean skip path).

**A second, independent host precondition was missing and would have re-triggered the anti-vacuity gate on the very next run.** `mariadb-dump` / `mariadb` client binaries are absent from the runner (confirmed directly) — `test_mariadb_dump_arm.py` (2 tests) and `test_mariadb_restore_arm.py` (13 tests) check `shutil.which("mariadb-dump")`/`shutil.which("mariadb")` and would still skip even with the runtime fix landed. Both CI mirrors now install `mariadb-client` before the suite runs; the Forgejo mirror's install step is package-manager-portable (`apt-get` or `apk`, `sudo` optional) since that runner's host OS for a bare (no `container:`) job could not be confirmed the same way the GitHub runner's was.

**Unverified: only a live CI run settles whether the fix is complete.** The diagnosis (podman broken, docker working, mariadb-dump absent) is directly observed on the runner; the fix (functional runtime selection, mariadb-client install, `-ra` reporting) is exercised locally (shimmed-binary tests, full local suite pass) but not yet exercised on the actual GitHub Actions / Forgejo Actions infrastructure end-to-end. If anything else is still missing on that host, the next run's artifact will now say what and why, rather than 41 bare `s` characters.

**fix(engine-2): the relational migration has never run — a DDL-less account, and grants applied before the tables exist.** Core `5.181.43` → `5.181.44`; backend `5.72.33` → `5.72.34`.

**Engine #2's schema has been frozen at `0001_config` since 8 Aug: `alembic_version` and `config` exist, every ledger table does not.** Two independent defects, both in `entrypoint-backend.sh`'s `_bootstrap_mariadb_accounts`, each sufficient on its own.

**(1) The migration ran as an account with no DDL.** `_migrate_engine_two` drives `alembic upgrade head` over `sql_storage.engine`, which authenticates as `yadgar_app` — whose D19 grant is per-table `SELECT, INSERT, UPDATE, DELETE, REFERENCES` with no `CREATE`. Reproduced verbatim against `mariadb:11.4` and `mariadb:11.8`: `(asyncmy.errors.OperationalError) (1142, "CREATE command denied to user 'yadgar_app'@'localhost' for table `yadgar`.`task`")`. **(2) MariaDB rejects a table-level `GRANT` naming a table that does not exist** — `ERROR 1146 (42S02)` — and the heredoc runs without `--force`, so the first such statement aborts every one behind it. On a fresh install that is the FIRST grant (`alembic_version`): the app account is left with `USAGE` and the entrypoint prints `MariaDB account bootstrap failed — engine #2 present but unusable by the app`. On the long-lived host the abort landed one line later at `task`, which is why exactly two tables carry grants.

**The fix is a third account plus an ordering change, and NOT a wider runtime grant.** Bootstrap is now three phases: **A** database + both accounts (nothing table-level — no such grant can succeed against an empty database), **B** `python3 -m yadgar._shared.storage.sql.migrate` as a new `yadgar_migrate` account (password auth, an explicit `CREATE, DROP, ALTER, INDEX, …` list scoped to the engine-#2 database, no `GRANT OPTION`), **C** the app account's per-table narrowing — now that the tables exist. Phase C is gated on B succeeding: a partial chain plus the `REVOKE` would leave the runtime account with strictly LESS than it has today. The admin socket account cannot be used for B — it authenticates with MariaDB's `unix_socket` plugin and asyncmy implements only `mysql_native_password` / `caching_sha2_password` / `sha256_password` / `client_ed25519` (`asyncmy/connection.pyx:1141-1290`), so no asyncmy engine can ever connect as it; giving it a password instead would put a root-equivalent secret on disk. `_migrate_engine_two` reaches for the migration credentials itself and disposes that engine immediately, so no DDL-capable connection outlives the migration and the runtime engine is never widened.

**A failed bootstrap on a RUNNING mysqld is now fatal (exit 3, via the writers-first safe stop).** ADR-0222's rule one layer lower: ABSENT is not FAILED — mysqld missing or never ready still boots with a warning, but present-and-broken must not. This is why nobody noticed for months, and it also closes the hole that survives the lifespan gate: a phase-A failure leaves no usable option file, `_init_sql_storage` degrades to `None`, and `_migrate_engine_two` correctly SKIPS.

**The failure was undiagnosable, which cost a full cycle.** The error record carried `"error":"OperationalError"` and a truncated traceback — never the `(1142, …)` pair that says WHICH `OperationalError`. Two causes, both fixed: `JSONLogFormatter` truncated tracebacks from the FRONT, and a Python traceback puts the exception message LAST, so the one line naming the failure was the one line deleted (truncation now keeps head AND tail within the same `TRACEBACK_MAX_CHARS` budget); and `_migrate_engine_two` now logs `error_code` / `error_message` extracted from `exc.orig.args` via `describe_dbapi_error` — the DBAPI args rather than `str(exc)`, which on a DML path appends the statement and its bound parameters.

**Three test holes, all closed.** `yadgar/tests/integration/test_mariadb_migrations.py` provisioned its user via `MARIADB_USER=yadgar_app` on the stock image, which grants ALL PRIVILEGES — same username, opposite privileges, so the suite passed against a privilege set production has never had. It now REPLAYS the entrypoint's own heredocs, parsed out of the shipped file by `yadgar/tests/_entrypoint_sql.py` (no second copy to drift), from an empty database. `test_entrypoint_grants.py` could only text-match a list that names every table; it now also pins the two-phase structure, the migration's position between them, and that no phase ever grants the runtime account `CREATE`/`ALTER`/`INDEX`/`DROP`. And the whole engine-#2 suite had **never executed in CI** (marked `integration`; every job excludes that marker) — a new bare-host `test-engine2-integration` job runs it, wired into `test-gate`, with a missing container runtime and a SKIPPED test both treated as failures rather than green.

**fix(ci): Car F10 — `make ci-local` OOMs; make it match CI's process structure (PR #40 remediation).** Core `5.181.39` → `5.181.42`; backend unchanged (no `yadgar/backend/**` edits).

**`make ci-local` could not complete on a real box.** Measured twice: the default `TEST_TIMEOUT=5400` (90 min) got KILLed at 82%; `TEST_TIMEOUT=18000` got KILLed at 98% after 1h32m by `MemoryMax=20G` (`20G memory peak, 2.7G memory swap peak` per the journal). The root cause is structural, not a tuning problem: `ci-pr.yml` runs its four subsystem jobs (test-fast/test-shared/test-backend/test-core) as **four separate processes**, so CI never accumulates that much memory in one process. Car F5's target ran the same union of dirs and marker in **one** pytest invocation, which does.

**`make ci-local` now runs the same four subsystem jobs as four separate SEQUENTIAL pytest processes**, mirroring `ci-pr.yml`'s per-job process boundary — `scripts/ci-local-legs.sh` is the new leg runner, invoked once by the Makefile's (now much smaller) `ci-local:` recipe. `TEST_TIMEOUT` now applies **per leg** (each leg is its own `scripts/test-capped.sh` invocation) rather than to the whole four-leg run. `DIRS=` still works for the mid-work subset use case — `make ci-local DIRS=yadgar/tests/_shared` now runs exactly those dirs as **one** leg, bypassing the four-leg split entirely. Every leg runs to completion even if an earlier leg failed (one run gives the full picture, never a first-failure abort); a per-leg PASS/FAIL summary prints at the end, and the target exits non-zero iff **any** leg failed — this repo has shipped five vacuous "ran everything, reported success because the last thing passed" gates already this train; this is not a sixth. `-n`/`--reruns`/`--dist` are unchanged from Car F5 — nothing was traded away to fit in memory.

**The drift guard could not have seen this defect, and now can.** `scripts/check_ci_local_parity.py` (Car F5) compared only the directory union and the marker expression — a single-process target and a four-process target with an identical union+marker look IDENTICAL to that comparison. The guard now ALSO pins the **leg structure**: the set of CI subsystem jobs (discovered by SHAPE — a job block with ≥1 `yadgar/tests/.../` path and a QUOTED `-m '...'` marker, not a hardcoded 4-name list, so a fifth same-shaped CI job is picked up automatically) must match, 1:1, the set of `CI_LOCAL_DIRS_<leg>` Makefile variables AND the set of `run_leg <leg> ...` calls in `scripts/ci-local-legs.sh`, with each leg's dirs matching its job's dirs exactly. Verified to discriminate on scratch copies: collapsing the four `run_leg` calls back into one lumped invocation fails loudly (`the legs have been collapsed back into one lumped invocation, the exact regression Car F10 fixed`); adding a same-shaped fifth job to `ci-pr.yml` with no matching leg fails loudly (`ci-pr.yml's subsystem jobs test [...] but the Makefile's CI_LOCAL_DIRS_<leg> variables do not`); the real files pass clean.

**test(scoping): Car F6 — the "cross-project leak" is F4's stale premise again, and the SQL push-down had no test at all (PR #40 remediation).** Core `5.181.38` → `5.181.39`; backend unchanged (no `yadgar/backend/**` edits).

**`test_agent_in_proj_A_does_not_see_proj_B_writes` was the PR's blocking finding, triaged as a real cross-project leak. It is not one.** Car F1's vector-null fix let the test reach its assertion for the first time, and the assertion failed — but the failure is the same shape F4 found: the test's `_insert_memory_direct` stamped **both** rows with the caller's `owner/repo` and varied only `directory_context`. Instrumented against a live embedded engine, every stage-1 query the recall emits carries `((project_id = $sc_pid OR $sc_reach IN tags))` — memory FTS, memory vector, both wiki arms — plus `project_id = $pid` on the profile/belief arms, and **not one of them mentions `directory_context`**. A fourth seeded row under a genuinely different project was excluded; a fifth carrying the `global` reach tag was admitted. Cross-project scoping works; the test was asserting a directory mechanism Car C7 retired.

**Introduced by `a7dc951d` (C13), and the reasoning there was right for the rest of the file.** That commit stamped one shared `_TEST_PROJECT_ID` across the direct-insert helpers because "these fixtures vary the DIRECTORY axis" — true of the wiki tests around it, since §25 slug resolution and `wiki_list` are still genuinely directory-keyed. Recall is the one call in the file where the axis had already moved. Its setup now varies `project_id`; the assertion is unchanged.

**Two tests added, and the second exists because the mutation check caught the first one passing for the wrong reason.** Gutting `build_project_scope_clause` to return `("", {})` leaves the result-level assertions **green** — `MemoryProvider.candidates` runs `is_project_eligible` over everything the retriever returns, so the Python residual guard alone still yields a correctly-scoped result *list*. What it cannot restore is what C7 exists for (ADR-0206): a filter that runs after the query has already spent its `LIMIT` silently under-returns. So `test_every_candidate_query_carries_the_scope_predicate` asserts on the emitted SQL — every `ORDER BY … LIMIT` query over `memory`/`wiki_page` must bind `sc_pid`/`sc_reach`, with the `WHERE id IN [...]` hydration reads exempt (they spend no budget). Dropping `scope_sql` from the memory FTS arm alone leaves all 27 `test_c7_recall_scope_clause` tests and both result-level tests green and fails **only** this one. `test_reach_tagged_row_from_another_project_is_still_admitted` pins the other arm end-to-end: two rows, same foreign project, same foreign directory, separated only by the `global` tag.
...
**The assertions are unchanged in strength; their SETUP now creates the condition they name.** `_insert_mem` takes an explicit `project_id`, the aws rows are seeded under `other-owner/aws-work`, and `test_directory_arg_changes_results` became `test_project_arg_changes_results` — a rewrite, not a rename, because its proof technique was "hold `project=` fixed, vary `directory=`, watch results change" and `directory=` no longer changes anything. Held to the identical two-directional shape on the axis that is load-bearing. One tautological `assert … or True` was replaced with the real assertion it was hiding. `TestProjectBriefWikiScoping` was **left alone**: `list_wiki_pages` is still genuinely directory-keyed, so its directory language is accurate, not stale.

**`test_v5_65_directory_required.py` had the same premise failure one layer up, and pinning its contract required moving where the pin lives.** `_filter_prompt_recall_results` is already re-keyed onto `is_project_eligible(project_id, tags, caller)`; the fixture rows carried no `project_id` and the fake request sent no `?project=`, so `hook_project_id` raised, `_scoped` fell to `None`, and the filter skipped by design. Rows now carry projects and the request carries one — including a `global`-REACH-TAG row rather than the abolished `project_id='global'` value. The "a directory with no project must never forward unscoped" contract is pinned on **`_forward_hook_recall`**, where the guard actually is: the sibling tests patch `_recall_with_timeout` to inject a corpus, which bypasses the guard entirely, and a pin placed there would assert a state production cannot reach.

**Three tests were failing because the system now works as designed (ADR-0227 deleted every `project_id` fallback), so they were given a real identity rather than a relaxed assertion.** `TestOmitPortableEmbedded` (a SurrealKV `OMIT` parse test), and both `test_recall_forward_only_*`. The last of those is the instructive one: `UnresolvedProjectError` is **not** a `RuntimeError`, so it escaped the test's `except RuntimeError` and the test redded on the wrong error while asserting nothing about the backend-down path it exists for.

**Two more were stale test-side state, fixed; three are PRODUCTION defects this train introduced, diagnosed and deliberately NOT fixed here.** Fixed: a `RulesEngine.__new__` construction that skips `__init__` and therefore missed C10's new `_reported_unmigrated` latch, and a rule written with `scope="directory"` — a kind C10 retired and now rejects, re-keyed onto `scope="project"`. **Not fixed (reported):** `_sentinel_memorize` (`http.py:357`) calls `memorize` with no `project=`, so every session-end sentinel import is rejected `unresolved_project` and the sentinel file is never consumed; and C10(f) now stamps `directory_context` from the **resolved project_id**, while `_check_session_end_sentinel` still queries `WHERE directory_context = $dir` with a filesystem path — measured: rows store `directory_context='test-owner/test-repo'`, the reader looks for `'/tmp/sentinel_signals_test'`. The session-end capture feature is dead in production, not merely in its tests, and the fix is a design decision rather than a test edit.

**fix(tests): Car F3 — CI run 31567583261's self-inflicted gates: a stale import allowlist, an empty-allowlist assumption invalidated by its own file, and three signature-drift assertions (PR #40 remediation).** Core `5.181.33` → `5.181.36`; backend unchanged (no `yadgar/backend/**` edits).

**Group 1 — `_identity_mint`'s import allowlist was stale, not the import.** `yadgar/core/cli/_shared.py::resolve_cli_project` is a third legitimate host-side minting site, not a container-side derivation ADR-0227 forbids: `entrypoint.sh`'s container CMD is `yadgar --transport streamable-http` (the MCP server only), while `drain`/`restore`/`seed`/`capture`/`context` run as separate host subprocesses via the `yadgar` console-script — invoked directly by hook scripts (e.g. `pre-compact-drain.sh` calling `yadgar drain`) with no SessionStart-minted value to inherit, the same "this process can see the git worktree, the containers cannot" carve-out the module's own two existing entries already document. Widened `_ALLOWED_IMPORTERS` in `test_identity_mint.py` to three entries with that reasoning recorded, and synced `_identity_mint.py`'s docstring (which still named only two) to match.

**Group 2 — `test_shipped_allowlist_is_wellformed` asserted non-emptiness; its own name promises well-formedness.** 0047 C15a emptied `.test-weakening-allowlist.json` on purpose, per that file's own `_stale_policy`: both prior entries' source files had left the merge-base diff for good once the ADR-0215 train merged, so the entries could never stop warning. Rewrote the test to route through `check_diff("", None, None, allowlist=load_allowlist(...))` — the exact validation path production runs — so it checks well-formedness of whatever is present rather than presence itself; empty is now a legitimate governed state. Added `test_corrupted_allowlist_copy_is_rejected`, which corrupts a scratch copy (non-negative `allowed_delta` + a too-short `rationale`) and proves the rewrite still surfaces `MALFORMED`.

**Group 3 — three signature-drift assertions, each traced before touching it; all three are legitimate "not supplied" sentinels, not production bugs.** `_forward_restore`'s own docstring: a `None` project_id means restore's memory-backed sinks (anchor/hot-memory/gap) come back **empty**, never widened to the corpus — narrows, doesn't unscope. `CheckpointRestore.pre_compact_drain`'s `get_active_checkpoint(directory, project_id=project_id or "")` falls back to the legacy `directory` key when `project_id` is absent — always scoped by directory, `project_id` is an additional key C11 is still backfilling, not a replacement. Updated `test_cli_shared_module.py::TestForwardRestore::test_delegates_to_forward_restore` and the two `pre_compact_drain` call-arg assertions in `test_restore_forward_unit.py` to expect the `project_id=None` keyword now present, with the trace recorded inline as the premise-false justification `check_test_weakening.py` requires.

**fix(0047 F2): `insert_profile`/`insert_belief` stop binding SQL `NULL` into `project_id` (PR #40 CI failure class C).** Core `5.181.33` → `5.181.35`; backend `5.72.31` → `5.72.33`.

Migration 033 declares `project_id` `option<string>` on nine tables — `option<string>` accepts `none | string`, and SurrealDB's coercer rejects a bound Python `None` as `NULL` (a distinct value from `NONE`). `user.py::insert_profile` (`user_profile`, both CREATE branches) and `narrative.py::insert_belief` (`derived_belief`) bound `project_id = $pid` unconditionally with a possibly-`None` value, unlike their four sibling writers on the same table set (`blocks.py::create_block`, `episode.py::insert_episode`, `ops.py::insert_checkpoint`, `narrative.py::insert_narrative_entry`), which already route through `project_id_set_fragment()` (`_project_id_writer.py`) to emit a `NONE` literal instead of a bound `None`. Both fixed sites now use the same helper — `(a)`, not `(b)`: neither writer has a live production caller today (grepped tree-wide; both are exercised only by `tests/_shared/test_bitemporal_extension.py` and `tests/e2e/test_phase1_db_layer.py`, none of which pass `project_id`), so there is no caller-forgot-to-thread-identity defect to surface per ADR-0227 — these are general storage-layer CRUD primitives, architecturally identical to their four already-fixed siblings, not session-bound hot-path writes like `memory.py::insert_memory` (which correctly raises `UnresolvedProjectError` via `_resolve_project_id_for_write` because it has a real sessioned caller). `test_bitemporal_extension.py` (unmodified) reproduced the verbatim CI error and is now green; mutation-checked (revert → red, restore → green).

**fix(storage): Car F1 — SurrealDB `NONE` is not `NULL`, and C7's scoped vector arms only guarded one of them (PR #40 CI failure class B).** Core `5.181.33` → `5.181.34`; backend `5.72.31` → `5.72.32`.

**CI run 31567583261 died on `Incorrect arguments for function vector::similarity::cosine(). Argument 1 was the wrong type. Expected `array<number>` but found `NULL`` — with `embedding IS NOT NONE` ALREADY in the WHERE clause C7 wrote.** That is the whole point of the bug: **SurrealDB's `NONE` and `NULL` are different values, and each guard admits the other's**. Probed directly against a scratch engine over the HTTP transport CI uses: a row with `embedding = NULL` reports `is_none: false`, `is_null: true`, and `embedding IS NOT NONE` → **true**, so it sails through the filter and hands a non-array to `cosine()`. The converse was probed too — `embedding IS NOT NULL` alone ADMITS a `NONE` row — which is why the fix adds a second guard rather than swapping the first (`backend/graph/graph_api.py:320` already carries a comment about the same trap in the opposite direction). `type::is::array(embedding)` was measured and REJECTED: it works embedded but returns HTTP 400 in server mode, so it is not portable across the two transports this codebase runs.

**The competing hypothesis — that SurrealDB type-checks the SELECT projection independently of the WHERE, making any guard useless — was tested and REFUTED**, not reasoned away. A `SELECT cosine(embedding, $qv) FROM memory WHERE content = 'real'` over a corpus containing four non-array rows returns the one matching row and no error. The projection is evaluated only for rows the WHERE admits; the guard was simply admitting the wrong rows.

**Fixed on both sides of the seam, because each side hides the other's regression.** READ: the three brute-force cosine shapes take `embedding IS NOT NONE AND embedding IS NOT NULL` — `storage/vector.py::search_vectors` (scoped arm), `storage/wiki.py::search_wiki_vectors` (scoped arm), and `storage/wiki.py::search_wiki_vectors_tagged`, which is the worst of the three because it is brute-force **unconditionally** and so was exposed on every call, with or without a scope. The KNN arms are untouched and unaffected — probed: the HNSW index never offers a non-array row as a neighbour. WRITE: four sites stopped minting `NULL`. Over the HTTP transport a Python `None` bound into `embedding = $p` serialises to JSON `null` and lands as SurrealDB `NULL`, so `_build_memory_insert_clause`, `update_memory_compression`, `insert_wiki_page`, and `update_wiki_page` now emit a literal `embedding = NONE` — the convention `_build_memory_insert_clause`'s own docstring already stated for its other optional fields. `update_wiki_page` was a **live** production NULL source: `WikiStore._compute_embedding` returns `None` when the embed service is down and `WIKI_EMBED_FAILURE_BLOCKS_WRITE` is off.

**The repo had already written this defect down, in the file the C7 guard was added to.** `storage/wiki.py::get_wiki_pages_without_embedding` says verbatim: *"SurrealDB distinguishes NONE (field absent) from null (explicit null). We catch both: rows inserted via JSON params receive null (not NONE) when embedding=None is passed."* It has filtered on `IS NONE OR IS NULL` all along — 500 lines above the search method that guarded on one.

`yadgar/tests/_shared/test_vector_null_embedding_guard.py` (8 tests) pins both sides **independently**, which is not decoration: seeding the NULL row through `insert_memory` would make the read-guard tests pass on the writer fix alone, so they seed it with raw SQL instead. Mutation-verified in both directions — reverting only `AND embedding IS NOT NULL` reddens exactly the 3 read-guard tests (writer tests stay green); reverting only the writer changes reddens exactly the 3 writer tests (read-guard tests stay green). Two positive controls, one per arm, because a guard that excluded EVERYTHING would also "not crash": a real-vector memory must still surface from the scoped search, and a wiki page written WITH an embedding must still store the array (that one is mutation-verified too — neutering the conditional splat in `insert_wiki_page` reddens it, which is the failure mode where every new wiki page silently becomes unsearchable).

**Not swept: pre-existing NULL rows.** The writer fix stops new ones; it does not clean up history. A live corpus may still hold `embedding = NULL` rows written before this car. Reads are safe regardless — that is what the read guard is for — and the wiki backfill (`IS NONE OR IS NULL`) already finds them, but `stats.py:439`'s `dq_null_embedding_count` filters on `IS NONE` alone and therefore **under-counts** them. Neither is fixed here.

**Measured across the 6 affected modules: 5 failed / 114 passed before, 3 failed / 123 passed after (7 modules, incl. the new file) — and the 3 survivors are a DIFFERENT failure class, deliberately left alone.** `test_other_project_excluded` and `test_directory_arg_changes_results` were already failing on `assert mid_aws1 not in result_ids` before this car, never on the crash. `test_agent_in_proj_A_does_not_see_proj_B_writes` DID die on the NULL crash and now reaches its assertion — where it reports `proj-B memory leaked into /proj/A recall`. That is a real cross-project scoping leak the crash was masking; it is **not** fixed here, because fixing a leak is not fixing a crash and conflating them would bury one of the two. Named for whichever car owns the scoping class. `yadgar/_shared/storage/wiki.py` sat at exactly the 1000-line I30 HARD cap, so the +17 net lines took a `file_loc` allowlist entry with the rationale (comments were compressed from +32 first; the canonical account lives in `storage/vector.py`).
...
**Five properties the runbook exists to carry, none of them visible from the code, each learned the expensive way across 28 cars.** Core and backend images must deploy **together** — C7 re-keyed `RecallRequest` (`embed_service_models.py:60`), which is `extra="forbid"` with `project_id` required, so a split deploy 422s every recall in both directions and has **no working window**. The registry guard goes **after** the rows, never before: it raises `UnknownProjectError` on an unregistered key (`registry.py:158`), so enabling it against an empty registry makes every key unregistered and **bricks every write**. The backfill precedes the read flip, or a `project_id`-keyed WHERE over `NONE` rows returns zero results — a quiet, total outage of retrieval. `seed_agent_prompts` seeds from **deployed** code, so C14's corrected agent-facing prose cannot reach the live corpus from a worktree; **re-seeding is a deploy step**, and skipping it leaves the live pages teaching a removed API. And the nightly sweep **has never selected a row in production** (C15a found the swallowed `TypeError`), so its first post-deploy run is its first real execution and the 500-row circuit breaker is unexercised — measure candidate counts first.

**Verification is one query per migration with a named expected value, never a health check.** ADR-0222 measured migration 029 aborting while the container reported `active (healthy)`, `NRestarts=0`, `Result=success` — so "it booted" is not evidence, and §3.4 is the only evidence that exists. Rollback is restore-from-snapshot: `004_agent_pattern_model_client.py:145-153` states in its own docstring that `downgrade()` is reversible **only while the tables are empty**.

**§8's rehearsal was NOT executed, and the exit criterion stays UNTICKED rather than reinterpreted.** Measured, not reasoned: the VM it names is unreachable (`ping` 100% loss, TCP/22 closed, `virsh list --all` defines no domains) and the snapshot it names is absent (oldest surviving is `…20260809_020535`). **A local substitute was rejected rather than improvised** — this box runs the production daemon, and a second engine on a restored copy is one path argument from the live corpus. §4.1 records the ephemeral-engine tier that *was* run (`make e2e` 182 passed / 4 skipped; 11 targeted modules, 214 passed) and, per step, what it does **not** cover — result-set equivalence (§8 step 10b) and the pre-backfill read window (step 5b) are the two real gaps, both carried forward into §3.6. Marking them green would convert a loud failure into a silent one, which is §8's own opening argument.

**Plan lifecycle — three breaks, three different resolutions, because they are three different failures.** The remediation plan was archived by `git mv` as C16's first commit (ADR-0082). **The parent plan was NOT archived**: its §6.2 cutover has never run, so ADR-0081's partial-ship clause applies and it stays live with a header naming exactly what remains — archiving it would make a live, unexecuted migration invisible, the inverse of the failure ADR-0081 exists to stop. Its stale `BUILD-READY … no code exists yet` line is **kept and marked falsified**, not deleted. The Car K doc's `Status: shipped` was **re-verified rather than rubber-stamped**: C15a made its two central claims true (`task.completed_at` in `002`; the RED at `test_archive_sweep_car_k.py:447`), and a third divergence surfaced while checking — §2 names `consolidation/archive_sweep.py`, but the sweep shipped as an admin op at `admin_exec/nightly_sweep.py`. **D32② was corrected**: the cycle it argues against is a phantom, because `project.key_override` has **zero code readers** (grep over every `.py`/`.sql` finds it in four markdown docs and nowhere else); the conclusion stands on its two live reasons instead.

**`[VERIFY]` — `check-plan-signature-drift` was out of remit twice, and neither exclusion should be widened.** It excludes `docs/plans/archive/**` (`check_plan_signature_drift.py:400`), so it never opened the Car K file; and it matches call kwargs against in-corpus `def`s, so a **prose claim about a test's existence** is categorically outside what it models. Widening it would make it cry wolf across 60 plan docs — which its own docstring already names as strictly worse than no gate.

**feat(lint): Car C15 — the ADR-0225 `directory` residue sweep, green on arrival (PR #40 remediation, §5.C15).** Core `5.181.31` → `5.181.32`; backend unchanged (no `yadgar/backend/**` edits).

**ADR-0225 named a residue sweep as *the* enforcement mechanism — "without it the ADR is a code-review promise" — and none existed.** `scripts/check_directory_residue.py` is that sweep, wired as pre-commit hook 45 (`always_run`, `pass_filenames: false`). It fails in **both** directions, the shape of `check_capability_coverage.py`: **Direction 1**, a residue token in a scoping position with no allowlist entry; **Direction 2**, an entry whose path matches no scanned file *or* whose files no longer carry residue.

**It lands last so it ratchets what the train achieved, not what the plan hoped for.** C14 measured that `directory` was **not removed** — C5 removed its ability to *resolve*, and the parameter survives on 46 tools in two classes: the `resolve_effective_project` class (raises without `project=`; `directory` accepted-but-inert) and the `accept_project_param` class (**still directory-keyed**). A lint flagging every `directory=` would be red on arrival and wrong. The `tool-surface` tag names which class each file is in, so the next reader cannot "finish the job" and break live scoping.

**Direction 2 is a HARD FAIL, deliberately unlike `.test-weakening-allowlist.json`'s warn-only `_stale_policy`.** Two clauses: that guard diffs a merge-base that MOVES, so its entries go stale through nobody's fault, while this lint's input is the filesystem and a stale entry always has an owner; and C15a just demonstrated the alternative — two warn-only entries sat stale from the ADR-0215 train's merge until a car finally deleted them.

**The cross-car stale-entry class is closed structurally, not procedurally.** A sweep in one tree emptying a file stranded a *sibling's* allowlist entry twice on this train. The mechanism was never carelessness: C9a and C9b are **pytest modules**, so `pre-commit run --all-files` never ran them and ADR-0218 forbids the full suite — a car sweeping `backend` was structurally unable to see `_shared` go red. One whole-repo hook against one allowlist fixes that, and the lint additionally AST-parses `_ALLOWLIST` / `_SWEPT` / `ALLOWLIST` out of the two sibling modules (never importing them) and hard-fails on an entry whose subject no longer exists. Subject existence **only** — the three matchers differ by design, so cross-judging "residue is gone" would emit false verdicts. **All three siblings are kept; no test was deleted, weakened or shrunk.**

**The matcher is an AST walk over four scoping positions** — parameter names, call-site keyword args, string-literal dict keys/subscripts, class-body fields — never a text scan: ~1/3 of the ~2 400 raw `directory` hits are the English word in prose. Deliberate blind spots are recorded in the docstring per ADR-0080 rather than left implied: attribute reads, local variables, `getattr`/`add_argument` string names, positional argument values, `yadgar/tests/**` (1 890 of the raw hits, legitimately directory-keyed while the second tool class is), markdown (which makes **carve-out 4 moot by construction**), and C10's six globs, of which three survive and cover 37 files as a block.

**Carve-out 2 is a class strip, not entries:** `directory_context` in a string-literal position is the stored column, alive until the next PR's drop migration because the C6 backfill derives from it — enumerating ~100 column reads would produce an allowlist that asserts nothing. A parameter of that name is a signature and does get an entry.

**`scripts/directory_residue_allowlist.txt` is promoted from C10's classification into the lint's allowlist** — 78 entries over six tags (`carve-out-1/2/3`, `tool-surface`, `wire-contract`, `legacy-key`), each with a reason ≥40 chars enforced by the parser — 40 to match the repo's governed-allowlist family (`.test-weakening-`, `.health-endpoint-`, `.urllib-httperror-close-`) and C9a's own floor, so the lint promoting three siblings is not the weakest of the four on the one field reviewers read; malformed rows, unknown tags and duplicate paths hard-fail. 25 of C10's original entries were dropped rather than carried: they came from a word-boundary **text** scan and named files whose only `directory` is a positional argument, a local, an attribute read or prose — positions the AST matcher excludes by construction, so they would have been stale on arrival.

**The lint's first real run found a live bug, which is the point of a gate.** `scan_source` returned `[]` on a `SyntaxError`, so an unreadable file scored *clean* — C15a's `_parse_iso` failure in a new costume. Three files (`benchmarks/run_eval.py` ×3 sites, `benchmarks/run_longmemeval.py`, `docs/diagrams/generate.py`) parse on the venv's **Python 3.14** (PEP 758 permits an unparenthesized `except A, B:`) and **fail on the Python 3.13** that pre-commit's `language: system` hooks actually run — so two allowlist entries looked STALE, a false Direction-2 verdict visible only at commit time and only via a probe. All five sites are parenthesised with `# fmt: skip` (the repo's existing convention against ruff-format re-stripping them, already used at six sites), and an `UNPARSEABLE` violation class now makes the next one loud instead of silent. That class has **two arms**, because the `ast.parse` arm can only fire under a Python that *rejects* PEP 758 and is therefore structurally blind on 3.14: an anchored regex arm catches the bare form on either interpreter (the repo's existing `test_v5_46_16_except_tuple_sweep.py` covers `yadgar/**` only, which is exactly why `benchmarks/**` and `docs/diagrams/**` rotted), and a test pins all five sites so ruff-format cannot silently undo them. The lint is verified green under **both** interpreters.

**Anti-vacuity, given this train produced four vacuous passes.** Three measured floors (506 files walked, 408 residue sites, 80 sibling entries parsed) plus a planted-violation test built in `tmp_path` — not a committed fixture, which the repo-wide scan would itself find — whose assertions name the **planted subject** rather than merely counting errors. Five mutations prove each mechanism is load-bearing: removing `directory` from the token set blinds the matcher (44 sites, floor trips); blinding `scan_source` trips the floor at 0; deleting the `tools/recall.py` entry makes Direction 1 name that file; renaming a sibling binding raises `SIBLING UNPARSEABLE` instead of going green; a stranded sibling entry is named. The `branch` arm is documented as a **ratchet at zero** — ADR-0215 already removed branch scoping — and exact-identifier matching is pinned by a test so git's `default_branch` and Alembic's required module-level `branch_labels` survive.

**feat(ledger/sweep): Car C15a — the archive sweep gets two real retirement clocks (PR #40 remediation, §5.C15a).** Core `5.181.30` → `5.181.31`; backend `5.72.29` → `5.72.31`.

**`002_ledger_tables` is edited in place, not amended.** §0's licence holds: the revision CREATES both tables, `EXPECTED_HEAD` is still `0001_config`, and `002`/`003`/`004` have never run anywhere. `task.completed_at` and `adr.superseded_at` are added as `DATETIME NULL`, plus `UNIQUE(project_id, body_slug)` on `task` (parent-plan D8). **No migration was executed against any corpus.**

**Car K aged rows off the wrong two columns, and both were measurably wrong.** `updated_at` bumps on every edit, so editing a completed task reset its 90-day clock — the §14.2 regression by name. `created_at` measures age-since-authoring, so an ADR written 120 days ago and superseded today archived on the very next nightly run with **zero grace**. The sweep now reads `completed_at` / `superseded_at` through two single-source resolvers (`_task_retired_at`, `_adr_retired_at`) used by **both** the sweep and `_count_candidates`, so the circuit-breaker cannot count one population while the sweep archives another. NULL means "the clock never started", never "infinitely old". `superseded_at` falls back to `created_at` because `rejected`/`deprecated` rows are swept too and nothing supersedes them — a strict reading would have silently stopped archiving two of the three statuses.

**The RED the Car K plan promised at its `:151` and never wrote now exists**, and the pin it sat next to was inverted rather than deleted: `completed_at=_ago(120), updated_at=_ago(1)` still archives; `completed_at=_ago(10), updated_at=_ago(100)` does not. The ADR pins are the analogue plus the grace case. Nine assertions were RED before the source changed and green after.

**A pre-existing bug that made the whole sweep a production no-op.** `_parse_iso` took `str`; SQLAlchemy returns a `datetime` for a MariaDB `DATETIME`, and `fromisoformat(datetime)` raises `TypeError`, which the `except` swallowed — so against a real database every age comparison fell through the `is None` guard and **nothing was ever archived**, silently, with every test green (the fakes hand back ISO strings). It now accepts both shapes and reads a naive value as UTC, since the cutoff is tz-aware and mixing the two raises.

**Observability (#13, ADR-0074).** Three `exempt=` rationales in `nightly_sweep.py` claimed spans that did not exist — `_sweep_tasks_and_adrs` was exempt "because each leaf already carries its own @observe span" while **both** leaves were themselves exempt no-ops, so there were zero spans below the boundary and I33 still reported `MISSING=0`. All three become `span=False`, which ADR-0074 names as the right tool for loop helpers. The guard against a repeat is an AST scan that resolves callees both by name and from the function body, and it carries its own planted-violation test so it cannot pass vacuously.

**Seed idempotency (#16) — the premise is refuted, and the schema half is what shipped.** The constraint is in `002`, but the plan's RED ("run the seed twice, assert an unchanged row count") would pass **vacuously**: `seed_task_from_pages` resolves `_get_storage()` (SurrealDB), and `create_task_row` exists only on `MariaStorageEngine`, so `getattr(...)` is always `None` and the seed inserts **zero rows on the first pass too**. Separately, the `## task:<id>` schema has no `body_slug` field, so every row took the `""` default — which `UNIQUE(project_id, body_slug)` would have collapsed onto one row per project, breaking D35c's exact-equality gate the moment the seed started working. That default is now `None` (MySQL permits repeated NULLs), pinned by a test. Fixing the seed's storage handle is a new write path and belongs to Car E's owner.

**The slug collision note (#18, ADR-0202).** `_project_id_to_slug` now records that `a_b/c` and `a/b_c` both yield `a_b_c`, that this was accepted knowingly, and that "fixing" it with escaping would re-slug an immutable corpus.

**`mariadb.py` was refactored, not baselined.** It sat at 989/1000 against I13's HARD file cap. The five duplicated SELECT projections (three for `task`, two for `adr`, three for `agent_pattern`) are now one constant per table in a new `sql/ledger_columns.py` — the duplication was the hazard, since a reader that missed the new column would return rows without it and the sweep's filter would degrade to "archive nothing", exactly revision 031's dead-filter shape. Ledger SQL stays inside `MariaStorageEngine`: `check_ledger_chokepoint` sanctions by class name and its docstring records PR #32's `_LedgerMixin` MRO failure.

**Governance: `.test-weakening-allowlist.json` is now empty.** Its two ADR-0215 entries were permanently stale — both files are absent from merge-base `92dbb4b5` *and* HEAD, so neither could re-enter the branch diff and neither warning could ever clear. The file's own `_stale_policy` said "delete them then"; the guard printed "remove it from .test-weakening-allowlist.json" on every run. Guard now exits 0 with no warnings.

**docs(contracts/sdk/templates): Car C14 — the agent-facing docs stop teaching a key that no longer resolves (PR #40 remediation, ADR-0225/0227).** Core `5.181.29` → `5.181.30`; backend unchanged.

**The plan's framing was wrong in a way that changed every edit: `directory` was never "removed".** It is still a parameter on 46 tools — what C5 removed is its ability to RESOLVE. `_project_param.py` now has two tiers (`project` → `session_project` → raise); the `directory`-derivation and the `"global"` fallback are gone. So the fix is **add `project=` where it is missing**, never **delete `directory=`** — and the two are not interchangeable, because the tools split into two classes that a blanket sweep would have broken:

| class | tools | what actually scopes |
|---|---|---|
| `resolve_effective_project` | `recall`, `memorize`, `anchor`, `restore`, `wiki_add`, `adr_add`/`adr_list`/`adr_get`, `agent_prompt_save`, `bootstrap_project`, `update_active_work`, `seed_project` | `project` — a call without it RAISES |
| `accept_project_param` | `project_brief`, `audit_anchors`, `block_*`, `checkpoint`, `wiki_list`, `get_rules` | still `directory`; `project` is validated-only |

**Four agent-facing call sites were live `UnresolvedProjectError`s, not stale prose.** `session_project` is passed as a literal `None` at every production call site (only tests supply a value), so tier 2 is dead in production and a project-keyed call with `directory=` alone fails outright — verified by executing the resolver, not by reading it. Fixed: `stop_checkpoint_prompt.md`'s `adr_list`, `adr_add` and `restore` (the protocol violated its own rule at line 82, *"EVERY scoped call in this protocol carries `project`"*), and `anchor_audit_prompt.md`'s `recall`. The directory-keyed calls in the same files — `project_brief`, `audit_anchors`, `block_update`/`block_create` — are **correct as written and were left alone**.

**Two templates still defined `{project}` as "basename of `{directory}`" — the exact wrong-but-well-formed key ADR-0227 forbids.** `stop_checkpoint_prompt.md` had already been corrected; `anchor_audit_prompt.md` now carries the same minted-`owner/repo` definition, and `code_graph_refresh_prompt.md` — whose only writes are directory-keyed `block_*` calls — drops the unused placeholder rather than defining an identity it never needs.

**The seeded prose that binds every dispatch.** `agent_prompts.yaml`'s prelude **contract** (injected before every subagent dispatch — the single highest-leverage string in the repo) told agents to call bare `recall("…topic…")`; it now names `project` and says why omitting it is rejected rather than guessed. Same for the `adr-consult` discipline and three pattern prompts. Contract body is 1110 chars, well inside the 3500-char prelude cap. **Coverage boundary:** `seed_agent_prompts` re-seeds from DEPLOYED code, so the live corpus is unchanged by this car — re-seeding is a C16 post-deploy runbook step.

**SDK: `scripts/generate-types.ts` re-verified as a stub** (it prints notes and `process.exit(0)`s; live introspection is explicitly "v0.2 work"), so `src/generated/**` stays hand-edited. The SDK had **no `project` parameter at all** — every project-keyed call from a JS client would have failed — so 23 arg interfaces gain an optional `project?: string`, documented per class so the SDK does not re-teach the wrong scoping. README's primary `memorize` / `recall` / `anchor` examples gain it too. Additive by design: renaming `directory` would have broken both existing callers and the still-directory-keyed half of the surface. `tsc --noEmit` exit 0, 72/72 vitest, `verify-tool-coverage` 52/52.

**Contracts: measured, not assumed — and found NOT stale.** `check_contract_coverage.py` and `check_capability_coverage.py` both exit 0 before and after; the plan's counts (265 `BC-*` ids, 25 `directory` lines) reproduce exactly. The predicted CI failure never materialised. What WAS stale is prose the coverage lints cannot see, because they key on id↔test correspondence rather than meaning: BC-A1 documented a `memorize(directory=)` parameter that does not exist (it is `context`, and C10f stripped its scoping role), and BC-B1/B2/B3 + BC-G11 described recall's filter as directory-keyed — while their own green e2e tests now seed by `project_id` and comment that a row differing only by `directory_context` "is no longer excluded by anything". BC-B3's "raise on absent directory" is re-stated as the `UnresolvedProjectError` it actually is. All 265 ids preserved. BC-G1/G2 were left alone deliberately: both are `⏳[r]` unverified and `directory_context` is still a written column under ADR-0225 carve-out 2, so re-keying them would assert something this car did not measure.

**No hook file was deleted.** Plan item 4 is struck: hooks are installed and executed BY PATH, so an import-graph check finds zero importers for every hook in the directory and calls all of them dead. `core/hooks/prompt-recall.py` was not touched at all — a separate unpushed branch rewrites that exact file.

**feat(storage): Car C12 — migration 032 retires `wiki_page_version.branch` and the branch seeding kwargs (PR #40 remediation, ADR-0226).** Core `5.181.28` → `5.181.29`; backend `5.72.28` → `5.72.29`.

**Both of ADR-0215's deliberate survivors are revoked.** Migration 029 dropped `branch` from `memory` + `wiki_page`, but the implementing train left two things standing on purpose: `wiki_page_version.branch` (Car 9, as an audit-trail snapshot, with a boundary test that actively pinned it in place) and the low-level seeding kwargs `insert_memory(branch=)` / `insert_wiki_page(branch=)` / `anchor_memory(branch=)` (Car 10, because removing them turned ~33 tests red). ADR-0226: *"a history table holding a column the system has otherwise retired is a second source of truth for a concept that no longer exists"* and *"Thirty-three red tests is a cost, not an argument."*

**The DATA step is the substance of 032, not the DDL — and that was proven, not assumed.** `wiki_page_version` is SCHEMALESS and migration 013 never issued a `DEFINE FIELD` for `branch`, so on most databases there is no field definition to remove: a migration whose whole body is `REMOVE FIELD IF EXISTS` is a **no-op that still passes an `INFO FOR TABLE` assertion** — migration 031's dead-filter shape in different clothing. This was verified empirically before the real body was written: with a `REMOVE FIELD`-only body the new e2e row-level assertion fails while the field-definition assertion passes. So 032 mirrors 029's order — count, `UPDATE wiki_page_version SET branch = NONE WHERE branch != NONE` (literal SurQL `NONE`, never a bound Python `None`, which would store an explicit null the sweep would miss), a re-count that raises `Migration032Abort` if anything survived, then `REMOVE FIELD IF EXISTS` for symmetry. Rows are NULLED, never deleted: the version row is the audit trail; only the retired concept leaves. Replay is a no-op (count is 0 → guarded UPDATE skipped; `IF EXISTS` does nothing).

**Killing the writers is the safety property; the schema statement never was.** Because the tables are SCHEMALESS, a surviving writer re-creates the column untyped while `INFO FOR TABLE` still reports clean. Two of the killed writers were LIVE re-creation paths for a column 029 had *already* dropped — `insert_wiki_page(branch=)` appended `branch = $branch` to the **`wiki_page`** SET clause and `insert_memory(branch=)` did the same on **`memory`** — which is exactly ADR-0226's *"the kwargs … are in fact the exact mechanism by which the dropped column comes back."* Also silenced: the `ver_branch` binding in all three `wiki_page_version` snapshot paths (`insert_wiki_page`, `update_wiki_page`, `set_wiki_page_metadata`), `insert_wiki_page_version`'s `snapshot.get("branch")`, and migration 013's own seeder — where the dict key and BOTH SQL strings had to move together, because the server-mode path derives its `LET` preamble from `row.items()` and dropping the key alone would leave `$branch` unbound and crash the seeder at startup.

**Both boundary tests were REWRITTEN, not deleted.** 029's blast radius is genuinely unchanged and still asserted (a widened shipped migration is still worth catching); new siblings assert the other half — that 032 completes it, and that a real write through the storage chokepoint after both migrations puts nothing back. C11's "no `032*` registered" assertion was re-pointed rather than renumbered, as its own failure message instructed.

**Also fixes the episode filter C11 measured and handed over.** `backend/causal_discovery/pc.py` filtered `e["directory"] == project_id` — a filesystem PATH column compared against an `owner/repo` IDENTITY — so a real `project_id` selected **zero** episodes and `build_event_matrix` returned an empty matrix instead of a scoped one, silently, because an empty matrix is a legitimate "insufficient data" outcome. Latent rather than live: the only production caller (`discover_dag()`) passes no `project_id`, so the branch was skipped; the public facade is where it bit. Re-keyed onto `project_id` using `.get()` (pre-033 rows have no such key and the hard subscript would `KeyError`). A dual-arm `project_id OR directory` fallback was considered and **rejected as dead code**, with both measurements recorded at the site: a path never equals `owner/repo`, and `ActionLogger.capture` assigns `episode["directory"] = directory` unconditionally while recall passes `""`, so any episode a recall touched has had its `directory` blanked. The recall-originated gap (`_shared/runtime/recall_session.py` has no project in scope, correctly) is therefore DOCUMENTED and pinned by a test rather than papered over. The pre-existing `test_directory_filter` used one string for BOTH keys — passing under either keying, proving neither — and is rewritten with them deliberately distinct plus a negative case.

`make e2e` — the pre-push gate for this PR — goes from **175 passed / 4 skipped, exit 0** to **182 passed / 4 skipped, exit 0**. The delta reconciles exactly: 5 new tests in the 032 e2e module + 2 new siblings in the 029 one; no test vanished and the skip count is unmoved at 4. Removes the now-stale `.complexity-allowlist.json` entry for `anchor_memory`, whose own rationale named this sweep as the place to revisit it.

**feat(storage): Car C11 — migration 033 declares `project_id` on the other nine directory-bearing tables, and the writers dual-write (PR #40 remediation).** Core `5.181.27` → `5.181.28`; backend `5.72.27` → `5.72.28`.

**Migration 033 is schema-only, and that is the idempotency proof.** Nine tables — `memory_block`, `episode`, `action_log`, `runtime_config` (the legacy `directory` COLUMN owners) plus `checkpoint`, `narrative_entry`, `user_profile`, `derived_belief`, `wiki_page_version` (the SCHEMALESS `directory_context` users no `DEFINE FIELD` ever covered) — each get `DEFINE FIELD IF NOT EXISTS project_id … TYPE option<string>` and a matching `DEFINE INDEX IF NOT EXISTS`. There is no data step, no `SELECT`, and no derivation, which is the STRUCTURAL reason 031's defect cannot recur: 031's phase filter was dead code because it filtered on a column it never projected, and a migration with no row-touching statement has no such shape to get wrong. Asserted per statement (`IF NOT EXISTS`, `option<string>`, `DEFINE`-prefixed) and by replay-equality across three runs.

**The plan's fourth table, `queue`, DOES NOT EXIST — and no column was declared to make the count come out right.** The cited site (`_shared/storage/queue.py:81`) is inside `insert_action_log`'s docstring; the queue itself is file-backed (`_shared/file_queue/`), with no `DEFINE TABLE queue`, no writer and no row. `runtime_config` replaces it in substance: it carries its own `directory` column, had no `project_id`, and was absent from the plan's list — the GAP three separate cars (C9a, C9c, C10) reported upward with no owner.

**The writers DUAL-WRITE: they stamp `project_id` and KEEP writing the legacy column.** The plan's TDD line ("no `directory` value is written") is deferred to the drop PR with the reason recorded, because three live consumers read those columns today — `causal_discovery/pc.py` filters episodes on `e["directory"]`, `consolidation/cls.py` reads `ep.get("directory")`, and `consolidation/cleanup.py` takes the action-log row's `directory` as the summary memory's `directory_context`. More decisively, ADR-0225's stated reason for keeping the columns is that *the backfill derives from them*: a row written between this car and the drop with a `project_id` but no `directory` would have nothing for the backfill to derive from AND nothing for the un-backfilled readers to match. The ADR-0225/0226 "kill the writers" property is about the DROP — on a SCHEMALESS table `REMOVE FIELD` removes only the type definition while a surviving writer re-creates the column untyped and `INFO FOR TABLE` still looks clean. That warning is restated in 033's docstring for the next PR.

**No backfill covers these tables, so every re-keyed reader keeps a transitional legacy arm.** `project_backfill._TABLES` is `("memory", "wiki_page")` and §8 names no step for the rest, so the degraded-window sanction of §8 5b does not extend here. A `project_id`-only predicate on `memory_block` would not be a window — it would silently drop the user's own curated blocks out of every `restore()`, permanently. The two restore sinks C10g deliberately left path-keyed therefore move onto a two-arm predicate (`project_id = $pid OR <legacy> = $dir`), which is NOT the `project_id IS NONE` sentinel `build_project_scope_clause` refuses: it matches a specific legacy key the caller actually holds, not every unstamped row in the corpus.
