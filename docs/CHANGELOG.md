...
`yadgar/tests/_shared/test_vector_null_embedding_guard.py` (7 tests) pins both sides **independently**, which is not decoration: seeding the NULL row through `insert_memory` would make the read-guard tests pass on the writer fix alone, so they seed it with raw SQL instead. Mutation-verified in both directions — reverting only `AND embedding IS NOT NULL` reddens exactly the 3 read-guard tests (writer tests stay green); reverting only the writer changes reddens exactly the 3 writer tests (read-guard tests stay green). A positive-control test asserts a real-vector row still surfaces, so a guard that excluded everything could not pass.
