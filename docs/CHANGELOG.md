...
**The maintenance write-gate told an instance almost nothing true, and ten tools could not deliver even that.** Core `5.187.0` → `5.188.0`; backend unchanged. The gate payload was two keys — `{"error": "maintenance", "message": "yadgar maintenance in progress (vacuum); retry shortly"}` — and each part of it failed differently during the live 2026-08-20 21:00 vacuum. **"Shortly" understated the wait by two orders of magnitude:** measured, the gate engaged at 21:00:01, swap artifacts appeared at 21:06:07, `.old-` dirs cleared at 21:07:04, and the backend needed a further ~30s to come up AFTER the gate lifted — a SECOND window in which the gate is gone and calls still fail, for a different reason, which reads as a new problem. **"(vacuum)" was a guess:** nightly (`nightly_cycle` step 1) and the backup quiesce driver engage the same flag, so the message named the wrong job whenever they held it. **And every check an instance would run to confirm the window is real reports that no vacuum is running:** `GET /health` returned `status: ok` throughout, `yadgar-vacuum.service` read `inactive (dead)` (a trigger-file-initiated vacuum leaves the unit idle) as did `list-timers`, `state/yadgar/triggers/` was empty (consumed at start), and the on-disk artifacts do not appear until roughly halfway in. Four signals, four false negatives — one step short of a filed "stuck maintenance gate" production bug against a healthy system. The payload now carries `operation`, `phase`, `started_at`, `elapsed_seconds`, `typical_duration_seconds`, `expected_done_by`, `looks_stuck`, `retry_after_seconds`, `writes_were_rejected_not_queued`, plus `message` and `resume` prose that names each of those four lying signals so the reader does not go looking, and states that the ONLY reliable signal is a DB-backed call that succeeds. `retry_after_seconds` is a flat 60s rather than a curve, chosen so it provably clears the post-gate backend warm-up. **`typical_duration_seconds` is PER-OPERATION, because one flat figure makes the envelope lie about the long window and reintroduces the exact false signal it exists to delete:** nightly holds the gate across steps 1-7 and `_maintenance_entered_at` is stamped by the OUTER enter, so `elapsed` spans the whole cycle — at a vacuum-shaped 10 minutes a perfectly healthy night would read `looks_stuck: true` half an hour in and `resume` would open by telling the caller to stop and report a broken system. Provenance is unequal and is labelled as such in the table: 600s for vacuum is MEASURED, 3600s for nightly is DERIVED (sized so `looks_stuck` fires at 3h, past a healthy night and before `_NIGHTLY_MAINTENANCE_TTL_SEC`'s 6h self-heal), and an unrecognised operation falls back to 600. `writes_were_rejected_not_queued` is the field that stops silent work loss: a `task_write` inside the window consumed no id and left no row — confirmed empirically after it, by the next successful write taking the id the gated one would have — so "it'll drain later" is exactly wrong, and `resume` spells out the re-issue-and-re-read recipe.

**The gate now RAISES on the MCP path instead of returning, because no returned value could satisfy the tool surface.** The SDK derives an output model from each tool's RETURN ANNOTATION and validates the returned value against it, so the envelope dict was a pydantic failure for every `-> list[dict]` tool (`type=list_type`) and every `-> str` tool (`type=string_type`) — those callers saw a schema crash where a retry signal should have been. The affected set is **ten tools, not the four or five a grep of the obvious ones suggests**: `recall`, `task_list`, `wiki_query`, `wiki_list`, `block_list`, `dlq_inspect`, `bookmark_list`, `config_list`, `get_rules`, `agent_dispatch_prelude`. Three more (`task_get`, `memory_get`, `wiki_get`, all `-> dict | None`) silently nested the envelope under `{"result": …}`. Synthesising a shape-matched value per annotation was rejected: `[envelope]` for `recall` reports SUCCESS and injects an error dict into a result list, which is worse than the crash, and `-> str` has no sane value at all. A raise is annotation-independent, so it covers all 95 registered tools and whatever a future one declares; `str(MaintenanceGateError)` carries the prose then the structured fields as parseable JSON, since a `ToolError` reaches the caller as text only. The SYNC wrapper still RETURNS the dict — nothing but FastMCP ever holds the async wrapper, so the direct-call contract used by internal and test callers is untouched. The new sweep is driven off the LIVE tool registry rather than a hand-listed set, so a future tool with a new annotation is covered the day it lands.

**`GET /health` carries a `maintenance` block** (`active`, `operation`, `phase`, `elapsed_seconds`), so the one signal an operator reaches for stops contradicting every gated tool. **Additive only, and that is load-bearing:** `health_check` returns 503 on any non-ok `status` and P0 watches this endpoint, so scoring the window here would turn every nightly into a 503 and potentially a health-kill — a test pins `status == "ok"` while gated. An expired TTL is not reported as an active window (the gate owns the clearing, loudly, since a fired TTL means a vacuum died without cleanup). The block is applied on BOTH handler paths — `health_check`'s `TimeoutError` fallback builds its payload from scratch, and that path is more likely during a window, not less, since the backend is stopped and a dependency probe is what exhausts the budget. Note the endpoint can still answer 503 while gated for a reason this car does not touch: the pre-existing `#74` anti-flap counts consecutive `db` probe misses, which is the normal state during a vacuum. What is pinned here is that the maintenance block never itself changes `status`. The payload itself moved to a leaf `yadgar/_shared/runtime/maintenance.py` that both `_app.py` and `http.py` import, so `/health` reuses the envelope rather than duplicating elapsed/`looks_stuck` arithmetic — which is also what keeps the `http.py` delta to six lines. `operation` is set by the OUTER enter only, like `_maintenance_entered_at`: nightly labels the window at step 1 and its step-4 vacuum must not relabel it, because the window the caller is waiting on is nightly's. `phase` is the opposite — a nested enter IS the phase channel, so advancing it needs no new route — and it **ships null-until-driven**: wiring real transitions out of `vacuum/phases.py` is deliberately a separate change, and inventing a value would have been a fifth lying signal. **One test's MECHANISM changed, not just its expectations:** `test_real_gate_payload_matches_this_copy` guarded a hand-copy by GREPPING `_app.py` for each exact string, which a composed, interpolated message cannot satisfy at any wording; it now imports the builder and calls it, and there is no copy left to keep honest.
