...
- **Known gap, stated rather than papered over:** `wiki_update(fields={"content": ...})` calls `storage.update_wiki_page` directly in the backend admin op and never enters `WikiStore`, so it is not covered. Closing it needs either a core-side pre-check in the tool shell or the guard pushed into the storage layer, where it would also double-gate `discipline_save`'s own writes.
