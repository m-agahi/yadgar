...
**Two known-pre-existing families are left alone, both proven rather than assumed.** Five tests asserting semantic quality (`TestUnifiedRecall`, `TestRecallRanking`, `TestEmbeddingCacheHit`, `TestCoverageAssessment.test_coverage_sufficient`) fail only because `sentence-transformers` is absent from a `--extra dev --extra sql` venv, so `encode()` returns `None` — installing `--extra ml` (what `Dockerfile.ci` bakes) turns all five green with no source change. One astrocyte test fails under `-n 6` with `Transaction conflict` and passes at `-n0`; that is the xdist/live-DB collision family, not a scoping failure. Final: **1557 passed / 1 xdist-collision failure**, from 196. Core `5.181.11` → `5.181.12`; backend `5.72.11` → `5.72.12`.
