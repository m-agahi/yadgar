    ...
    *,
    ...
    for page in pages:
        ...
        try:
            ...
            tally["repaired"] += 1
        except Exception as exc:  # noqa: BLE001 — per-page isolation: update_wiki_page reaches storage, whose failures share no common base, and one unrepairable page must not abort the sweep
            tally["failed"] += 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    except Exception as exc:  # noqa: BLE001 — CLI top-level: StorageEngine() reaches config, filesystem and DB connect, which raise with no common base; the contract is exit-2-with-a-message
