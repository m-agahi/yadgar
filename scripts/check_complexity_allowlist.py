...
def run_check(repo_root: Path) -> tuple[list[str], list[str], list[str], list[str]]:
    ...
    # Stale/drift checks are scoped to the audited prefix (yadgar/) only.
    # Entries for other paths (scripts/, etc.) are governed by the I13 hook
    # + baseline, not by this invariant.
    audited_prefix = "yadgar/"
    audited_entries = [e for e in allowlist if e.path.startswith(audited_prefix)]
