    ...
    *,
    ...
    for page in pages:
        ...
        try:
            ...
            tally["repaired"] += 1
        except Exception as exc:  # noqa: BLE001 — per-page isolation: storage._q raises RuntimeError (SurrealDB), the httpx transport family and arbitrary embedded-SDK types with no common base, and one unrepairable page must not abort the sweep
            tally["failed"] += 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    except Exception as exc:  # noqa: BLE001 — CLI top-level: StorageEngine() reaches config, filesystem and DB connect, which raise with no common base; the contract here is exit-2-with-a-message, never a traceback
    ...
    try:
        ...
        try:
            storage.close()
        except Exception:  # noqa: BLE001 — close() in finally: a teardown failure must not replace the real exit status of the sweep above
            pass
